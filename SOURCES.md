...
| `randomize_kstack_offset=on` | ✅ | Per-syscall stack randomization |
| `pti=on` | ✅ | Meltdown |
| `vsyscall=none` | ✅ | Legacy table off |
| `iommu=pt` | ✅ | VFIO passthrough |
| `amd_iommu=on` / `intel_iommu=on` | ✅ | IOMMU enable |
| `nvidia-drm.modeset=1` | ✅ | GNOME Wayland |
| `lockdown=integrity` | ✅ | (NOT confidentiality — chosen for kexec compatibility) |
| `spectre_v2=on`, `spec_store_bypass_disable=on`, `l1tf=full,force`, `gather_data_sampling=force` | ✅ | Side-channel mitigations |
...
| llama.cpp server | `http://localhost:8080/v1` | ✅ | ✅ | ✅ via grammars | ⚠️ ignored | ❌ | ❌ | ❌ | ❌ | external |
| LiteLLM proxy | `http://localhost:4000/v1` | ✅ | ✅ | ✅ proxied | ✅ proxied | proxied | translates | proxied | proxied | proxied |
| OpenRouter | `https://openrouter.ai/api/v1` | ✅ | partial | ✅ | per-model | ❌ | ❌ | ❌ | ❌ | ❌ |

`⚠️ ignored` = the runtime accepts `strict: true` as an unknown field
