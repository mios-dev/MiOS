#!/usr/bin/env bash
# AI-hint: Offline bare-metal installer for MiOS. Performs bootc install to-disk --transport oci from staged oci-archive on MiOS-Repo/MiOS-Data.
set -euo pipefail
...
if (( DRY_RUN )); then
    echo "[install.sh] DRY-RUN: Would execute -> bootc install to-disk --transport oci-archive \"${TARGET_DISK:-/dev/sda}\" \"$OCI_ARCHIVE\""
    exit 0
fi
...
bootc install to-disk --transport oci-archive "$TARGET_DISK" "$OCI_ARCHIVE"
