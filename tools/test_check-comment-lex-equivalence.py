#!/usr/bin/env python3
# AI-hint: Fixtures for check-comment-lex-equivalence.py -- proves it runs clean on the shipped tree and that its exit code is meaningful rather than constant.
# AI-related: tools/check-comment-lex-equivalence.py, usr/share/mios/mios.toml, automation/98-drift-checks.sh
# AI-functions: main
...
TOOL = os.path.join(HERE, "check-comment-lex-equivalence.py")
...
def main() -> int:
    ...
    print(f"[test_check-comment-lex-equivalence] {PASSED} passed, {len(FAILED)} failed")
    for f in FAILED:
        print(f"  FAIL {f}")
