...
| `tools/install.sh` | `bootc-baremetal-disk-installer` | Offline bare-metal disk installer. Executes `bootc install to-disk --transport oci-archive`. **Note (AGY-1888):** Disconnected from automated USB/Ventoy kickstarts; requires manual invocation from a live shell. | **YES (Wipes Target Disk)** |
...
> [!WARNING]
> **Deployment Status Realities (AGY-1888)**:
> `tools/install.sh` provides the `bootc install to-disk` mechanism, but automated bare-metal deployment from Ventoy/USB media currently defaults to standard Fedora kickstart installation rather than executing `tools/install.sh`. Automatic bootc image deployment remains disconnected and unbuilt.
