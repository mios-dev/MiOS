...
_render_with_envsubst() {
    ...
    printf '%s' "$content" | envsubst '${MIOS_LOCALAI_IMAGE} ${MIOS_K3S_IMAGE} ${MIOS_CEPH_IMAGE} ${MIOS_FORGE_IMAGE} ${MIOS_SEARXNG_IMAGE} ${MIOS_HERMES_IMAGE} ${MIOS_HERMES_WORKSPACE_IMAGE} ${MIOS_WEBUI_IMAGE} ${MIOS_OLLAMA_IMAGE} ${MIOS_GUACAMOLE_IMAGE} ${MIOS_FORGE_RUNNER_IMAGE} ${MIOS_CROWDSEC_IMAGE} ${MIOS_POSTGRES_IMAGE} ${MIOS_GUACD_IMAGE} ${MIOS_PXE_HUB_IMAGE} ${MIOS_BIB_ALPINE_IMAGE} ${MIOS_PORT_SSH} ${MIOS_PORT_FORGE_HTTP} ${MIOS_PORT_FORGE_SSH} ${MIOS_PORT_LOCALAI} ${MIOS_PORT_COCKPIT} ${MIOS_PORT_COCKPIT_LINK} ${MIOS_PORT_OLLAMA} ${MIOS_PORT_SEARXNG} ${MIOS_PORT_HERMES} ${MIOS_PORT_HERMES_WORKSPACE} ${MIOS_PORT_WEBUI} ${MIOS_K3S_API_PORT} ${MIOS_GUACAMOLE_PORT} ${MIOS_CEPH_DASHBOARD_PORT} ${MIOS_RDP_PORT} ${MIOS_FORGE_USER} ${MIOS_FORGE_UID} ${MIOS_FORGE_GID} ${MIOS_LOCALAI_USER} ${MIOS_LOCALAI_UID} ${MIOS_LOCALAI_GID} ${MIOS_SEARXNG_USER} ${MIOS_SEARXNG_UID} ${MIOS_SEARXNG_GID} ${MIOS_CEPH_USER} ${MIOS_CEPH_UID} ${MIOS_CEPH_GID} ${MIOS_HERMES_USER} ${MIOS_HERMES_UID} ${MIOS_HERMES_GID} ${MIOS_OLLAMA_USER} ${MIOS_OLLAMA_UID} ${MIOS_OLLAMA_GID} ${MIOS_WEBUI_USER} ${MIOS_WEBUI_UID} ${MIOS_WEBUI_GID} ${MIOS_QUADLET_NETWORK} ${MIOS_QUADLET_SUBNET} ${MIOS_AI_DIR} ${MIOS_AI_MODELS_DIR} ${MIOS_AI_MCP_DIR}'
}
...
_render_with_bash() {
    ...
    for var in MIOS_LOCALAI_IMAGE MIOS_K3S_IMAGE MIOS_CEPH_IMAGE MIOS_FORGE_IMAGE \
               MIOS_SEARXNG_IMAGE MIOS_HERMES_IMAGE MIOS_HERMES_WORKSPACE_IMAGE MIOS_WEBUI_IMAGE \
               ...
               MIOS_PORT_HERMES MIOS_PORT_HERMES_WORKSPACE MIOS_PORT_WEBUI MIOS_K3S_API_PORT MIOS_GUACAMOLE_PORT \
               ...
               MIOS_HERMES_USER MIOS_HERMES_UID MIOS_HERMES_GID MIOS_OLLAMA_USER MIOS_OLLAMA_UID MIOS_OLLAMA_GID \
