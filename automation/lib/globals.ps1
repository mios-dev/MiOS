...
$script:MIOS_AI_ENDPOINT = if ($env:MIOS_AI_ENDPOINT) { $env:MIOS_AI_ENDPOINT } else { "http://localhost:$($script:MIOS_PORT_AGENT_PIPE)/v1" }
...
$script:MIOS_AI_TAG_TEACHER_PORT_KEY = if ($env:MIOS_AI_TAG_TEACHER_PORT_KEY) { $env:MIOS_AI_TAG_TEACHER_PORT_KEY } else { 'llm_light' }
...
$script:MIOS_BLADE_ARCHETYPES_HA_NODE = if ($env:MIOS_BLADE_ARCHETYPES_HA_NODE) { $env:MIOS_BLADE_ARCHETYPES_HA_NODE } else { 'controller,service-plane' }
$script:MIOS_BLADE_ARCHETYPES_HEADLESS = if ($env:MIOS_BLADE_ARCHETYPES_HEADLESS) { $env:MIOS_BLADE_ARCHETYPES_HEADLESS } else { 'service-plane' }
$script:MIOS_BLADE_ARCHETYPES_HYBRID = if ($env:MIOS_BLADE_ARCHETYPES_HYBRID) { $env:MIOS_BLADE_ARCHETYPES_HYBRID } else { 'gpu-serving,controller,service-plane' }
$script:MIOS_BLADE_ARCHETYPES_K3S_MASTER = if ($env:MIOS_BLADE_ARCHETYPES_K3S_MASTER) { $env:MIOS_BLADE_ARCHETYPES_K3S_MASTER } else { 'controller,service-plane' }
$script:MIOS_BLADE_CLUSTER_CONTROL_PLANE_HA = if ($env:MIOS_BLADE_CLUSTER_CONTROL_PLANE_HA) { $env:MIOS_BLADE_CLUSTER_CONTROL_PLANE_HA } else { 'false' }
$script:MIOS_BLADE_CLUSTER_K3S_SERVERS = if ($env:MIOS_BLADE_CLUSTER_K3S_SERVERS) { $env:MIOS_BLADE_CLUSTER_K3S_SERVERS } else { 1 }
$script:MIOS_BLADE_COLLAPSE_DWELL_S = if ($env:MIOS_BLADE_COLLAPSE_DWELL_S) { $env:MIOS_BLADE_COLLAPSE_DWELL_S } else { 30 }
$script:MIOS_BLADE_COLLAPSE_FAIL_CHECKS = if ($env:MIOS_BLADE_COLLAPSE_FAIL_CHECKS) { $env:MIOS_BLADE_COLLAPSE_FAIL_CHECKS } else { 3 }
$script:MIOS_BLADE_COLLAPSE_RECOVER_DWELL_S = if ($env:MIOS_BLADE_COLLAPSE_RECOVER_DWELL_S) { $env:MIOS_BLADE_COLLAPSE_RECOVER_DWELL_S } else { 120 }
$script:MIOS_BLADE_CPU_FALLBACKS_MIOS_LLM_HEAVY = if ($env:MIOS_BLADE_CPU_FALLBACKS_MIOS_LLM_HEAVY) { $env:MIOS_BLADE_CPU_FALLBACKS_MIOS_LLM_HEAVY } else { 'mios-llm-light' }
$script:MIOS_BLADE_CPU_FALLBACKS_MIOS_LLM_HEAVY_ALT = if ($env:MIOS_BLADE_CPU_FALLBACKS_MIOS_LLM_HEAVY_ALT) { $env:MIOS_BLADE_CPU_FALLBACKS_MIOS_LLM_HEAVY_ALT } else { 'mios-llm-light' }
$script:MIOS_BLADE_CPU_FALLBACKS_MIOS_LLM_WORKER_ = if ($env:MIOS_BLADE_CPU_FALLBACKS_MIOS_LLM_WORKER_) { $env:MIOS_BLADE_CPU_FALLBACKS_MIOS_LLM_WORKER_ } else { 'mios-llm-light' }
$script:MIOS_BLADE_DISCOVERY_HEALTH_PATH = if ($env:MIOS_BLADE_DISCOVERY_HEALTH_PATH) { $env:MIOS_BLADE_DISCOVERY_HEALTH_PATH } else { '/v1/models' }
$script:MIOS_BLADE_DISCOVERY_HEALTH_TIMEOUT_S = if ($env:MIOS_BLADE_DISCOVERY_HEALTH_TIMEOUT_S) { $env:MIOS_BLADE_DISCOVERY_HEALTH_TIMEOUT_S } else { 3 }
$script:MIOS_BLADE_DISCOVERY_ORDER = if ($env:MIOS_BLADE_DISCOVERY_ORDER) { $env:MIOS_BLADE_DISCOVERY_ORDER } else { 'localhost,mdns,tailnet,remote' }
$script:MIOS_BLADE_FALLBACK = if ($env:MIOS_BLADE_FALLBACK) { $env:MIOS_BLADE_FALLBACK } else { 'headless' }
$script:MIOS_BLADE_FENCING_DISKLESS = if ($env:MIOS_BLADE_FENCING_DISKLESS) { $env:MIOS_BLADE_FENCING_DISKLESS } else { 'true' }
$script:MIOS_BLADE_FENCING_METHOD = if ($env:MIOS_BLADE_FENCING_METHOD) { $env:MIOS_BLADE_FENCING_METHOD } else { 'sbd' }
$script:MIOS_BLADE_HARDWARE_MIN_AP_CAPABLE = if ($env:MIOS_BLADE_HARDWARE_MIN_AP_CAPABLE) { $env:MIOS_BLADE_HARDWARE_MIN_AP_CAPABLE } else { 1 }
$script:MIOS_BLADE_HARDWARE_MIN_INTERFACES = if ($env:MIOS_BLADE_HARDWARE_MIN_INTERFACES) { $env:MIOS_BLADE_HARDWARE_MIN_INTERFACES } else { 2 }
$script:MIOS_BLADE_PLACEMENT_CONTAINERS = if ($env:MIOS_BLADE_PLACEMENT_CONTAINERS) { $env:MIOS_BLADE_PLACEMENT_CONTAINERS } else { 'k3s' }
$script:MIOS_BLADE_PLACEMENT_FAILOVER_ORDER = if ($env:MIOS_BLADE_PLACEMENT_FAILOVER_ORDER) { $env:MIOS_BLADE_PLACEMENT_FAILOVER_ORDER } else { 'local,localhost,cluster' }
$script:MIOS_BLADE_PLACEMENT_VMS = if ($env:MIOS_BLADE_PLACEMENT_VMS) { $env:MIOS_BLADE_PLACEMENT_VMS } else { 'pacemaker' }
$script:MIOS_BLADE_PLANES_AI_OWNER = if ($env:MIOS_BLADE_PLANES_AI_OWNER) { $env:MIOS_BLADE_PLANES_AI_OWNER } else { 'either' }
$script:MIOS_BLADE_PLANES_AI_ROLE = if ($env:MIOS_BLADE_PLANES_AI_ROLE) { $env:MIOS_BLADE_PLANES_AI_ROLE } else { 'the OpenAI-compatible front door and the lanes behind it' }
$script:MIOS_BLADE_PLANES_AI_WIRED_BY = if ($env:MIOS_BLADE_PLANES_AI_WIRED_BY) { $env:MIOS_BLADE_PLANES_AI_WIRED_BY } else { 'usr/share/containers/systemd/mios-llm-light.container' }
$script:MIOS_BLADE_PLANES_HA_MARKERS = if ($env:MIOS_BLADE_PLANES_HA_MARKERS) { $env:MIOS_BLADE_PLANES_HA_MARKERS } else { 'pacemaker,corosync' }
$script:MIOS_BLADE_PLANES_HA_OWNER = if ($env:MIOS_BLADE_PLANES_HA_OWNER) { $env:MIOS_BLADE_PLANES_HA_OWNER } else { 'either' }
$script:MIOS_BLADE_PLANES_HA_ROLE = if ($env:MIOS_BLADE_PLANES_HA_ROLE) { $env:MIOS_BLADE_PLANES_HA_ROLE } else { 'Pacemaker/Corosync: places and live-migrates VMs (ADR-0017 D1). A guest may hold a vote (ADR-0016 D11), so fencing one goes via its host' }
$script:MIOS_BLADE_PLANES_HA_WIRED_BY = if ($env:MIOS_BLADE_PLANES_HA_WIRED_BY) { $env:MIOS_BLADE_PLANES_HA_WIRED_BY } else { 'usr/lib/greenboot/check/wanted.d/50-mios-ha-cluster.sh' }
$script:MIOS_BLADE_PLANES_HYPERVISOR_MARKERS = if ($env:MIOS_BLADE_PLANES_HYPERVISOR_MARKERS) { $env:MIOS_BLADE_PLANES_HYPERVISOR_MARKERS } else { 'libvirt,qemu-kvm' }
$script:MIOS_BLADE_PLANES_HYPERVISOR_OWNER = if ($env:MIOS_BLADE_PLANES_HYPERVISOR_OWNER) { $env:MIOS_BLADE_PLANES_HYPERVISOR_OWNER } else { 'mini' }
$script:MIOS_BLADE_PLANES_HYPERVISOR_ROLE = if ($env:MIOS_BLADE_PLANES_HYPERVISOR_ROLE) { $env:MIOS_BLADE_PLANES_HYPERVISOR_ROLE } else { 'runs the VMs and containers every other plane lands in' }
$script:MIOS_BLADE_PLANES_HYPERVISOR_WIRED_BY = if ($env:MIOS_BLADE_PLANES_HYPERVISOR_WIRED_BY) { $env:MIOS_BLADE_PLANES_HYPERVISOR_WIRED_BY } else { 'usr/lib/sysctl.d/99-mios-vmhost.conf' }
$script:MIOS_BLADE_PLANES_MESH_MARKERS = if ($env:MIOS_BLADE_PLANES_MESH_MARKERS) { $env:MIOS_BLADE_PLANES_MESH_MARKERS } else { 'tailscale' }
$script:MIOS_BLADE_PLANES_MESH_OWNER = if ($env:MIOS_BLADE_PLANES_MESH_OWNER) { $env:MIOS_BLADE_PLANES_MESH_OWNER } else { 'mini' }
$script:MIOS_BLADE_PLANES_MESH_ROLE = if ($env:MIOS_BLADE_PLANES_MESH_ROLE) { $env:MIOS_BLADE_PLANES_MESH_ROLE } else { 'joins every node and service into one addressable overlay' }
$script:MIOS_BLADE_PLANES_ORCHESTRATOR_MARKERS = if ($env:MIOS_BLADE_PLANES_ORCHESTRATOR_MARKERS) { $env:MIOS_BLADE_PLANES_ORCHESTRATOR_MARKERS } else { 'k3s' }
$script:MIOS_BLADE_PLANES_ORCHESTRATOR_OWNER = if ($env:MIOS_BLADE_PLANES_ORCHESTRATOR_OWNER) { $env:MIOS_BLADE_PLANES_ORCHESTRATOR_OWNER } else { 'either' }
$script:MIOS_BLADE_PLANES_ORCHESTRATOR_ROLE = if ($env:MIOS_BLADE_PLANES_ORCHESTRATOR_ROLE) { $env:MIOS_BLADE_PLANES_ORCHESTRATOR_ROLE } else { 'k3s: places containers across the fleet (ADR-0017 D1). Peers JOIN one elected server, never stand up their own (ADR-0016 D11)' }
$script:MIOS_BLADE_PLANES_ORCHESTRATOR_WIRED_BY = if ($env:MIOS_BLADE_PLANES_ORCHESTRATOR_WIRED_BY) { $env:MIOS_BLADE_PLANES_ORCHESTRATOR_WIRED_BY } else { 'usr/share/containers/systemd/mios-k3s.container' }
$script:MIOS_BLADE_PLANES_RADIO_MARKERS = if ($env:MIOS_BLADE_PLANES_RADIO_MARKERS) { $env:MIOS_BLADE_PLANES_RADIO_MARKERS } else { 'hostapd,wpa_supplicant' }
$script:MIOS_BLADE_PLANES_RADIO_OWNER = if ($env:MIOS_BLADE_PLANES_RADIO_OWNER) { $env:MIOS_BLADE_PLANES_RADIO_OWNER } else { 'mini' }
$script:MIOS_BLADE_PLANES_RADIO_ROLE = if ($env:MIOS_BLADE_PLANES_RADIO_ROLE) { $env:MIOS_BLADE_PLANES_RADIO_ROLE } else { 'serves WiFi clients as an access point; needs [blade.hardware].min_ap_capable of the min_interfaces' }
$script:MIOS_BLADE_PLANES_ROUTER_MARKERS = if ($env:MIOS_BLADE_PLANES_ROUTER_MARKERS) { $env:MIOS_BLADE_PLANES_ROUTER_MARKERS } else { 'firewalld,dnsmasq' }
$script:MIOS_BLADE_PLANES_ROUTER_OWNER = if ($env:MIOS_BLADE_PLANES_ROUTER_OWNER) { $env:MIOS_BLADE_PLANES_ROUTER_OWNER } else { 'mini' }
$script:MIOS_BLADE_PLANES_ROUTER_ROLE = if ($env:MIOS_BLADE_PLANES_ROUTER_ROLE) { $env:MIOS_BLADE_PLANES_ROUTER_ROLE } else { 'is the uplink: forwards, NATs and filters for the clients it serves, on a different interface from the one serving them' }
$script:MIOS_BLADE_PLANES_ROUTER_WIRED_BY = if ($env:MIOS_BLADE_PLANES_ROUTER_WIRED_BY) { $env:MIOS_BLADE_PLANES_ROUTER_WIRED_BY } else { 'usr/lib/sysctl.d/99-mios-vmhost.conf' }
$script:MIOS_BLADE_PLANES_STORAGE_MARKERS = if ($env:MIOS_BLADE_PLANES_STORAGE_MARKERS) { $env:MIOS_BLADE_PLANES_STORAGE_MARKERS } else { 'ceph-common,cephadm' }
$script:MIOS_BLADE_PLANES_STORAGE_OWNER = if ($env:MIOS_BLADE_PLANES_STORAGE_OWNER) { $env:MIOS_BLADE_PLANES_STORAGE_OWNER } else { 'either' }
$script:MIOS_BLADE_PLANES_STORAGE_ROLE = if ($env:MIOS_BLADE_PLANES_STORAGE_ROLE) { $env:MIOS_BLADE_PLANES_STORAGE_ROLE } else { 'CephFS, shadow-copied across the mesh (local, localhost, remote) so a shed workload lands on its data rather than cold-starting' }
$script:MIOS_BLADE_PLANES_STORAGE_WIRED_BY = if ($env:MIOS_BLADE_PLANES_STORAGE_WIRED_BY) { $env:MIOS_BLADE_PLANES_STORAGE_WIRED_BY } else { 'usr/share/containers/systemd/mios-ceph.container' }
...
$script:MIOS_BLADE_RECONCILE_SESSION = if ($env:MIOS_BLADE_RECONCILE_SESSION) { $env:MIOS_BLADE_RECONCILE_SESSION } else { 'last-writer-wins' }
$script:MIOS_BLADE_REQUIRES_HERMES_WORKER = if ($env:MIOS_BLADE_REQUIRES_HERMES_WORKER) { $env:MIOS_BLADE_REQUIRES_HERMES_WORKER } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_K3S = if ($env:MIOS_BLADE_REQUIRES_K3S) { $env:MIOS_BLADE_REQUIRES_K3S } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_ACCOUNT_SYNC = if ($env:MIOS_BLADE_REQUIRES_MIOS_ACCOUNT_SYNC) { $env:MIOS_BLADE_REQUIRES_MIOS_ACCOUNT_SYNC } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_ADGUARD = if ($env:MIOS_BLADE_REQUIRES_MIOS_ADGUARD) { $env:MIOS_BLADE_REQUIRES_MIOS_ADGUARD } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_AGENTS = if ($env:MIOS_BLADE_REQUIRES_MIOS_AGENTS) { $env:MIOS_BLADE_REQUIRES_MIOS_AGENTS } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_CEPH = if ($env:MIOS_BLADE_REQUIRES_MIOS_CEPH) { $env:MIOS_BLADE_REQUIRES_MIOS_CEPH } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_COCKPIT_LINK = if ($env:MIOS_BLADE_REQUIRES_MIOS_COCKPIT_LINK) { $env:MIOS_BLADE_REQUIRES_MIOS_COCKPIT_LINK } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_CODE_SERVER = if ($env:MIOS_BLADE_REQUIRES_MIOS_CODE_SERVER) { $env:MIOS_BLADE_REQUIRES_MIOS_CODE_SERVER } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_CPU_NODE = if ($env:MIOS_BLADE_REQUIRES_MIOS_CPU_NODE) { $env:MIOS_BLADE_REQUIRES_MIOS_CPU_NODE } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_CRON_DIRECTOR = if ($env:MIOS_BLADE_REQUIRES_MIOS_CRON_DIRECTOR) { $env:MIOS_BLADE_REQUIRES_MIOS_CRON_DIRECTOR } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_DAEMON = if ($env:MIOS_BLADE_REQUIRES_MIOS_DAEMON) { $env:MIOS_BLADE_REQUIRES_MIOS_DAEMON } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_EMBED_BACKFILL = if ($env:MIOS_BLADE_REQUIRES_MIOS_EMBED_BACKFILL) { $env:MIOS_BLADE_REQUIRES_MIOS_EMBED_BACKFILL } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_FINETUNE_SERVE = if ($env:MIOS_BLADE_REQUIRES_MIOS_FINETUNE_SERVE) { $env:MIOS_BLADE_REQUIRES_MIOS_FINETUNE_SERVE } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_FORGE = if ($env:MIOS_BLADE_REQUIRES_MIOS_FORGE) { $env:MIOS_BLADE_REQUIRES_MIOS_FORGE } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_FORGEJO_RUNNER = if ($env:MIOS_BLADE_REQUIRES_MIOS_FORGEJO_RUNNER) { $env:MIOS_BLADE_REQUIRES_MIOS_FORGEJO_RUNNER } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_FORGEJO_RUNNER_FIRSTBOOT = if ($env:MIOS_BLADE_REQUIRES_MIOS_FORGEJO_RUNNER_FIRSTBOOT) { $env:MIOS_BLADE_REQUIRES_MIOS_FORGEJO_RUNNER_FIRSTBOOT } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_FORGE_FIRSTBOOT = if ($env:MIOS_BLADE_REQUIRES_MIOS_FORGE_FIRSTBOOT) { $env:MIOS_BLADE_REQUIRES_MIOS_FORGE_FIRSTBOOT } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_GIT_ROOT_INIT = if ($env:MIOS_BLADE_REQUIRES_MIOS_GIT_ROOT_INIT) { $env:MIOS_BLADE_REQUIRES_MIOS_GIT_ROOT_INIT } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_GUACAMOLE = if ($env:MIOS_BLADE_REQUIRES_MIOS_GUACAMOLE) { $env:MIOS_BLADE_REQUIRES_MIOS_GUACAMOLE } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_GUACD = if ($env:MIOS_BLADE_REQUIRES_MIOS_GUACD) { $env:MIOS_BLADE_REQUIRES_MIOS_GUACD } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_HERMES_BROWSER_WORKER = if ($env:MIOS_BLADE_REQUIRES_MIOS_HERMES_BROWSER_WORKER) { $env:MIOS_BLADE_REQUIRES_MIOS_HERMES_BROWSER_WORKER } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_K3S = if ($env:MIOS_BLADE_REQUIRES_MIOS_K3S) { $env:MIOS_BLADE_REQUIRES_MIOS_K3S } else { 'controller,service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_K3S_MASTER = if ($env:MIOS_BLADE_REQUIRES_MIOS_K3S_MASTER) { $env:MIOS_BLADE_REQUIRES_MIOS_K3S_MASTER } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_LLM_HEAVY = if ($env:MIOS_BLADE_REQUIRES_MIOS_LLM_HEAVY) { $env:MIOS_BLADE_REQUIRES_MIOS_LLM_HEAVY } else { 'gpu-serving,service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_LLM_HEAVY_ALT = if ($env:MIOS_BLADE_REQUIRES_MIOS_LLM_HEAVY_ALT) { $env:MIOS_BLADE_REQUIRES_MIOS_LLM_HEAVY_ALT } else { 'gpu-serving,service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_LLM_LIGHT = if ($env:MIOS_BLADE_REQUIRES_MIOS_LLM_LIGHT) { $env:MIOS_BLADE_REQUIRES_MIOS_LLM_LIGHT } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_LLM_WORKER_ = if ($env:MIOS_BLADE_REQUIRES_MIOS_LLM_WORKER_) { $env:MIOS_BLADE_REQUIRES_MIOS_LLM_WORKER_ } else { 'gpu-serving,service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_MCP = if ($env:MIOS_BLADE_REQUIRES_MIOS_MCP) { $env:MIOS_BLADE_REQUIRES_MIOS_MCP } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_OPENCODE_GATEWAY = if ($env:MIOS_BLADE_REQUIRES_MIOS_OPENCODE_GATEWAY) { $env:MIOS_BLADE_REQUIRES_MIOS_OPENCODE_GATEWAY } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_OPEN_WEBUI = if ($env:MIOS_BLADE_REQUIRES_MIOS_OPEN_WEBUI) { $env:MIOS_BLADE_REQUIRES_MIOS_OPEN_WEBUI } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_OTELCOL = if ($env:MIOS_BLADE_REQUIRES_MIOS_OTELCOL) { $env:MIOS_BLADE_REQUIRES_MIOS_OTELCOL } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_PASSPORT_PROVISION = if ($env:MIOS_BLADE_REQUIRES_MIOS_PASSPORT_PROVISION) { $env:MIOS_BLADE_REQUIRES_MIOS_PASSPORT_PROVISION } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_PGVECTOR = if ($env:MIOS_BLADE_REQUIRES_MIOS_PGVECTOR) { $env:MIOS_BLADE_REQUIRES_MIOS_PGVECTOR } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_PGVECTOR_BACKUP = if ($env:MIOS_BLADE_REQUIRES_MIOS_PGVECTOR_BACKUP) { $env:MIOS_BLADE_REQUIRES_MIOS_PGVECTOR_BACKUP } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_POLICY_ARBITER = if ($env:MIOS_BLADE_REQUIRES_MIOS_POLICY_ARBITER) { $env:MIOS_BLADE_REQUIRES_MIOS_POLICY_ARBITER } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_PXE_HUB = if ($env:MIOS_BLADE_REQUIRES_MIOS_PXE_HUB) { $env:MIOS_BLADE_REQUIRES_MIOS_PXE_HUB } else { 'controller,service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_SEARXNG = if ($env:MIOS_BLADE_REQUIRES_MIOS_SEARXNG) { $env:MIOS_BLADE_REQUIRES_MIOS_SEARXNG } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_SKILLS_MINER = if ($env:MIOS_BLADE_REQUIRES_MIOS_SKILLS_MINER) { $env:MIOS_BLADE_REQUIRES_MIOS_SKILLS_MINER } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_SYS_ENV_REFRESH = if ($env:MIOS_BLADE_REQUIRES_MIOS_SYS_ENV_REFRESH) { $env:MIOS_BLADE_REQUIRES_MIOS_SYS_ENV_REFRESH } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_USERDB_RENDER = if ($env:MIOS_BLADE_REQUIRES_MIOS_USERDB_RENDER) { $env:MIOS_BLADE_REQUIRES_MIOS_USERDB_RENDER } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_WEBTOOLS_CRAWL4AI = if ($env:MIOS_BLADE_REQUIRES_MIOS_WEBTOOLS_CRAWL4AI) { $env:MIOS_BLADE_REQUIRES_MIOS_WEBTOOLS_CRAWL4AI } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_WEBTOOLS_FIRECRAWL_API = if ($env:MIOS_BLADE_REQUIRES_MIOS_WEBTOOLS_FIRECRAWL_API) { $env:MIOS_BLADE_REQUIRES_MIOS_WEBTOOLS_FIRECRAWL_API } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_WEBTOOLS_FIRECRAWL_WORKER = if ($env:MIOS_BLADE_REQUIRES_MIOS_WEBTOOLS_FIRECRAWL_WORKER) { $env:MIOS_BLADE_REQUIRES_MIOS_WEBTOOLS_FIRECRAWL_WORKER } else { 'service-plane' }
$script:MIOS_BLADE_REQUIRES_MIOS_WEBTOOLS_REDIS = if ($env:MIOS_BLADE_REQUIRES_MIOS_WEBTOOLS_REDIS) { $env:MIOS_BLADE_REQUIRES_MIOS_WEBTOOLS_REDIS } else { 'service-plane' }
$script:MIOS_BLADE_ROLE_ALIASES_HA = if ($env:MIOS_BLADE_ROLE_ALIASES_HA) { $env:MIOS_BLADE_ROLE_ALIASES_HA } else { 'ha-node' }
$script:MIOS_BLADE_ROLE_ALIASES_K3S = if ($env:MIOS_BLADE_ROLE_ALIASES_K3S) { $env:MIOS_BLADE_ROLE_ALIASES_K3S } else { 'k3s-master' }
$script:MIOS_BLADE_SEAT_SIDE = if ($env:MIOS_BLADE_SEAT_SIDE) { $env:MIOS_BLADE_SEAT_SIDE } else { 'mios-agent-pipe,hermes-dashboard,mios-hermes-browser,mios-hermes-tail,mios-ttyd-bash,mios-ttyd-powershell' }
$script:MIOS_BLADE_SOFT_OK = if ($env:MIOS_BLADE_SOFT_OK) { $env:MIOS_BLADE_SOFT_OK } else { 'hermes-worker' }
$script:MIOS_BLADE_STORAGE_AT_REST = if ($env:MIOS_BLADE_STORAGE_AT_REST) { $env:MIOS_BLADE_STORAGE_AT_REST } else { 'encrypted' }
$script:MIOS_BLADE_STORAGE_REPLICATION = if ($env:MIOS_BLADE_STORAGE_REPLICATION) { $env:MIOS_BLADE_STORAGE_REPLICATION } else { 'all' }
$script:MIOS_BLADE_TYPE = if ($env:MIOS_BLADE_TYPE) { $env:MIOS_BLADE_TYPE } else { 'hybrid' }
$script:MIOS_BLADE_UPLINK_FAILOVER = if ($env:MIOS_BLADE_UPLINK_FAILOVER) { $env:MIOS_BLADE_UPLINK_FAILOVER } else { 'peer' }
...
$script:MIOS_BROWSER_AI_PROVIDER_URL = if ($env:MIOS_BROWSER_AI_PROVIDER_URL) { $env:MIOS_BROWSER_AI_PROVIDER_URL } else { "http://localhost:$($script:MIOS_PORT_OPEN_WEBUI)" }
...
$script:MIOS_BUILD_BAKE_FIRSTBOOT_TOKENS = if ($env:MIOS_BUILD_BAKE_FIRSTBOOT_TOKENS) { $env:MIOS_BUILD_BAKE_FIRSTBOOT_TOKENS } else { 'vllm,sglang,crawl4ai,firecrawl' }
$script:MIOS_BUILD_BAKE_GROUPS = if ($env:MIOS_BUILD_BAKE_GROUPS) { $env:MIOS_BUILD_BAKE_GROUPS } else { 'sys,cuda,heavy,extra' }
...
$script:MIOS_BUILD_BAKE_GROUP_MEMBERS_HEAVY = if ($env:MIOS_BUILD_BAKE_GROUP_MEMBERS_HEAVY) { $env:MIOS_BUILD_BAKE_GROUP_MEMBERS_HEAVY } else { 'open-webui,ceph' }
...
$script:MIOS_BUILD_RATCHET_MAX_PHASE_SCRIPTS = if ($env:MIOS_BUILD_RATCHET_MAX_PHASE_SCRIPTS) { $env:MIOS_BUILD_RATCHET_MAX_PHASE_SCRIPTS } else { 72 }
...
$script:MIOS_CONSENSUS_ENABLE = if ($env:MIOS_CONSENSUS_ENABLE) { $env:MIOS_CONSENSUS_ENABLE } else { 'false' }
$script:MIOS_CONSENSUS_MIN_LANES = if ($env:MIOS_CONSENSUS_MIN_LANES) { $env:MIOS_CONSENSUS_MIN_LANES } else { 2 }
$script:MIOS_CONSENSUS_RRF_K = if ($env:MIOS_CONSENSUS_RRF_K) { $env:MIOS_CONSENSUS_RRF_K } else { 60 }
$script:MIOS_CONSENSUS_THRESHOLD = if ($env:MIOS_CONSENSUS_THRESHOLD) { $env:MIOS_CONSENSUS_THRESHOLD } else { '0.5' }
$script:MIOS_CONSENSUS_TIMEOUT_S = if ($env:MIOS_CONSENSUS_TIMEOUT_S) { $env:MIOS_CONSENSUS_TIMEOUT_S } else { '20.0' }
$script:MIOS_CONSENSUS_WEIGHT_FLOOR = if ($env:MIOS_CONSENSUS_WEIGHT_FLOOR) { $env:MIOS_CONSENSUS_WEIGHT_FLOOR } else { '0.1' }
$script:MIOS_CONVERGE_GATEWAY_FALLBACK_HTTP = if ($env:MIOS_CONVERGE_GATEWAY_FALLBACK_HTTP) { $env:MIOS_CONVERGE_GATEWAY_FALLBACK_HTTP } else { "http://localhost:$($script:MIOS_PORT_HERMES)/v1" }
...
$script:MIOS_PORT_CHROME_CDP = if ($env:MIOS_PORT_CHROME_CDP) { $env:MIOS_PORT_CHROME_CDP } else { 9222 }
$script:MIOS_CRAWL_CDP_URL = if ($env:MIOS_CRAWL_CDP_URL) { $env:MIOS_CRAWL_CDP_URL } else { "http://127.0.0.1:$($script:MIOS_PORT_CHROME_CDP)" }
...
$script:MIOS_DESKTOP_LAUNCHERS_MIOS_CONFIGURATOR_KEYWORDS = if ($env:MIOS_DESKTOP_LAUNCHERS_MIOS_CONFIGURATOR_KEYWORDS) { $env:MIOS_DESKTOP_LAUNCHERS_MIOS_CONFIGURATOR_KEYWORDS } else { 'mios;configurator;system;settings;packages;flatpak;ai;' }
$script:MIOS_DESKTOP_LAUNCHERS_MIOS_CONFIGURATOR_STARTUP_WM_CLASS = if ($env:MIOS_DESKTOP_LAUNCHERS_MIOS_CONFIGURATOR_STARTUP_WM_CLASS) { $env:MIOS_DESKTOP_LAUNCHERS_MIOS_CONFIGURATOR_STARTUP_WM_CLASS } else { 'org.gnome.Epiphany' }
$script:MIOS_DESKTOP_LAUNCHERS_MIOS_CONFIGURATOR_TITLE = if ($env:MIOS_DESKTOP_LAUNCHERS_MIOS_CONFIGURATOR_TITLE) { $env:MIOS_DESKTOP_LAUNCHERS_MIOS_CONFIGURATOR_TITLE } else { 'MiOS Settings' }
...
$script:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_CEPH_NO_DISPLAY = if ($env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_CEPH_NO_DISPLAY) { $env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_CEPH_NO_DISPLAY } else { 'false' }
$script:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_CEPH_PORT_KEY = if ($env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_CEPH_PORT_KEY) { $env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_CEPH_PORT_KEY } else { 'ceph_dashboard' }
$script:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_CEPH_SCHEME = if ($env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_CEPH_SCHEME) { $env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_CEPH_SCHEME } else { 'https' }
...
$script:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_COCKPIT_PORT_KEY = if ($env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_COCKPIT_PORT_KEY) { $env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_COCKPIT_PORT_KEY } else { 'cockpit' }
$script:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_COCKPIT_SCHEME = if ($env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_COCKPIT_SCHEME) { $env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_COCKPIT_SCHEME } else { 'https' }
...
$script:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_CODE_SERVER_PORT_KEY = if ($env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_CODE_SERVER_PORT_KEY) { $env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_CODE_SERVER_PORT_KEY } else { 'code_server' }
...
$script:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_FORGE_PORT_KEY = if ($env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_FORGE_PORT_KEY) { $env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_FORGE_PORT_KEY } else { 'forge_http' }
...
$script:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_GUACAMOLE_PATH = if ($env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_GUACAMOLE_PATH) { $env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_GUACAMOLE_PATH } else { '/guacamole/' }
$script:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_GUACAMOLE_PORT_KEY = if ($env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_GUACAMOLE_PORT_KEY) { $env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_GUACAMOLE_PORT_KEY } else { 'guacamole_web' }
...
$script:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_HERMES_PATH = if ($env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_HERMES_PATH) { $env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_HERMES_PATH } else { '/v1' }
$script:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_HERMES_PORT_KEY = if ($env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_HERMES_PORT_KEY) { $env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_HERMES_PORT_KEY } else { 'hermes' }
...
$script:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_LLM_LIGHT_PORT_KEY = if ($env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_LLM_LIGHT_PORT_KEY) { $env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_LLM_LIGHT_PORT_KEY } else { 'llm_light' }
...
$script:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_SEARXNG_PORT_KEY = if ($env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_SEARXNG_PORT_KEY) { $env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_SEARXNG_PORT_KEY } else { 'searxng' }
$script:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_SEARXNG_TITLE = if ($env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_SEARXNG_TITLE) { $env:MIOS_DESKTOP_LAUNCHERS_MIOS_SVC_SEARXNG_TITLE } else { 'MiOS Search (SearXNG)' }
...
$script:MIOS_DEVELOPER = if ($env:MIOS_DEVELOPER) { $env:MIOS_DEVELOPER } else { 'MiOS' }
...
$script:MIOS_DOCS_SIGNALS_CODE = if ($env:MIOS_DOCS_SIGNALS_CODE) { $env:MIOS_DOCS_SIGNALS_CODE } else { '^\s*(if|for|while|def|class|function|export|set|return|elif|else|fi|done|esac|end)\b|[;{]\s*$|^\s*[\w.]+\s*=[^=]' }
$script:MIOS_DOCS_SIGNALS_FACT = if ($env:MIOS_DOCS_SIGNALS_FACT) { $env:MIOS_DOCS_SIGNALS_FACT } else { '\b(broken|deprecated|removed|disabled|not supported|only on|requires|since|as of|WS-[A-Z]+|AGY-[0-9]+|ADR-[0-9]+|Law [0-9]+)\b' }
$script:MIOS_DOCS_SIGNALS_NARRATIVE = if ($env:MIOS_DOCS_SIGNALS_NARRATIVE) { $env:MIOS_DOCS_SIGNALS_NARRATIVE } else { '\b(operator|used to|no longer|previously|regression|root cause|incident|reverted|scrapped|rejected|instead of|alternative|rationale|invariant|degrade|ADR-[0-9]+|Law [0-9]+|AGY-[0-9]+|WS-[A-Z]+|[0-9]{4}-[0-9]{2}-[0-9]{2})\b' }
$script:MIOS_DOCS_SIGNALS_WHY = if ($env:MIOS_DOCS_SIGNALS_WHY) { $env:MIOS_DOCS_SIGNALS_WHY } else { '\b(because|so that|otherwise|avoid|prevents?|must not|never|do not|fail-open|fail-closed|deliberately|intentionally|noqa|workaround|upstream bug|race|deadlock)\b' }
...
$script:MIOS_DRIFT_DENYLIST = if ($env:MIOS_DRIFT_DENYLIST) { $env:MIOS_DRIFT_DENYLIST } else { 'mios_ctxpack,mios_embed_backfill,mios_provider_translate,mios_smartroute,mios_worker_tools' }
$script:MIOS_DRIFT_MONITOR_AXES = if ($env:MIOS_DRIFT_MONITOR_AXES) { $env:MIOS_DRIFT_MONITOR_AXES } else { 'verdict,intent' }
$script:MIOS_DRIFT_MONITOR_ENABLE = if ($env:MIOS_DRIFT_MONITOR_ENABLE) { $env:MIOS_DRIFT_MONITOR_ENABLE } else { 'false' }
$script:MIOS_DRIFT_MONITOR_MIN_SAMPLES = if ($env:MIOS_DRIFT_MONITOR_MIN_SAMPLES) { $env:MIOS_DRIFT_MONITOR_MIN_SAMPLES } else { 30 }
$script:MIOS_DRIFT_MONITOR_THRESHOLD = if ($env:MIOS_DRIFT_MONITOR_THRESHOLD) { $env:MIOS_DRIFT_MONITOR_THRESHOLD } else { '0.2' }
$script:MIOS_DRIFT_MONITOR_WINDOW = if ($env:MIOS_DRIFT_MONITOR_WINDOW) { $env:MIOS_DRIFT_MONITOR_WINDOW } else { 200 }
...
$script:MIOS_EDITIONS_MIOS_METAL_GPU_ARBITRATION = if ($env:MIOS_EDITIONS_MIOS_METAL_GPU_ARBITRATION) { $env:MIOS_EDITIONS_MIOS_METAL_GPU_ARBITRATION } else { 'static' }
$script:MIOS_EDITIONS_MIOS_METAL_GPU_ASSIGNMENTS_MIOS_GUEST = if ($env:MIOS_EDITIONS_MIOS_METAL_GPU_ASSIGNMENTS_MIOS_GUEST) { $env:MIOS_EDITIONS_MIOS_METAL_GPU_ASSIGNMENTS_MIOS_GUEST } else { '0000:01:00.0' }
...
$script:MIOS_EDITIONS_MIOS_XBOX_ARM_METAL_GPU_ARBITRATION = if ($env:MIOS_EDITIONS_MIOS_XBOX_ARM_METAL_GPU_ARBITRATION) { $env:MIOS_EDITIONS_MIOS_XBOX_ARM_METAL_GPU_ARBITRATION } else { 'static' }
$script:MIOS_EDITIONS_MIOS_XBOX_ARM_METAL_GPU_ASSIGNMENTS_MIOS_GUEST = if ($env:MIOS_EDITIONS_MIOS_XBOX_ARM_METAL_GPU_ASSIGNMENTS_MIOS_GUEST) { $env:MIOS_EDITIONS_MIOS_XBOX_ARM_METAL_GPU_ASSIGNMENTS_MIOS_GUEST } else { '0000:01:00.0' }
...
$script:MIOS_EDITIONS_MIOS_XBOX_METAL_GPU_ARBITRATION = if ($env:MIOS_EDITIONS_MIOS_XBOX_METAL_GPU_ARBITRATION) { $env:MIOS_EDITIONS_MIOS_XBOX_METAL_GPU_ARBITRATION } else { 'static' }
$script:MIOS_EDITIONS_MIOS_XBOX_METAL_GPU_ASSIGNMENTS_MIOS_GUEST = if ($env:MIOS_EDITIONS_MIOS_XBOX_METAL_GPU_ASSIGNMENTS_MIOS_GUEST) { $env:MIOS_EDITIONS_MIOS_XBOX_METAL_GPU_ASSIGNMENTS_MIOS_GUEST } else { '0000:01:00.0' }
$script:MIOS_ENDPOINT = if ($env:MIOS_ENDPOINT) { $env:MIOS_ENDPOINT } else { "http://localhost:$($script:MIOS_PORT_AGENT_PIPE)/v1" }
...
$script:MIOS_FIREWALL_OPEN_PORTS = if ($env:MIOS_FIREWALL_OPEN_PORTS) { $env:MIOS_FIREWALL_OPEN_PORTS } else { 'forge_http,open_webui,code_server,hermes,searxng,cockpit,hermes_dashboard,llm_light,pgvector,cockpit_link,adguard_ui,ssh,forge_ssh,vllm,sglang,cpu_node' }
...
$script:MIOS_GREENBOOT_CRITICAL_SERVICES = if ($env:MIOS_GREENBOOT_CRITICAL_SERVICES) { $env:MIOS_GREENBOOT_CRITICAL_SERVICES } else { 'agent-pipe,llm-light,pgvector,hermes' }
$script:MIOS_GREENBOOT_PROBE_AGENT_PIPE_KIND = if ($env:MIOS_GREENBOOT_PROBE_AGENT_PIPE_KIND) { $env:MIOS_GREENBOOT_PROBE_AGENT_PIPE_KIND } else { 'http' }
$script:MIOS_GREENBOOT_PROBE_AGENT_PIPE_PATH = if ($env:MIOS_GREENBOOT_PROBE_AGENT_PIPE_PATH) { $env:MIOS_GREENBOOT_PROBE_AGENT_PIPE_PATH } else { '/v1/models' }
$script:MIOS_GREENBOOT_PROBE_HERMES_UNIT = if ($env:MIOS_GREENBOOT_PROBE_HERMES_UNIT) { $env:MIOS_GREENBOOT_PROBE_HERMES_UNIT } else { 'hermes-worker.service' }
...
$script:MIOS_MEMORY_CONSOLIDATE = if ($env:MIOS_MEMORY_CONSOLIDATE) { $env:MIOS_MEMORY_CONSOLIDATE } else { 'true' }
$script:MIOS_MEMORY_CONSOLIDATE_INTERVAL_S = if ($env:MIOS_MEMORY_CONSOLIDATE_INTERVAL_S) { $env:MIOS_MEMORY_CONSOLIDATE_INTERVAL_S } else { 3600 }
$script:MIOS_MEMORY_CONSOLIDATE_MAX_GROUPS = if ($env:MIOS_MEMORY_CONSOLIDATE_MAX_GROUPS) { $env:MIOS_MEMORY_CONSOLIDATE_MAX_GROUPS } else { 200 }
...
$script:MIOS_METAL_BIND_DGPU_VFIO = if ($env:MIOS_METAL_BIND_DGPU_VFIO) { $env:MIOS_METAL_BIND_DGPU_VFIO } else { 'true' }
$script:MIOS_METAL_DGPUMODE = if ($env:MIOS_METAL_DGPUMODE) { $env:MIOS_METAL_DGPUMODE } else { 'vfio-pci' }
$script:MIOS_METAL_ENABLED = if ($env:MIOS_METAL_ENABLED) { $env:MIOS_METAL_ENABLED } else { 'false' }
$script:MIOS_METAL_GPU_ARBITRATION = if ($env:MIOS_METAL_GPU_ARBITRATION) { $env:MIOS_METAL_GPU_ARBITRATION } else { 'static' }
$script:MIOS_METAL_GPU_ASSIGNMENTS_MIOS_GUEST = if ($env:MIOS_METAL_GPU_ASSIGNMENTS_MIOS_GUEST) { $env:MIOS_METAL_GPU_ASSIGNMENTS_MIOS_GUEST } else { '0000:01:00.0' }
$script:MIOS_METAL_GUEST_CPU_PERCENT = if ($env:MIOS_METAL_GUEST_CPU_PERCENT) { $env:MIOS_METAL_GUEST_CPU_PERCENT } else { 85 }
$script:MIOS_METAL_GUEST_RAM_PERCENT = if ($env:MIOS_METAL_GUEST_RAM_PERCENT) { $env:MIOS_METAL_GUEST_RAM_PERCENT } else { 85 }
$script:MIOS_METAL_MESH_ENABLED = if ($env:MIOS_METAL_MESH_ENABLED) { $env:MIOS_METAL_MESH_ENABLED } else { 'false' }
$script:MIOS_METAL_MESH_HEADSCALE_DOMAIN = if ($env:MIOS_METAL_MESH_HEADSCALE_DOMAIN) { $env:MIOS_METAL_MESH_HEADSCALE_DOMAIN } else { 'mesh.mios.local' }
$script:MIOS_METAL_MESH_SWTPM_VTPM = if ($env:MIOS_METAL_MESH_SWTPM_VTPM) { $env:MIOS_METAL_MESH_SWTPM_VTPM } else { 'true' }
$script:MIOS_METAL_MESH_VNET_CIDR = if ($env:MIOS_METAL_MESH_VNET_CIDR) { $env:MIOS_METAL_MESH_VNET_CIDR } else { '100.64.0.0/10' }
...
$script:MIOS_MIGRATION_USE_COMPILED_AINODE = if ($env:MIOS_MIGRATION_USE_COMPILED_AINODE) { $env:MIOS_MIGRATION_USE_COMPILED_AINODE } else { 'true' }
$script:MIOS_MIGRATION_USE_COMPILED_OSCONTROL = if ($env:MIOS_MIGRATION_USE_COMPILED_OSCONTROL) { $env:MIOS_MIGRATION_USE_COMPILED_OSCONTROL } else { 'true' }
...
$script:MIOS_NAME = if ($env:MIOS_NAME) { $env:MIOS_NAME } else { 'MiOS AI' }
...
$script:MIOS_NODES_LOCAL_CPU_API = if ($env:MIOS_NODES_LOCAL_CPU_API) { $env:MIOS_NODES_LOCAL_CPU_API } else { 'llamacpp' }
$script:MIOS_PORT_CPU_NODE = if ($env:MIOS_PORT_CPU_NODE) { $env:MIOS_PORT_CPU_NODE } else { 8510 }
$script:MIOS_NODES_LOCAL_CPU_ENDPOINT = if ($env:MIOS_NODES_LOCAL_CPU_ENDPOINT) { $env:MIOS_NODES_LOCAL_CPU_ENDPOINT } else { "http://localhost:$($script:MIOS_PORT_CPU_NODE)/v1" }
...
$script:MIOS_NODES_LOCAL_CPU_LANE = if ($env:MIOS_NODES_LOCAL_CPU_LANE) { $env:MIOS_NODES_LOCAL_CPU_LANE } else { 'cpu' }
$script:MIOS_NODES_LOCAL_CPU_MODEL = if ($env:MIOS_NODES_LOCAL_CPU_MODEL) { $env:MIOS_NODES_LOCAL_CPU_MODEL } else { 'mios-agent-cpu' }
...
$script:MIOS_NODES_LOCAL_LLAMASWAP_API = if ($env:MIOS_NODES_LOCAL_LLAMASWAP_API) { $env:MIOS_NODES_LOCAL_LLAMASWAP_API } else { 'llamacpp' }
$script:MIOS_NODES_LOCAL_LLAMASWAP_ENDPOINT = if ($env:MIOS_NODES_LOCAL_LLAMASWAP_ENDPOINT) { $env:MIOS_NODES_LOCAL_LLAMASWAP_ENDPOINT } else { "http://localhost:$($script:MIOS_PORT_LLM_LIGHT)/v1" }
...
$script:MIOS_NODES_LOCAL_LLAMASWAP_LANE = if ($env:MIOS_NODES_LOCAL_LLAMASWAP_LANE) { $env:MIOS_NODES_LOCAL_LLAMASWAP_LANE } else { 'cpu' }
$script:MIOS_NODES_LOCAL_LLAMASWAP_MODEL = if ($env:MIOS_NODES_LOCAL_LLAMASWAP_MODEL) { $env:MIOS_NODES_LOCAL_LLAMASWAP_MODEL } else { 'mios-agent-cpu' }
...
$script:MIOS_NON_ADDRESSABLE_URL = if ($env:MIOS_NON_ADDRESSABLE_URL) { $env:MIOS_NON_ADDRESSABLE_URL } else { 'adguard_dns,adguard_ui,agent_pipe,arbiter,ceph_dashboard,crawl4ai,hermes,llm_light,pgvector,chrome_cdp,chrome_cdp_worker,cockpit_link,cpu_node,daemon_agent,firecrawl,forge_ssh,guacamole_web,guacd,hermes_dashboard,k3s_api,mcp,model_router,opencode_gateway,oscontrol,otelcol_otlp,prefilter,pxe_hub_api,rdp,redis,sglang,ssh,ttyd_bash,ttyd_powershell,vllm' }
...
$script:MIOS_OBSERVABILITY_OTEL_ENDPOINT = if ($env:MIOS_OBSERVABILITY_OTEL_ENDPOINT) { $env:MIOS_OBSERVABILITY_OTEL_ENDPOINT } else { "http://localhost:$($script:MIOS_PORT_OTELCOL_OTLP)" }
...
$script:MIOS_OTELCOL_IMAGE = if ($env:MIOS_OTELCOL_IMAGE) { $env:MIOS_OTELCOL_IMAGE } else { 'docker.io/jaegertracing/all-in-one:1.76.0' }
...
$script:MIOS_OTELCOL_UI_URL = if ($env:MIOS_OTELCOL_UI_URL) { $env:MIOS_OTELCOL_UI_URL } else { "http://localhost:$($script:MIOS_PORT_OTELCOL_UI)/" }
$script:MIOS_OTELCOL_VERSION = if ($env:MIOS_OTELCOL_VERSION) { $env:MIOS_OTELCOL_VERSION } else { 'docker.io/jaegertracing/all-in-one:1.76.0' }
...
$script:MIOS_PGVECTOR_IMAGE = if ($env:MIOS_PGVECTOR_IMAGE) { $env:MIOS_PGVECTOR_IMAGE } else { 'docker.io/pgvector/pgvector:pg18' }
...
$script:MIOS_PGVECTOR_RESTORE_SQL = if ($env:MIOS_PGVECTOR_RESTORE_SQL) { $env:MIOS_PGVECTOR_RESTORE_SQL } else { '/var/lib/mios/pgvector-restore.sql' }
...
$script:MIOS_PGVECTOR_VERSION = if ($env:MIOS_PGVECTOR_VERSION) { $env:MIOS_PGVECTOR_VERSION } else { 'docker.io/pgvector/pgvector:pg18' }
$script:MIOS_PG_BACKUP_DIR = if ($env:MIOS_PG_BACKUP_DIR) { $env:MIOS_PG_BACKUP_DIR } else { '/var/lib/mios/backups' }
$script:MIOS_PG_BACKUP_ENABLE = if ($env:MIOS_PG_BACKUP_ENABLE) { $env:MIOS_PG_BACKUP_ENABLE } else { 'true' }
$script:MIOS_PG_BACKUP_KEEP = if ($env:MIOS_PG_BACKUP_KEEP) { $env:MIOS_PG_BACKUP_KEEP } else { 7 }
...
$script:MIOS_PG_RESTORE_SQL = if ($env:MIOS_PG_RESTORE_SQL) { $env:MIOS_PG_RESTORE_SQL } else { '/var/lib/mios/pgvector-restore.sql' }
...
$script:MIOS_PORTS_CHROME_CDP = if ($env:MIOS_PORTS_CHROME_CDP) { $env:MIOS_PORTS_CHROME_CDP } else { 9222 }
$script:MIOS_PORTS_CHROME_CDP_WORKER = if ($env:MIOS_PORTS_CHROME_CDP_WORKER) { $env:MIOS_PORTS_CHROME_CDP_WORKER } else { 9223 }
...
$script:MIOS_PORTS_UNBOUND = if ($env:MIOS_PORTS_UNBOUND) { $env:MIOS_PORTS_UNBOUND } else { 'chrome_cdp_worker' }
...
$script:MIOS_PORT_CHROME_CDP_WORKER = if ($env:MIOS_PORT_CHROME_CDP_WORKER) { $env:MIOS_PORT_CHROME_CDP_WORKER } else { 9223 }
...
$script:MIOS_PORT_UNBOUND = if ($env:MIOS_PORT_UNBOUND) { $env:MIOS_PORT_UNBOUND } else { 'chrome_cdp_worker' }
...
$script:MIOS_POWERSHELL_ENUMERATION_LIMIT = if ($env:MIOS_POWERSHELL_ENUMERATION_LIMIT) { $env:MIOS_POWERSHELL_ENUMERATION_LIMIT } else { 16 }
...
$script:MIOS_POWERSHELL_FLATTEN = if ($env:MIOS_POWERSHELL_FLATTEN) { $env:MIOS_POWERSHELL_FLATTEN } else { 'true' }
$script:MIOS_POWERSHELL_FLATTEN_WIDTH = if ($env:MIOS_POWERSHELL_FLATTEN_WIDTH) { $env:MIOS_POWERSHELL_FLATTEN_WIDTH } else { 200 }
$script:MIOS_POWERSHELL_MAX_OUTPUT_BYTES = if ($env:MIOS_POWERSHELL_MAX_OUTPUT_BYTES) { $env:MIOS_POWERSHELL_MAX_OUTPUT_BYTES } else { 262144 }
$script:MIOS_POWERSHELL_MAX_SCRIPT_BYTES = if ($env:MIOS_POWERSHELL_MAX_SCRIPT_BYTES) { $env:MIOS_POWERSHELL_MAX_SCRIPT_BYTES } else { 65536 }
$script:MIOS_POWERSHELL_PLAIN_TEXT = if ($env:MIOS_POWERSHELL_PLAIN_TEXT) { $env:MIOS_POWERSHELL_PLAIN_TEXT } else { 'true' }
$script:MIOS_POWERSHELL_STAGE_DIR = if ($env:MIOS_POWERSHELL_STAGE_DIR) { $env:MIOS_POWERSHELL_STAGE_DIR } else { '/mnt/c/Users/Public/Documents/mios-ps' }
$script:MIOS_POWERSHELL_TRIM_TRAILING = if ($env:MIOS_POWERSHELL_TRIM_TRAILING) { $env:MIOS_POWERSHELL_TRIM_TRAILING } else { 'true' }
...
$script:MIOS_REFACTOR_MAX_LINES = if ($env:MIOS_REFACTOR_MAX_LINES) { $env:MIOS_REFACTOR_MAX_LINES } else { 800 }
$script:MIOS_REFACTOR_OVERSIZE = if ($env:MIOS_REFACTOR_OVERSIZE) { $env:MIOS_REFACTOR_OVERSIZE } else { '{''path'': ''mios_pipe/federation/a2a.py'', ''lines'': 1545},{''path'': ''mios_pipe/federation/http_caps.py'', ''lines'': 820},{''path'': ''mios_pipe/memory/knowledge.py'', ''lines'': 961},{''path'': ''mios_pipe/routing/agent_call.py'', ''lines'': 1153},{''path'': ''mios_pipe/routing/chat.py'', ''lines'': 1786},{''path'': ''mios_pipe/routing/dag_exec.py'', ''lines'': 1297},{''path'': ''mios_pipe/routing/native_loop.py'', ''lines'': 1190},{''path'': ''mios_pipe/routing/portal.py'', ''lines'': 1675},{''path'': ''mios_pipe/routing/refine.py'', ''lines'': 1120},{''path'': ''mios_pipe/routing/swarm.py'', ''lines'': 1062},{''path'': ''mios_pipe/routing/web_research.py'', ''lines'': 987},{''path'': ''mios_dispatch.py'', ''lines'': 943},{''path'': ''server.py'', ''lines'': 4975}' }
...
$script:MIOS_ROLE = if ($env:MIOS_ROLE) { $env:MIOS_ROLE } else { 'the ONE name you go by on EVERY surface (the `@`/`mios` CLI, OWUI, Discord, the desktop app, the API)' }
...
$script:MIOS_RUN_TEMPLATE_REPLAY_CANDIDATES = if ($env:MIOS_RUN_TEMPLATE_REPLAY_CANDIDATES) { $env:MIOS_RUN_TEMPLATE_REPLAY_CANDIDATES } else { 50 }
$script:MIOS_RUN_TEMPLATE_REPLAY_ENABLE = if ($env:MIOS_RUN_TEMPLATE_REPLAY_ENABLE) { $env:MIOS_RUN_TEMPLATE_REPLAY_ENABLE } else { 'false' }
$script:MIOS_RUN_TEMPLATE_REPLAY_THRESHOLD = if ($env:MIOS_RUN_TEMPLATE_REPLAY_THRESHOLD) { $env:MIOS_RUN_TEMPLATE_REPLAY_THRESHOLD } else { '0.85' }
...
$script:MIOS_SEARCH_ENDPOINT = if ($env:MIOS_SEARCH_ENDPOINT) { $env:MIOS_SEARCH_ENDPOINT } else { "http://localhost:$($script:MIOS_PORT_SEARXNG)/" }
...
$script:MIOS_SECURITY_ALLOWLIST_HOSTS = if ($env:MIOS_SECURITY_ALLOWLIST_HOSTS) { $env:MIOS_SECURITY_ALLOWLIST_HOSTS } else { 'localhost,127.0.0.1,::1,host.containers.internal,mios-llm-light,mios-open-webui,mios-hermes,mios-pgvector,mios-forge,mios-searxng,mios-crawl4ai,mios-code-server' }
$script:MIOS_SECURITY_PROBE_VERIFY_TLS = if ($env:MIOS_SECURITY_PROBE_VERIFY_TLS) { $env:MIOS_SECURITY_PROBE_VERIFY_TLS } else { 'true' }
$script:MIOS_SECURITY_PROVENANCE_TAINT = if ($env:MIOS_SECURITY_PROVENANCE_TAINT) { $env:MIOS_SECURITY_PROVENANCE_TAINT } else { 'false' }
...
$script:MIOS_SERVICES_WEBTOOLS_CDP_URL = if ($env:MIOS_SERVICES_WEBTOOLS_CDP_URL) { $env:MIOS_SERVICES_WEBTOOLS_CDP_URL } else { "http://127.0.0.1:$($script:MIOS_PORT_CHROME_CDP)" }
...
$script:MIOS_SHELL_SESSION_ENABLE = if ($env:MIOS_SHELL_SESSION_ENABLE) { $env:MIOS_SHELL_SESSION_ENABLE } else { 'false' }
$script:MIOS_SHELL_SESSION_HISTORY_LIMIT = if ($env:MIOS_SHELL_SESSION_HISTORY_LIMIT) { $env:MIOS_SHELL_SESSION_HISTORY_LIMIT } else { 50000 }
$script:MIOS_SHELL_SESSION_IDLE_S = if ($env:MIOS_SHELL_SESSION_IDLE_S) { $env:MIOS_SHELL_SESSION_IDLE_S } else { 1800 }
$script:MIOS_SHELL_SESSION_MAX_OUTPUT_CHARS = if ($env:MIOS_SHELL_SESSION_MAX_OUTPUT_CHARS) { $env:MIOS_SHELL_SESSION_MAX_OUTPUT_CHARS } else { 24000 }
$script:MIOS_SHELL_SESSION_MAX_OUTPUT_LINES = if ($env:MIOS_SHELL_SESSION_MAX_OUTPUT_LINES) { $env:MIOS_SHELL_SESSION_MAX_OUTPUT_LINES } else { 400 }
$script:MIOS_SHELL_SESSION_MAX_SESSIONS = if ($env:MIOS_SHELL_SESSION_MAX_SESSIONS) { $env:MIOS_SHELL_SESSION_MAX_SESSIONS } else { 8 }
$script:MIOS_SHELL_SESSION_SHELL = if ($env:MIOS_SHELL_SESSION_SHELL) { $env:MIOS_SHELL_SESSION_SHELL } else { '/bin/bash' }
$script:MIOS_SHELL_SESSION_SOCKET_NAME = if ($env:MIOS_SHELL_SESSION_SOCKET_NAME) { $env:MIOS_SHELL_SESSION_SOCKET_NAME } else { 'mios' }
$script:MIOS_SHELL_SESSION_STATE_DIR = if ($env:MIOS_SHELL_SESSION_STATE_DIR) { $env:MIOS_SHELL_SESSION_STATE_DIR } else { '/var/lib/mios/shell-sessions' }
$script:MIOS_SHELL_SESSION_TIMEOUT_S = if ($env:MIOS_SHELL_SESSION_TIMEOUT_S) { $env:MIOS_SHELL_SESSION_TIMEOUT_S } else { 120 }
...
$script:MIOS_SSOT_CONSUMERS_DOC = if ($env:MIOS_SSOT_CONSUMERS_DOC) { $env:MIOS_SSOT_CONSUMERS_DOC } else { 'Shipped Python reads config as _toml_section("<table>").get("<key>"). When <table>.<key> does not exist the consumer silently takes its compiled default -- the SSOT and the code disagree with nobody told, and every test that stubs the value still passes. Nine security controls sat unreachable this way under an unclosed [security.nohc_allowlist] header (T-325). MISPLACED means the key name is declared elsewhere in the SSOT, so one side has the wrong path; UNDECLARED means it exists nowhere, so it is an optional escape hatch or a dead read. Draining an entry: decide which side is right, move the key or fix the consumer, then lower max_unresolved. Gate: check_ssot_consumer_keys.' }
$script:MIOS_SSOT_CONSUMERS_MAX_UNRESOLVED = if ($env:MIOS_SSOT_CONSUMERS_MAX_UNRESOLVED) { $env:MIOS_SSOT_CONSUMERS_MAX_UNRESOLVED } else { 9 }
$script:MIOS_SSOT_CONSUMERS_UNRESOLVED = if ($env:MIOS_SSOT_CONSUMERS_UNRESOLVED) { $env:MIOS_SSOT_CONSUMERS_UNRESOLVED } else { 'a2a.security,agent_passport.principal_mode,ai.micro_endpoint,ai.micro_model,ai.permission_tiers,computer_use.hidpi_scale_factor,pgvector.memguard_judge_mode,pgvector.memory_guard_mode,pgvector.memory_provider' }
...
$script:MIOS_UNBOUND_PORT = if ($env:MIOS_UNBOUND_PORT) { $env:MIOS_UNBOUND_PORT } else { 'chrome_cdp_worker' }
...
$script:MIOS_UNITS_HERMES_WORKER_FIRSTBOOT_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNITS_HERMES_WORKER_FIRSTBOOT_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNITS_HERMES_WORKER_FIRSTBOOT_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNITS_HERMES_WORKER_FIRSTBOOT_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNITS_HERMES_WORKER_FIRSTBOOT_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNITS_HERMES_WORKER_FIRSTBOOT_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/hermes-worker-firstboot' }
$script:MIOS_UNITS_HERMES_WORKER_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNITS_HERMES_WORKER_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNITS_HERMES_WORKER_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
...
$script:MIOS_UNITS_HERMES_WORKER_PATH_INSTALL_WANTEDBY = if ($env:MIOS_UNITS_HERMES_WORKER_PATH_INSTALL_WANTEDBY) { $env:MIOS_UNITS_HERMES_WORKER_PATH_INSTALL_WANTEDBY } else { 'multi-user.target' }
...
$script:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_GROUP = if ($env:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_GROUP) { $env:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_GROUP } else { 'mios-ai' }
$script:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_LIMITNOFILE = if ($env:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_LIMITNOFILE) { $env:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_LIMITNOFILE } else { 1048576 }
$script:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_NONEWPRIVILEGES = if ($env:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_NONEWPRIVILEGES) { $env:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_NONEWPRIVILEGES } else { 'false' }
$script:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_PRIVATETMP = if ($env:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_PRIVATETMP) { $env:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_PRIVATETMP } else { 'yes' }
$script:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_PROTECTHOME = if ($env:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_PROTECTHOME) { $env:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_PROTECTHOME } else { 'false' }
$script:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_PROTECTSYSTEM = if ($env:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_PROTECTSYSTEM) { $env:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_PROTECTSYSTEM } else { 'strict' }
$script:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_READWRITEPATHS = if ($env:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_READWRITEPATHS) { $env:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_READWRITEPATHS } else { '/etc/mios /var/lib/mios /run/mios-launcher' }
$script:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_RESTART) { $env:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_RESTART } else { 'on-failure' }
$script:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_RESTARTSEC } else { '10s' }
$script:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_TIMEOUTSTARTSEC = if ($env:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_TIMEOUTSTARTSEC) { $env:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_TIMEOUTSTARTSEC } else { '300s' }
$script:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_TIMEOUTSTOPSEC = if ($env:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_TIMEOUTSTOPSEC) { $env:MIOS_UNITS_HERMES_WORKER_SERVICE_SERVICE_TIMEOUTSTOPSEC } else { '240s' }
...
$script:MIOS_UNITS_HERMES_WORKER_SERVICE_UNIT_COMMENT3 = if ($env:MIOS_UNITS_HERMES_WORKER_SERVICE_UNIT_COMMENT3) { $env:MIOS_UNITS_HERMES_WORKER_SERVICE_UNIT_COMMENT3 } else { '# Skip cleanly if the direct install didn''t land.' }
$script:MIOS_UNITS_HERMES_WORKER_SERVICE_UNIT_CONDITIONPATHEXISTS = if ($env:MIOS_UNITS_HERMES_WORKER_SERVICE_UNIT_CONDITIONPATHEXISTS) { $env:MIOS_UNITS_HERMES_WORKER_SERVICE_UNIT_CONDITIONPATHEXISTS } else { '/usr/lib/mios/agents/.venv/bin/hermes' }
$script:MIOS_UNITS_HERMES_WORKER_SERVICE_UNIT_DESCRIPTION = if ($env:MIOS_UNITS_HERMES_WORKER_SERVICE_UNIT_DESCRIPTION) { $env:MIOS_UNITS_HERMES_WORKER_SERVICE_UNIT_DESCRIPTION } else { '''MiOS'' Hermes-Worker (non-thin agent, native tool loop, :8643)' }
$script:MIOS_UNITS_HERMES_WORKER_SERVICE_UNIT_DOCUMENTATION = if ($env:MIOS_UNITS_HERMES_WORKER_SERVICE_UNIT_DOCUMENTATION) { $env:MIOS_UNITS_HERMES_WORKER_SERVICE_UNIT_DOCUMENTATION } else { 'https://github.com/NousResearch/hermes-agent' }
$script:MIOS_UNITS_HERMES_WORKER_SERVICE_UNIT_WANTS = if ($env:MIOS_UNITS_HERMES_WORKER_SERVICE_UNIT_WANTS) { $env:MIOS_UNITS_HERMES_WORKER_SERVICE_UNIT_WANTS } else { 'network-online.target mios-llm-heavy.service mios-llm-light.service hermes-worker-firstboot.service' }
...
$script:MIOS_UNITS_MIOS_ACCOUNT_SYNC_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNITS_MIOS_ACCOUNT_SYNC_SERVICE_SERVICE_RESTART) { $env:MIOS_UNITS_MIOS_ACCOUNT_SYNC_SERVICE_SERVICE_RESTART } else { 'always' }
...
$script:MIOS_UNITS_MIOS_ACCOUNT_SYNC_SERVICE_UNIT_STARTLIMITBURST = if ($env:MIOS_UNITS_MIOS_ACCOUNT_SYNC_SERVICE_UNIT_STARTLIMITBURST) { $env:MIOS_UNITS_MIOS_ACCOUNT_SYNC_SERVICE_UNIT_STARTLIMITBURST } else { 5 }
$script:MIOS_UNITS_MIOS_ACCOUNT_SYNC_SERVICE_UNIT_STARTLIMITINTERVALSEC = if ($env:MIOS_UNITS_MIOS_ACCOUNT_SYNC_SERVICE_UNIT_STARTLIMITINTERVALSEC) { $env:MIOS_UNITS_MIOS_ACCOUNT_SYNC_SERVICE_UNIT_STARTLIMITINTERVALSEC } else { 300 }
...
$script:MIOS_UNITS_MIOS_ADGUARD_FIRSTBOOT_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNITS_MIOS_ADGUARD_FIRSTBOOT_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNITS_MIOS_ADGUARD_FIRSTBOOT_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/mios-adguard-firstboot' }
$script:MIOS_UNITS_MIOS_ADGUARD_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNITS_MIOS_ADGUARD_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNITS_MIOS_ADGUARD_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
...
$script:MIOS_UNITS_MIOS_AGENTS_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNITS_MIOS_AGENTS_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNITS_MIOS_AGENTS_SERVICE_SERVICE_EXECSTART } else { "/usr/sbin/podman run --replace --name mios-agents --network=host --env PASSWORD=$($script:MIOS_DEFAULT_PASSWORD) --env MIOS_A2O_ENGINE=agy --env MIOS_A2O_WORK=/mnt/mios-root --env MIOS_A2O_ORCH_ENGINE=$($script:MIOS_A2O_ORCH_ENGINE) --env MIOS_A2O_ORCH_MODEL=$($script:MIOS_A2O_ORCH_MODEL) --env MIOS_A2O_ORCH_EFFORT=$($script:MIOS_A2O_ORCH_EFFORT) --env MIOS_A2O_LANE_A_ENGINE=$($script:MIOS_A2O_LANE_A_ENGINE) --env MIOS_A2O_LANE_A_MODEL=$($script:MIOS_A2O_LANE_A_MODEL) --env MIOS_A2O_LANE_A_EFFORT=$($script:MIOS_A2O_LANE_A_EFFORT) --env MIOS_A2O_LANE_B_ENGINE=$($script:MIOS_A2O_LANE_B_ENGINE) --env MIOS_A2O_LANE_B_MODEL=$($script:MIOS_A2O_LANE_B_MODEL) --env MIOS_A2O_LANE_B_EFFORT=$($script:MIOS_A2O_LANE_B_EFFORT) --env MIOS_A2O_CLAUDE_EFFORT_FLAG=$($script:MIOS_A2O_CLAUDE_EFFORT_FLAG) --env MIOS_A2O_AGY_EFFORT_FLAG=${MIOS_A2O_AGY_EFFORT_FLAG} --env MIOS_A2O_GEMINI_EFFORT_FLAG=${MIOS_A2O_GEMINI_EFFORT_FLAG} --volume /:/mnt/mios-root:rw,rslave --volume /var/lib/mios/agents:/home/coder:rw localhost/mios-agents:latest --bind-addr 0.0.0.0:8800 /mnt/mios-root" }
$script:MIOS_UNITS_MIOS_AGENTS_SERVICE_SERVICE_EXECSTARTPRE = if ($env:MIOS_UNITS_MIOS_AGENTS_SERVICE_SERVICE_EXECSTARTPRE) { $env:MIOS_UNITS_MIOS_AGENTS_SERVICE_SERVICE_EXECSTARTPRE } else { '/usr/libexec/mios/mios-agents-firstboot.sh' }
$script:MIOS_UNITS_MIOS_AGENTS_SERVICE_SERVICE_EXECSTOP = if ($env:MIOS_UNITS_MIOS_AGENTS_SERVICE_SERVICE_EXECSTOP) { $env:MIOS_UNITS_MIOS_AGENTS_SERVICE_SERVICE_EXECSTOP } else { '/usr/sbin/podman stop -t 10 mios-agents' }
$script:MIOS_UNITS_MIOS_AGENTS_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNITS_MIOS_AGENTS_SERVICE_SERVICE_RESTART) { $env:MIOS_UNITS_MIOS_AGENTS_SERVICE_SERVICE_RESTART } else { 'on-failure' }
$script:MIOS_UNITS_MIOS_AGENTS_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNITS_MIOS_AGENTS_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNITS_MIOS_AGENTS_SERVICE_SERVICE_RESTARTSEC } else { '10s' }
...
$script:MIOS_UNITS_MIOS_AGENTS_SERVICE_UNIT_WANTS = if ($env:MIOS_UNITS_MIOS_AGENTS_SERVICE_UNIT_WANTS) { $env:MIOS_UNITS_MIOS_AGENTS_SERVICE_UNIT_WANTS } else { 'network-online.target' }
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_ENVIRONMENT = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_ENVIRONMENT) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_ENVIRONMENT } else { 'MIOS_PORT_AGENT_PIPE=${MIOS_PORT_AGENT_PIPE:-8700},MIOS_AGENT_PIPE_BACKEND_LIGHT=1,MIOS_AI_MODEL=granite4.1:8b,MIOS_NO_TOOL_CHOICE_HINTS=11436,MIOS_KV_PAGING_HINTS=11436,11450,MIOS_LLM_API_HINTS=,MIOS_REFINE_TIMEOUT_S=45,MIOS_POLISH_TIMEOUT_S=45,MIOS_REFINE_MAX_TOKENS=1200,MIOS_AGENT_MEMORY_RECALL=1,MIOS_PASSPORT_AGENT_NAME=agent-pipe' }
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_ENVIRONMENTFILE = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_ENVIRONMENTFILE) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_ENVIRONMENTFILE } else { '-/etc/mios/agent-pipe.env,-/etc/mios/install.env' }
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_EXECSTART } else { '/usr/lib/mios/agents/.venv/bin/python3 -u /usr/lib/mios/agent-pipe/server.py' }
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_GROUP = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_GROUP) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_GROUP } else { 'mios-ai' }
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_NONEWPRIVILEGES = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_NONEWPRIVILEGES) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_NONEWPRIVILEGES } else { 'yes' }
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_PRIVATETMP = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_PRIVATETMP) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_PRIVATETMP } else { 'yes' }
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_PROTECTHOME = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_PROTECTHOME) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_PROTECTHOME } else { 'yes' }
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_PROTECTSYSTEM = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_PROTECTSYSTEM) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_PROTECTSYSTEM } else { 'strict' }
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_READONLYPATHS = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_READONLYPATHS) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_READONLYPATHS } else { '/var/lib/mios/agent-passports' }
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_READWRITEPATHS = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_READWRITEPATHS) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_READWRITEPATHS } else { '/var/lib/mios/agent-pipe /var/lib/mios/skills /var/log /var/lib/mios/agent-env /var/lib/mios/ai' }
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_RESTART) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_RESTART } else { 'on-failure' }
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_RESTARTSEC } else { '5s' }
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_SUPPLEMENTARYGROUPS = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_SUPPLEMENTARYGROUPS) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_SUPPLEMENTARYGROUPS } else { 'mios-hermes' }
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_TIMEOUTSTARTSEC = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_TIMEOUTSTARTSEC) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_TIMEOUTSTARTSEC } else { '60s' }
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_TYPE = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_TYPE) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_SERVICE_TYPE } else { 'simple' }
...
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_COMMENT3 = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_COMMENT3) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_COMMENT3 } else { '# R6: the pipe touches pgvector on every route, so upgrade the DB edge from After=
# (ordering only) to a hard Requires= -- a missing DB should hold/fail the unit.' }
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_COMMENT4 = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_COMMENT4) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_COMMENT4 } else { '# R6: bound the existing Restart=on-failure so a DB-down window (many quick
# restarts) can''t trip the default 5/10s start limit and land permanently failed.' }
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_COMMENT5 = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_COMMENT5) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_COMMENT5 } else { '# The venv python is baked into the FINAL image, but on the podman-MiOS-DEV substrate it is
# built at first boot by mios-ai-firstboot. Skip (rather than 203/EXEC-fail) until it exists.' }
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_CONDITIONPATHEXISTS = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_CONDITIONPATHEXISTS) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_CONDITIONPATHEXISTS } else { '/usr/lib/mios/agents/.venv/bin/python3' }
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_DESCRIPTION = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_DESCRIPTION) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_DESCRIPTION } else { '''MiOS'' Agent Pipe (router + refine + critic FastAPI; fronts hermes for every gateway)' }
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_DOCUMENTATION = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_DOCUMENTATION) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_DOCUMENTATION } else { 'file:///usr/lib/mios/agent-pipe/server.py' }
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_REQUIRES = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_REQUIRES) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_REQUIRES } else { 'mios-pgvector.service' }
$script:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_STARTLIMITBURST = if ($env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_STARTLIMITBURST) { $env:MIOS_UNITS_MIOS_AGENT_PIPE_SERVICE_UNIT_STARTLIMITBURST } else { 5 }
...
$script:MIOS_UNITS_MIOS_AI_FIRSTBOOT_SERVICE_UNIT_CONDITIONPATHEXISTS = if ($env:MIOS_UNITS_MIOS_AI_FIRSTBOOT_SERVICE_UNIT_CONDITIONPATHEXISTS) { $env:MIOS_UNITS_MIOS_AI_FIRSTBOOT_SERVICE_UNIT_CONDITIONPATHEXISTS } else { '!/var/lib/mios/.ai-firstboot-done' }
$script:MIOS_UNITS_MIOS_AI_FIRSTBOOT_SERVICE_UNIT_DESCRIPTION = if ($env:MIOS_UNITS_MIOS_AI_FIRSTBOOT_SERVICE_UNIT_DESCRIPTION) { $env:MIOS_UNITS_MIOS_AI_FIRSTBOOT_SERVICE_UNIT_DESCRIPTION } else { '''MiOS'' AI first-boot provisioning (agent venv + llama.cpp GGUFs)' }
...
$script:MIOS_UNITS_MIOS_BOUND_IMAGES_FIRSTBOOT_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNITS_MIOS_BOUND_IMAGES_FIRSTBOOT_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNITS_MIOS_BOUND_IMAGES_FIRSTBOOT_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/mios-bound-images-firstboot' }
$script:MIOS_UNITS_MIOS_BOUND_IMAGES_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNITS_MIOS_BOUND_IMAGES_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNITS_MIOS_BOUND_IMAGES_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
$script:MIOS_UNITS_MIOS_BOUND_IMAGES_FIRSTBOOT_SERVICE_SERVICE_SUCCESSEXITSTATUS = if ($env:MIOS_UNITS_MIOS_BOUND_IMAGES_FIRSTBOOT_SERVICE_SERVICE_SUCCESSEXITSTATUS) { $env:MIOS_UNITS_MIOS_BOUND_IMAGES_FIRSTBOOT_SERVICE_SERVICE_SUCCESSEXITSTATUS } else { '0 1 2 3' }
...
$script:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/ceph-bootstrap.sh' }
$script:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_EXECSTARTPOST = if ($env:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_EXECSTARTPOST) { $env:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_EXECSTARTPOST } else { '/usr/bin/touch /var/lib/ceph/.bootstrapped,-/bin/chmod 0600 /etc/ceph/ceph.client.admin.keyring' }
$script:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
$script:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_STANDARDERROR = if ($env:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_STANDARDERROR) { $env:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_STANDARDERROR } else { 'journal+console' }
$script:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_STANDARDOUTPUT = if ($env:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_STANDARDOUTPUT) { $env:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_STANDARDOUTPUT } else { 'journal+console' }
...
$script:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_UNIT_DOCUMENTATION = if ($env:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_UNIT_DOCUMENTATION) { $env:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_UNIT_DOCUMENTATION } else { 'https://docs.ceph.com/en/latest/cephadm/' }
$script:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_UNIT_STARTLIMITINTERVALSEC = if ($env:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_UNIT_STARTLIMITINTERVALSEC) { $env:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_UNIT_STARTLIMITINTERVALSEC } else { 0 }
$script:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_UNIT_WANTS = if ($env:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_UNIT_WANTS) { $env:MIOS_UNITS_MIOS_CEPH_BOOTSTRAP_SERVICE_UNIT_WANTS } else { 'network-online.target podman.socket' }
...
$script:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_LOCKPERSONALITY = if ($env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_LOCKPERSONALITY) { $env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_LOCKPERSONALITY } else { 'true' }
$script:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_NONEWPRIVILEGES = if ($env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_NONEWPRIVILEGES) { $env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_NONEWPRIVILEGES } else { 'true' }
$script:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_PRIVATETMP = if ($env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_PRIVATETMP) { $env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_PRIVATETMP } else { 'true' }
$script:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_PROTECTCONTROLGROUPS = if ($env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_PROTECTCONTROLGROUPS) { $env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_PROTECTCONTROLGROUPS } else { 'true' }
$script:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_PROTECTHOME = if ($env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_PROTECTHOME) { $env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_PROTECTHOME } else { 'true' }
$script:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_PROTECTKERNELTUNABLES = if ($env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_PROTECTKERNELTUNABLES) { $env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_PROTECTKERNELTUNABLES } else { 'true' }
$script:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_PROTECTSYSTEM = if ($env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_PROTECTSYSTEM) { $env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_PROTECTSYSTEM } else { 'strict' }
$script:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_READWRITEPATHS = if ($env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_READWRITEPATHS) { $env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_READWRITEPATHS } else { '/var/lib/mios/daemon /var/lib/mios/scratch' }
$script:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_RESTART) { $env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_RESTART } else { 'on-failure' }
$script:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_RESTARTSEC } else { '5s' }
$script:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_RESTRICTADDRESSFAMILIES = if ($env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_RESTRICTADDRESSFAMILIES) { $env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_RESTRICTADDRESSFAMILIES } else { 'AF_INET AF_INET6 AF_UNIX' }
$script:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_RESTRICTREALTIME = if ($env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_RESTRICTREALTIME) { $env:MIOS_UNITS_MIOS_DAEMON_SERVICE_SERVICE_RESTRICTREALTIME } else { 'true' }
...
$script:MIOS_UNITS_MIOS_DAEMON_SERVICE_UNIT_WANTS = if ($env:MIOS_UNITS_MIOS_DAEMON_SERVICE_UNIT_WANTS) { $env:MIOS_UNITS_MIOS_DAEMON_SERVICE_UNIT_WANTS } else { 'mios-llm-light.service' }
$script:MIOS_UNITS_MIOS_DASHBOARD_ISSUE_TIMER_INSTALL_WANTEDBY = if ($env:MIOS_UNITS_MIOS_DASHBOARD_ISSUE_TIMER_INSTALL_WANTEDBY) { $env:MIOS_UNITS_MIOS_DASHBOARD_ISSUE_TIMER_INSTALL_WANTEDBY } else { 'timers.target' }
$script:MIOS_UNITS_MIOS_DASHBOARD_ISSUE_TIMER_TIMER_ACCURACYSEC = if ($env:MIOS_UNITS_MIOS_DASHBOARD_ISSUE_TIMER_TIMER_ACCURACYSEC) { $env:MIOS_UNITS_MIOS_DASHBOARD_ISSUE_TIMER_TIMER_ACCURACYSEC } else { '30s' }
$script:MIOS_UNITS_MIOS_DASHBOARD_ISSUE_TIMER_TIMER_ONBOOTSEC = if ($env:MIOS_UNITS_MIOS_DASHBOARD_ISSUE_TIMER_TIMER_ONBOOTSEC) { $env:MIOS_UNITS_MIOS_DASHBOARD_ISSUE_TIMER_TIMER_ONBOOTSEC } else { '2min' }
...
$script:MIOS_UNITS_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_GROUP = if ($env:MIOS_UNITS_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_GROUP) { $env:MIOS_UNITS_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_GROUP } else { 'mios-ai' }
$script:MIOS_UNITS_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_NONEWPRIVILEGES = if ($env:MIOS_UNITS_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_NONEWPRIVILEGES) { $env:MIOS_UNITS_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_NONEWPRIVILEGES } else { 'true' }
$script:MIOS_UNITS_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_PRIVATETMP = if ($env:MIOS_UNITS_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_PRIVATETMP) { $env:MIOS_UNITS_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_PRIVATETMP } else { 'true' }
$script:MIOS_UNITS_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_PROTECTHOME = if ($env:MIOS_UNITS_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_PROTECTHOME) { $env:MIOS_UNITS_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_PROTECTHOME } else { 'true' }
$script:MIOS_UNITS_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_PROTECTSYSTEM = if ($env:MIOS_UNITS_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_PROTECTSYSTEM) { $env:MIOS_UNITS_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_PROTECTSYSTEM } else { 'strict' }
...
$script:MIOS_UNITS_MIOS_EMBED_BACKFILL_SERVICE_UNIT_DOCUMENTATION = if ($env:MIOS_UNITS_MIOS_EMBED_BACKFILL_SERVICE_UNIT_DOCUMENTATION) { $env:MIOS_UNITS_MIOS_EMBED_BACKFILL_SERVICE_UNIT_DOCUMENTATION } else { 'file:///usr/lib/mios/agent-pipe/mios_pipe/memory/embed_backfill.py' }
$script:MIOS_UNITS_MIOS_EMBED_BACKFILL_SERVICE_UNIT_REQUIRES = if ($env:MIOS_UNITS_MIOS_EMBED_BACKFILL_SERVICE_UNIT_REQUIRES) { $env:MIOS_UNITS_MIOS_EMBED_BACKFILL_SERVICE_UNIT_REQUIRES } else { 'mios-pgvector.service' }
$script:MIOS_UNITS_MIOS_EMBED_BACKFILL_TIMER_INSTALL_WANTEDBY = if ($env:MIOS_UNITS_MIOS_EMBED_BACKFILL_TIMER_INSTALL_WANTEDBY) { $env:MIOS_UNITS_MIOS_EMBED_BACKFILL_TIMER_INSTALL_WANTEDBY } else { 'timers.target' }
$script:MIOS_UNITS_MIOS_EMBED_BACKFILL_TIMER_TIMER_ACCURACYSEC = if ($env:MIOS_UNITS_MIOS_EMBED_BACKFILL_TIMER_TIMER_ACCURACYSEC) { $env:MIOS_UNITS_MIOS_EMBED_BACKFILL_TIMER_TIMER_ACCURACYSEC } else { '1min' }
$script:MIOS_UNITS_MIOS_EMBED_BACKFILL_TIMER_TIMER_ONBOOTSEC = if ($env:MIOS_UNITS_MIOS_EMBED_BACKFILL_TIMER_TIMER_ONBOOTSEC) { $env:MIOS_UNITS_MIOS_EMBED_BACKFILL_TIMER_TIMER_ONBOOTSEC } else { '5min' }
...
$script:MIOS_UNITS_MIOS_FINETUNE_SERVE_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNITS_MIOS_FINETUNE_SERVE_SERVICE_SERVICE_RESTART) { $env:MIOS_UNITS_MIOS_FINETUNE_SERVE_SERVICE_SERVICE_RESTART } else { 'on-failure' }
$script:MIOS_UNITS_MIOS_FINETUNE_SERVE_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNITS_MIOS_FINETUNE_SERVE_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNITS_MIOS_FINETUNE_SERVE_SERVICE_SERVICE_RESTARTSEC } else { 5 }
$script:MIOS_UNITS_MIOS_FINETUNE_SERVE_SERVICE_SERVICE_TIMEOUTSTARTSEC = if ($env:MIOS_UNITS_MIOS_FINETUNE_SERVE_SERVICE_SERVICE_TIMEOUTSTARTSEC) { $env:MIOS_UNITS_MIOS_FINETUNE_SERVE_SERVICE_SERVICE_TIMEOUTSTARTSEC } else { 180 }
...
$script:MIOS_UNITS_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNITS_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNITS_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_EXECSTART } else { '/bin/bash -c ''\
    ...
    exit 0\''' }
$script:MIOS_UNITS_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_PROTECTCONTROLGROUPS = if ($env:MIOS_UNITS_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_PROTECTCONTROLGROUPS) { $env:MIOS_UNITS_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_PROTECTCONTROLGROUPS } else { 'yes' }
$script:MIOS_UNITS_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_PROTECTKERNELTUNABLES = if ($env:MIOS_UNITS_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_PROTECTKERNELTUNABLES) { $env:MIOS_UNITS_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_PROTECTKERNELTUNABLES } else { 'yes' }
$script:MIOS_UNITS_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNITS_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNITS_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
...
$script:MIOS_UNITS_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNITS_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNITS_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNITS_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNITS_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNITS_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/mios-forgejo-runner-firstboot.sh' }
$script:MIOS_UNITS_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNITS_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNITS_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
$script:MIOS_UNITS_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNITS_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_RESTART) { $env:MIOS_UNITS_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_RESTART } else { 'on-failure' }
$script:MIOS_UNITS_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNITS_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNITS_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_RESTARTSEC } else { 30 }
$script:MIOS_UNITS_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_TIMEOUTSTARTSEC = if ($env:MIOS_UNITS_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_TIMEOUTSTARTSEC) { $env:MIOS_UNITS_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_TIMEOUTSTARTSEC } else { '180s' }
...
$script:MIOS_UNITS_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_UNIT_STARTLIMITBURST = if ($env:MIOS_UNITS_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_UNIT_STARTLIMITBURST) { $env:MIOS_UNITS_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_UNIT_STARTLIMITBURST } else { 6 }
...
$script:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_NONEWPRIVILEGES = if ($env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_NONEWPRIVILEGES) { $env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_NONEWPRIVILEGES } else { 'yes' }
$script:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_PRIVATETMP = if ($env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_PRIVATETMP) { $env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_PRIVATETMP } else { 'yes' }
$script:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_PROTECTHOME = if ($env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_PROTECTHOME) { $env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_PROTECTHOME } else { 'yes' }
$script:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_PROTECTSYSTEM = if ($env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_PROTECTSYSTEM) { $env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_PROTECTSYSTEM } else { 'strict' }
$script:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_READWRITEPATHS = if ($env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_READWRITEPATHS) { $env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_READWRITEPATHS } else { '/etc/mios/forge /var/lib/mios/forge /var/log/mios/forge /var/lib/containers /run' }
$script:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
$script:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_RESTART) { $env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_RESTART } else { 'on-failure' }
$script:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_RESTARTSEC } else { 30 }
$script:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_RESTRICTREALTIME = if ($env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_RESTRICTREALTIME) { $env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_RESTRICTREALTIME } else { 'yes' }
...
$script:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_CONDITIONPATHEXISTS = if ($env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_CONDITIONPATHEXISTS) { $env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_CONDITIONPATHEXISTS } else { '!/var/lib/mios/forge/.firstboot-done' }
$script:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_CONDITIONVIRTUALIZATION = if ($env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_CONDITIONVIRTUALIZATION) { $env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_CONDITIONVIRTUALIZATION } else { '|!container,|wsl' }
$script:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_DESCRIPTION = if ($env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_DESCRIPTION) { $env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_DESCRIPTION } else { '''MiOS'' Forge first-boot admin-bootstrap (Forgejo)' }
$script:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_DOCUMENTATION = if ($env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_DOCUMENTATION) { $env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_DOCUMENTATION } else { 'https://github.com/mios-dev/MiOS/blob/main/etc/containers/systemd/mios-forge.container' }
$script:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_STARTLIMITBURST = if ($env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_STARTLIMITBURST) { $env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_STARTLIMITBURST } else { 6 }
$script:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_STARTLIMITINTERVALSEC = if ($env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_STARTLIMITINTERVALSEC) { $env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_STARTLIMITINTERVALSEC } else { 1200 }
$script:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_WANTS = if ($env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_WANTS) { $env:MIOS_UNITS_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_WANTS } else { 'mios-forge.service network-online.target' }
$script:MIOS_UNITS_MIOS_GPU_AMD_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNITS_MIOS_GPU_AMD_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNITS_MIOS_GPU_AMD_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNITS_MIOS_GPU_AMD_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNITS_MIOS_GPU_AMD_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNITS_MIOS_GPU_AMD_SERVICE_SERVICE_EXECSTART } else { '/usr/bin/bash -c '' \
  ...
  exit 0\''' }
$script:MIOS_UNITS_MIOS_GPU_AMD_SERVICE_SERVICE_EXECSTARTPRE = if ($env:MIOS_UNITS_MIOS_GPU_AMD_SERVICE_SERVICE_EXECSTARTPRE) { $env:MIOS_UNITS_MIOS_GPU_AMD_SERVICE_SERVICE_EXECSTARTPRE } else { '/usr/sbin/modprobe -a amdgpu' }
$script:MIOS_UNITS_MIOS_GPU_AMD_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNITS_MIOS_GPU_AMD_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNITS_MIOS_GPU_AMD_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
$script:MIOS_UNITS_MIOS_GPU_AMD_SERVICE_SERVICE_STANDARDERROR = if ($env:MIOS_UNITS_MIOS_GPU_AMD_SERVICE_SERVICE_STANDARDERROR) { $env:MIOS_UNITS_MIOS_GPU_AMD_SERVICE_SERVICE_STANDARDERROR } else { 'journal' }
...
$script:MIOS_UNITS_MIOS_GPU_AMD_SERVICE_UNIT_DOCUMENTATION = if ($env:MIOS_UNITS_MIOS_GPU_AMD_SERVICE_UNIT_DOCUMENTATION) { $env:MIOS_UNITS_MIOS_GPU_AMD_SERVICE_UNIT_DOCUMENTATION } else { 'https://github.com/MiOS-DEV/MiOS/blob/main/docs/gpu-passthrough.md' }
$script:MIOS_UNITS_MIOS_GPU_DETECT_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNITS_MIOS_GPU_DETECT_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNITS_MIOS_GPU_DETECT_SERVICE_INSTALL_WANTEDBY } else { 'sysinit.target' }
$script:MIOS_UNITS_MIOS_GPU_DETECT_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNITS_MIOS_GPU_DETECT_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNITS_MIOS_GPU_DETECT_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/gpu-detect' }
$script:MIOS_UNITS_MIOS_GPU_DETECT_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNITS_MIOS_GPU_DETECT_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNITS_MIOS_GPU_DETECT_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
...
$script:MIOS_UNITS_MIOS_GPU_DETECT_SERVICE_UNIT_DESCRIPTION = if ($env:MIOS_UNITS_MIOS_GPU_DETECT_SERVICE_UNIT_DESCRIPTION) { $env:MIOS_UNITS_MIOS_GPU_DETECT_SERVICE_UNIT_DESCRIPTION } else { '''MiOS'' GPU Environment Detection' }
$script:MIOS_UNITS_MIOS_GPU_INTEL_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNITS_MIOS_GPU_INTEL_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNITS_MIOS_GPU_INTEL_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNITS_MIOS_GPU_INTEL_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNITS_MIOS_GPU_INTEL_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNITS_MIOS_GPU_INTEL_SERVICE_SERVICE_EXECSTART } else { '/usr/bin/bash -c '' \
  ...
  exit 0\''' }
$script:MIOS_UNITS_MIOS_GPU_INTEL_SERVICE_SERVICE_EXECSTARTPRE = if ($env:MIOS_UNITS_MIOS_GPU_INTEL_SERVICE_SERVICE_EXECSTARTPRE) { $env:MIOS_UNITS_MIOS_GPU_INTEL_SERVICE_SERVICE_EXECSTARTPRE } else { '-/usr/sbin/modprobe -a i915 xe amdgpu' }
$script:MIOS_UNITS_MIOS_GPU_INTEL_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNITS_MIOS_GPU_INTEL_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNITS_MIOS_GPU_INTEL_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
$script:MIOS_UNITS_MIOS_GPU_INTEL_SERVICE_SERVICE_STANDARDERROR = if ($env:MIOS_UNITS_MIOS_GPU_INTEL_SERVICE_SERVICE_STANDARDERROR) { $env:MIOS_UNITS_MIOS_GPU_INTEL_SERVICE_SERVICE_STANDARDERROR } else { 'journal' }
...
$script:MIOS_UNITS_MIOS_GPU_NVIDIA_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNITS_MIOS_GPU_NVIDIA_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNITS_MIOS_GPU_NVIDIA_SERVICE_SERVICE_EXECSTART } else { '/usr/bin/bash -c '' \
  ...
  exit 0\''' }
$script:MIOS_UNITS_MIOS_GPU_NVIDIA_SERVICE_SERVICE_EXECSTARTPRE = if ($env:MIOS_UNITS_MIOS_GPU_NVIDIA_SERVICE_SERVICE_EXECSTARTPRE) { $env:MIOS_UNITS_MIOS_GPU_NVIDIA_SERVICE_SERVICE_EXECSTARTPRE } else { '/usr/sbin/modprobe -a nvidia nvidia_uvm nvidia_modeset nvidia_drm' }
$script:MIOS_UNITS_MIOS_GPU_NVIDIA_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNITS_MIOS_GPU_NVIDIA_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNITS_MIOS_GPU_NVIDIA_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
$script:MIOS_UNITS_MIOS_GPU_NVIDIA_SERVICE_SERVICE_STANDARDERROR = if ($env:MIOS_UNITS_MIOS_GPU_NVIDIA_SERVICE_SERVICE_STANDARDERROR) { $env:MIOS_UNITS_MIOS_GPU_NVIDIA_SERVICE_SERVICE_STANDARDERROR } else { 'journal' }
$script:MIOS_UNITS_MIOS_GPU_NVIDIA_SERVICE_SERVICE_STANDARDOUTPUT = if ($env:MIOS_UNITS_MIOS_GPU_NVIDIA_SERVICE_SERVICE_STANDARDOUTPUT) { $env:MIOS_UNITS_MIOS_GPU_NVIDIA_SERVICE_SERVICE_STANDARDOUTPUT } else { 'journal' }
...
$script:MIOS_UNITS_MIOS_GPU_NVIDIA_SERVICE_UNIT_DOCUMENTATION = if ($env:MIOS_UNITS_MIOS_GPU_NVIDIA_SERVICE_UNIT_DOCUMENTATION) { $env:MIOS_UNITS_MIOS_GPU_NVIDIA_SERVICE_UNIT_DOCUMENTATION } else { 'https://github.com/MiOS-DEV/MiOS/blob/main/docs/gpu-passthrough.md' }
$script:MIOS_UNITS_MIOS_GPU_PV_DETECT_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNITS_MIOS_GPU_PV_DETECT_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNITS_MIOS_GPU_PV_DETECT_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNITS_MIOS_GPU_PV_DETECT_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNITS_MIOS_GPU_PV_DETECT_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNITS_MIOS_GPU_PV_DETECT_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/gpu-pv-detect' }
$script:MIOS_UNITS_MIOS_GPU_PV_DETECT_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNITS_MIOS_GPU_PV_DETECT_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNITS_MIOS_GPU_PV_DETECT_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
...
$script:MIOS_UNITS_MIOS_GPU_STATUS_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNITS_MIOS_GPU_STATUS_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNITS_MIOS_GPU_STATUS_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNITS_MIOS_GPU_STATUS_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNITS_MIOS_GPU_STATUS_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNITS_MIOS_GPU_STATUS_SERVICE_SERVICE_EXECSTART } else { '/usr/bin/bash -c '' \
  set +e; \
  ...
  exit 0\''' }
$script:MIOS_UNITS_MIOS_GPU_STATUS_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNITS_MIOS_GPU_STATUS_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNITS_MIOS_GPU_STATUS_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
$script:MIOS_UNITS_MIOS_GPU_STATUS_SERVICE_SERVICE_RUNTIMEDIRECTORY = if ($env:MIOS_UNITS_MIOS_GPU_STATUS_SERVICE_SERVICE_RUNTIMEDIRECTORY) { $env:MIOS_UNITS_MIOS_GPU_STATUS_SERVICE_SERVICE_RUNTIMEDIRECTORY } else { 'mios' }
$script:MIOS_UNITS_MIOS_GPU_STATUS_SERVICE_SERVICE_RUNTIMEDIRECTORYMODE = if ($env:MIOS_UNITS_MIOS_GPU_STATUS_SERVICE_SERVICE_RUNTIMEDIRECTORYMODE) { $env:MIOS_UNITS_MIOS_GPU_STATUS_SERVICE_SERVICE_RUNTIMEDIRECTORYMODE } else { 0755 }
$script:MIOS_UNITS_MIOS_GPU_STATUS_SERVICE_SERVICE_STANDARDERROR = if ($env:MIOS_UNITS_MIOS_GPU_STATUS_SERVICE_SERVICE_STANDARDERROR) { $env:MIOS_UNITS_MIOS_GPU_STATUS_SERVICE_SERVICE_STANDARDERROR } else { 'journal' }
...
$script:MIOS_UNITS_MIOS_GPU_STATUS_SERVICE_UNIT_DOCUMENTATION = if ($env:MIOS_UNITS_MIOS_GPU_STATUS_SERVICE_UNIT_DOCUMENTATION) { $env:MIOS_UNITS_MIOS_GPU_STATUS_SERVICE_UNIT_DOCUMENTATION } else { 'https://github.com/MiOS-DEV/MiOS/blob/main/docs/gpu-passthrough.md' }
...
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_NONEWPRIVILEGES = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_NONEWPRIVILEGES) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_NONEWPRIVILEGES } else { 'true' }
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_PRIVATETMP = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_PRIVATETMP) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_PRIVATETMP } else { 'false' }
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RESTART) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RESTART } else { 'on-failure' }
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RESTARTSEC } else { '5s' }
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RUNTIMEDIRECTORY = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RUNTIMEDIRECTORY) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RUNTIMEDIRECTORY } else { 'mios-hermes-browser' }
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RUNTIMEDIRECTORYMODE = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RUNTIMEDIRECTORYMODE) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RUNTIMEDIRECTORYMODE } else { 0700 }
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_STATEDIRECTORY = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_STATEDIRECTORY) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_STATEDIRECTORY } else { 'mios/hermes-browser/profile' }
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_STATEDIRECTORYMODE = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_STATEDIRECTORYMODE) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_STATEDIRECTORYMODE } else { 0700 }
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_TIMEOUTSTOPSEC = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_TIMEOUTSTOPSEC) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_SERVICE_TIMEOUTSTOPSEC } else { '30s' }
...
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_UNIT_DESCRIPTION = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_UNIT_DESCRIPTION) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_UNIT_DESCRIPTION } else { '''MiOS'' Hermes-Browser (ChromeDev w/ CDP for Hermes-Agent)' }
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_UNIT_DOCUMENTATION = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_UNIT_DOCUMENTATION) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_UNIT_DOCUMENTATION } else { 'https://chromedevtools.github.io/devtools-protocol/' }
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_UNIT_WANTS = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_UNIT_WANTS) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_SERVICE_UNIT_WANTS } else { 'mios-agent-pipe.service' }
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_ENVIRONMENT = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_ENVIRONMENT) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_ENVIRONMENT } else { 'HOME=/var/lib/mios/hermes-worker,XDG_RUNTIME_DIR=/run/mios-hermes-browser-worker,HERMES_BROWSER_HEADLESS=1,HERMES_BROWSER_CDP_PORT=${MIOS_PORT_CHROME_CDP_WORKER:-9223},HERMES_BROWSER_PROFILE_DIR=/var/lib/mios/hermes-browser/profile-w2,HERMES_BROWSER_LOG=/var/lib/mios/hermes-browser/launch-w2.log,NO_AT_BRIDGE=1' }
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/mios-hermes-browser start' }
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_GROUP = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_GROUP) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_GROUP } else { 'mios-ai' }
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_NONEWPRIVILEGES = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_NONEWPRIVILEGES) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_NONEWPRIVILEGES } else { 'true' }
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_PRIVATETMP = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_PRIVATETMP) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_PRIVATETMP } else { 'false' }
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RESTART) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RESTART } else { 'on-failure' }
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RESTARTSEC } else { '5s' }
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RUNTIMEDIRECTORY = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RUNTIMEDIRECTORY) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RUNTIMEDIRECTORY } else { 'mios-hermes-browser-worker' }
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RUNTIMEDIRECTORYMODE = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RUNTIMEDIRECTORYMODE) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RUNTIMEDIRECTORYMODE } else { 0700 }
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_STATEDIRECTORY = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_STATEDIRECTORY) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_STATEDIRECTORY } else { 'mios/hermes-browser/profile-w2' }
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_STATEDIRECTORYMODE = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_STATEDIRECTORYMODE) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_STATEDIRECTORYMODE } else { 0700 }
$script:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_TIMEOUTSTOPSEC = if ($env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_TIMEOUTSTOPSEC) { $env:MIOS_UNITS_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_TIMEOUTSTOPSEC } else { '30s' }
...
$script:MIOS_UNITS_MIOS_HYBRID_TARGET_UNIT_WANTS = if ($env:MIOS_UNITS_MIOS_HYBRID_TARGET_UNIT_WANTS) { $env:MIOS_UNITS_MIOS_HYBRID_TARGET_UNIT_WANTS } else { 'k3s-agent.service' }
...
$script:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_COMMENT4 = if ($env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_COMMENT4) { $env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_COMMENT4 } else { '# Drain timeout: MCP serves a long-lived TCP socket but in-flight
# context requests are sub-second; 30 s is generous.' }
$script:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/mcp-server-runner' }
$script:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_EXECSTARTPRE = if ($env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_EXECSTARTPRE) { $env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_EXECSTARTPRE } else { '/usr/libexec/mios/mcp-init.sh' }
$script:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_GROUP = if ($env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_GROUP) { $env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_GROUP } else { 'mios-ai' }
$script:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_NONEWPRIVILEGES = if ($env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_NONEWPRIVILEGES) { $env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_NONEWPRIVILEGES } else { 'true' }
$script:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_PRIVATETMP = if ($env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_PRIVATETMP) { $env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_PRIVATETMP } else { 'true' }
$script:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_PROTECTHOME = if ($env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_PROTECTHOME) { $env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_PROTECTHOME } else { 'read-only' }
$script:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_PROTECTSYSTEM = if ($env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_PROTECTSYSTEM) { $env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_PROTECTSYSTEM } else { 'strict' }
$script:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_READWRITEPATHS = if ($env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_READWRITEPATHS) { $env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_READWRITEPATHS } else { '/var/lib/mios/mcp /var/log/mios/mcp' }
$script:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_RESTART) { $env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_RESTART } else { 'always' }
$script:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_RESTARTSEC } else { 5 }
$script:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_TIMEOUTSTOPSEC = if ($env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_TIMEOUTSTOPSEC) { $env:MIOS_UNITS_MIOS_MCP_SERVICE_SERVICE_TIMEOUTSTOPSEC } else { '30s' }
...
$script:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_ENVIRONMENT = if ($env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_ENVIRONMENT) { $env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_ENVIRONMENT } else { 'HOME=/var/lib/mios/opencode-gateway/work,MIOS_PORT_OPENCODE_GATEWAY=${MIOS_PORT_OPENCODE_GATEWAY:-8780},MIOS_OPENCODE_HOST=127.0.0.1,MIOS_OPENCODE_BIN=/usr/lib/mios/agents/opencode/bin/opencode,MIOS_OPENCODE_MODEL=mios-opencode:latest,MIOS_OPENCODE_PROVIDER=local,MIOS_OPENCODE_CONFIG=/etc/mios/opencode/opencode.json,MIOS_OPENCODE_TIMEOUT_S=90' }
$script:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_EXECSTART } else { '/usr/lib/mios/agents/.venv/bin/python3 -u /usr/lib/mios/agents/opencode-gateway/server.py' }
$script:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_GROUP = if ($env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_GROUP) { $env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_GROUP } else { 'mios-ai' }
$script:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_LOCKPERSONALITY = if ($env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_LOCKPERSONALITY) { $env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_LOCKPERSONALITY } else { 'true' }
$script:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_NONEWPRIVILEGES = if ($env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_NONEWPRIVILEGES) { $env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_NONEWPRIVILEGES } else { 'true' }
$script:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PRIVATETMP = if ($env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PRIVATETMP) { $env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PRIVATETMP } else { 'true' }
$script:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTCONTROLGROUPS = if ($env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTCONTROLGROUPS) { $env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTCONTROLGROUPS } else { 'true' }
$script:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTHOME = if ($env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTHOME) { $env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTHOME } else { 'true' }
$script:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTKERNELMODULES = if ($env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTKERNELMODULES) { $env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTKERNELMODULES } else { 'true' }
$script:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTKERNELTUNABLES = if ($env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTKERNELTUNABLES) { $env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTKERNELTUNABLES } else { 'true' }
$script:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTSYSTEM = if ($env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTSYSTEM) { $env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTSYSTEM } else { 'strict' }
$script:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_READWRITEPATHS = if ($env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_READWRITEPATHS) { $env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_READWRITEPATHS } else { '/etc/mios /var/lib/mios' }
$script:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_RESTART) { $env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_RESTART } else { 'on-failure' }
$script:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_RESTARTSEC } else { '10s' }
$script:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_RESTRICTSUIDSGID = if ($env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_RESTRICTSUIDSGID) { $env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_RESTRICTSUIDSGID } else { 'true' }
$script:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_TIMEOUTSTARTSEC = if ($env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_TIMEOUTSTARTSEC) { $env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_TIMEOUTSTARTSEC } else { '30s' }
$script:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_TYPE = if ($env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_TYPE) { $env:MIOS_UNITS_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_TYPE } else { 'simple' }
...
$script:MIOS_UNITS_MIOS_PGVECTOR_BACKUP_SERVICE_SERVICE_PRIVATETMP = if ($env:MIOS_UNITS_MIOS_PGVECTOR_BACKUP_SERVICE_SERVICE_PRIVATETMP) { $env:MIOS_UNITS_MIOS_PGVECTOR_BACKUP_SERVICE_SERVICE_PRIVATETMP } else { 'true' }
...
$script:MIOS_UNITS_MIOS_PGVECTOR_BACKUP_SERVICE_UNIT_DESCRIPTION = if ($env:MIOS_UNITS_MIOS_PGVECTOR_BACKUP_SERVICE_UNIT_DESCRIPTION) { $env:MIOS_UNITS_MIOS_PGVECTOR_BACKUP_SERVICE_UNIT_DESCRIPTION } else { '''MiOS'' daily pg_dump backup of the unified agent-plane datastore (pgvector)' }
$script:MIOS_UNITS_MIOS_PGVECTOR_BACKUP_SERVICE_UNIT_DOCUMENTATION = if ($env:MIOS_UNITS_MIOS_PGVECTOR_BACKUP_SERVICE_UNIT_DOCUMENTATION) { $env:MIOS_UNITS_MIOS_PGVECTOR_BACKUP_SERVICE_UNIT_DOCUMENTATION } else { 'file:///usr/lib/systemd/system/mios-pgvector-backup.service' }
...
$script:MIOS_UNITS_MIOS_POLICY_ARBITER_SERVICE_SERVICE_GROUP = if ($env:MIOS_UNITS_MIOS_POLICY_ARBITER_SERVICE_SERVICE_GROUP) { $env:MIOS_UNITS_MIOS_POLICY_ARBITER_SERVICE_SERVICE_GROUP } else { 'mios-ai' }
$script:MIOS_UNITS_MIOS_POLICY_ARBITER_SERVICE_SERVICE_NONEWPRIVILEGES = if ($env:MIOS_UNITS_MIOS_POLICY_ARBITER_SERVICE_SERVICE_NONEWPRIVILEGES) { $env:MIOS_UNITS_MIOS_POLICY_ARBITER_SERVICE_SERVICE_NONEWPRIVILEGES } else { 'true' }
$script:MIOS_UNITS_MIOS_POLICY_ARBITER_SERVICE_SERVICE_PRIVATETMP = if ($env:MIOS_UNITS_MIOS_POLICY_ARBITER_SERVICE_SERVICE_PRIVATETMP) { $env:MIOS_UNITS_MIOS_POLICY_ARBITER_SERVICE_SERVICE_PRIVATETMP } else { 'true' }
$script:MIOS_UNITS_MIOS_POLICY_ARBITER_SERVICE_SERVICE_PROTECTHOME = if ($env:MIOS_UNITS_MIOS_POLICY_ARBITER_SERVICE_SERVICE_PROTECTHOME) { $env:MIOS_UNITS_MIOS_POLICY_ARBITER_SERVICE_SERVICE_PROTECTHOME } else { 'true' }
$script:MIOS_UNITS_MIOS_POLICY_ARBITER_SERVICE_SERVICE_PROTECTSYSTEM = if ($env:MIOS_UNITS_MIOS_POLICY_ARBITER_SERVICE_SERVICE_PROTECTSYSTEM) { $env:MIOS_UNITS_MIOS_POLICY_ARBITER_SERVICE_SERVICE_PROTECTSYSTEM } else { 'strict' }
$script:MIOS_UNITS_MIOS_POLICY_ARBITER_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNITS_MIOS_POLICY_ARBITER_SERVICE_SERVICE_RESTART) { $env:MIOS_UNITS_MIOS_POLICY_ARBITER_SERVICE_SERVICE_RESTART } else { 'on-failure' }
$script:MIOS_UNITS_MIOS_POLICY_ARBITER_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNITS_MIOS_POLICY_ARBITER_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNITS_MIOS_POLICY_ARBITER_SERVICE_SERVICE_RESTARTSEC } else { '15s' }
...
$script:MIOS_UNITS_MIOS_SHELL_SESSION_GC_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNITS_MIOS_SHELL_SESSION_GC_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNITS_MIOS_SHELL_SESSION_GC_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNITS_MIOS_SHELL_SESSION_GC_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNITS_MIOS_SHELL_SESSION_GC_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNITS_MIOS_SHELL_SESSION_GC_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/mios-shell-session gc' }
$script:MIOS_UNITS_MIOS_SHELL_SESSION_GC_SERVICE_SERVICE_GROUP = if ($env:MIOS_UNITS_MIOS_SHELL_SESSION_GC_SERVICE_SERVICE_GROUP) { $env:MIOS_UNITS_MIOS_SHELL_SESSION_GC_SERVICE_SERVICE_GROUP } else { 'mios-ai' }
...
$script:MIOS_UNITS_MIOS_SKILLS_MINER_TIMER_TIMER_ONBOOTSEC = if ($env:MIOS_UNITS_MIOS_SKILLS_MINER_TIMER_TIMER_ONBOOTSEC) { $env:MIOS_UNITS_MIOS_SKILLS_MINER_TIMER_TIMER_ONBOOTSEC } else { '10min' }
...
$script:MIOS_UNITS_MIOS_USERDB_RENDER_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNITS_MIOS_USERDB_RENDER_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNITS_MIOS_USERDB_RENDER_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/mios-userdb-render' }
$script:MIOS_UNITS_MIOS_USERDB_RENDER_SERVICE_SERVICE_EXECSTARTPRE = if ($env:MIOS_UNITS_MIOS_USERDB_RENDER_SERVICE_SERVICE_EXECSTARTPRE) { $env:MIOS_UNITS_MIOS_USERDB_RENDER_SERVICE_SERVICE_EXECSTARTPRE } else { '-/usr/bin/podman exec mios-pgvector pg_isready -q -h 127.0.0.1 -p ${MIOS_PORT_PGVECTOR:-8600} -U mios -d mios' }
...
$script:MIOS_UNITS_MIOS_WEBTOOLS_FIRSTBOOT_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNITS_MIOS_WEBTOOLS_FIRSTBOOT_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNITS_MIOS_WEBTOOLS_FIRSTBOOT_SERVICE_SERVICE_RESTARTSEC } else { 30 }
...
$script:MIOS_UNITS_MIOS_WEBTOOLS_FIRSTBOOT_SERVICE_UNIT_WANTS = if ($env:MIOS_UNITS_MIOS_WEBTOOLS_FIRSTBOOT_SERVICE_UNIT_WANTS) { $env:MIOS_UNITS_MIOS_WEBTOOLS_FIRSTBOOT_SERVICE_UNIT_WANTS } else { 'network-online.target' }
$script:MIOS_UNITS_MIOS_WSL_FIRSTBOOT_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNITS_MIOS_WSL_FIRSTBOOT_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNITS_MIOS_WSL_FIRSTBOOT_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNITS_MIOS_WSL_FIRSTBOOT_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNITS_MIOS_WSL_FIRSTBOOT_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNITS_MIOS_WSL_FIRSTBOOT_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/wsl-firstboot' }
$script:MIOS_UNITS_MIOS_WSL_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNITS_MIOS_WSL_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNITS_MIOS_WSL_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
...
$script:MIOS_UNITS_USER_SESSION = if ($env:MIOS_UNITS_USER_SESSION) { $env:MIOS_UNITS_USER_SESSION } else { 'user@${MIOS_UID}.service' }
$script:MIOS_UNITS_VAR_HOME_MOUNT_INSTALL_WANTEDBY = if ($env:MIOS_UNITS_VAR_HOME_MOUNT_INSTALL_WANTEDBY) { $env:MIOS_UNITS_VAR_HOME_MOUNT_INSTALL_WANTEDBY } else { 'remote-fs.target' }
$script:MIOS_UNITS_VAR_HOME_MOUNT_MOUNT_OPTIONS = if ($env:MIOS_UNITS_VAR_HOME_MOUNT_MOUNT_OPTIONS) { $env:MIOS_UNITS_VAR_HOME_MOUNT_MOUNT_OPTIONS } else { 'secretfile=/etc/ceph/mios.secret,noatime,_netdev,nofail,x-systemd.device-timeout=30,x-systemd.mount-timeout=30' }
$script:MIOS_UNITS_VAR_HOME_MOUNT_MOUNT_TYPE = if ($env:MIOS_UNITS_VAR_HOME_MOUNT_MOUNT_TYPE) { $env:MIOS_UNITS_VAR_HOME_MOUNT_MOUNT_TYPE } else { 'ceph' }
...
$script:MIOS_UNITS_VAR_HOME_MOUNT_UNIT_WANTS = if ($env:MIOS_UNITS_VAR_HOME_MOUNT_UNIT_WANTS) { $env:MIOS_UNITS_VAR_HOME_MOUNT_UNIT_WANTS } else { 'network-online.target' }
$script:MIOS_UNITS_VAR_LIB_CONTAINERS_MOUNT_INSTALL_WANTEDBY = if ($env:MIOS_UNITS_VAR_LIB_CONTAINERS_MOUNT_INSTALL_WANTEDBY) { $env:MIOS_UNITS_VAR_LIB_CONTAINERS_MOUNT_INSTALL_WANTEDBY } else { 'remote-fs.target' }
$script:MIOS_UNITS_VAR_LIB_CONTAINERS_MOUNT_MOUNT_OPTIONS = if ($env:MIOS_UNITS_VAR_LIB_CONTAINERS_MOUNT_MOUNT_OPTIONS) { $env:MIOS_UNITS_VAR_LIB_CONTAINERS_MOUNT_MOUNT_OPTIONS } else { 'secretfile=/etc/ceph/mios.secret,noatime,_netdev,nofail,x-systemd.device-timeout=30,x-systemd.mount-timeout=30' }
$script:MIOS_UNITS_VAR_LIB_CONTAINERS_MOUNT_MOUNT_TYPE = if ($env:MIOS_UNITS_VAR_LIB_CONTAINERS_MOUNT_MOUNT_TYPE) { $env:MIOS_UNITS_VAR_LIB_CONTAINERS_MOUNT_MOUNT_TYPE } else { 'ceph' }
...
$script:MIOS_UNITS_VAR_LIB_CONTAINERS_MOUNT_UNIT_WANTS = if ($env:MIOS_UNITS_VAR_LIB_CONTAINERS_MOUNT_UNIT_WANTS) { $env:MIOS_UNITS_VAR_LIB_CONTAINERS_MOUNT_UNIT_WANTS } else { 'network-online.target' }
$script:MIOS_UNITS_VAR_LIB_MACHINES_MOUNT_MOUNT_OPTIONS = if ($env:MIOS_UNITS_VAR_LIB_MACHINES_MOUNT_MOUNT_OPTIONS) { $env:MIOS_UNITS_VAR_LIB_MACHINES_MOUNT_MOUNT_OPTIONS } else { 'loop' }
...
$script:MIOS_UNIT_HERMES_WORKER_FIRSTBOOT_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNIT_HERMES_WORKER_FIRSTBOOT_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNIT_HERMES_WORKER_FIRSTBOOT_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNIT_HERMES_WORKER_FIRSTBOOT_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNIT_HERMES_WORKER_FIRSTBOOT_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNIT_HERMES_WORKER_FIRSTBOOT_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/hermes-worker-firstboot' }
$script:MIOS_UNIT_HERMES_WORKER_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNIT_HERMES_WORKER_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNIT_HERMES_WORKER_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
...
$script:MIOS_UNIT_HERMES_WORKER_PATH_INSTALL_WANTEDBY = if ($env:MIOS_UNIT_HERMES_WORKER_PATH_INSTALL_WANTEDBY) { $env:MIOS_UNIT_HERMES_WORKER_PATH_INSTALL_WANTEDBY } else { 'multi-user.target' }
...
$script:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_GROUP = if ($env:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_GROUP) { $env:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_GROUP } else { 'mios-ai' }
$script:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_LIMITNOFILE = if ($env:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_LIMITNOFILE) { $env:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_LIMITNOFILE } else { 1048576 }
$script:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_NONEWPRIVILEGES = if ($env:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_NONEWPRIVILEGES) { $env:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_NONEWPRIVILEGES } else { 'false' }
$script:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_PRIVATETMP = if ($env:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_PRIVATETMP) { $env:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_PRIVATETMP } else { 'yes' }
$script:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_PROTECTHOME = if ($env:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_PROTECTHOME) { $env:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_PROTECTHOME } else { 'false' }
$script:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_PROTECTSYSTEM = if ($env:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_PROTECTSYSTEM) { $env:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_PROTECTSYSTEM } else { 'strict' }
$script:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_READWRITEPATHS = if ($env:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_READWRITEPATHS) { $env:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_READWRITEPATHS } else { '/etc/mios /var/lib/mios /run/mios-launcher' }
$script:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_RESTART) { $env:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_RESTART } else { 'on-failure' }
$script:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_RESTARTSEC } else { '10s' }
$script:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_TIMEOUTSTARTSEC = if ($env:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_TIMEOUTSTARTSEC) { $env:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_TIMEOUTSTARTSEC } else { '300s' }
$script:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_TIMEOUTSTOPSEC = if ($env:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_TIMEOUTSTOPSEC) { $env:MIOS_UNIT_HERMES_WORKER_SERVICE_SERVICE_TIMEOUTSTOPSEC } else { '240s' }
...
# provisioner (hermes-worker-firstboot) + the inference lanes, NOT the boot-time
# GGUF/vLLM fetch owned by mios-ai-firstboot.' }
$script:MIOS_UNIT_HERMES_WORKER_SERVICE_UNIT_COMMENT3 = if ($env:MIOS_UNIT_HERMES_WORKER_SERVICE_UNIT_COMMENT3) { $env:MIOS_UNIT_HERMES_WORKER_SERVICE_UNIT_COMMENT3 } else { '# Skip cleanly if the direct install didn''t land.' }
$script:MIOS_UNIT_HERMES_WORKER_SERVICE_UNIT_CONDITIONPATHEXISTS = if ($env:MIOS_UNIT_HERMES_WORKER_SERVICE_UNIT_CONDITIONPATHEXISTS) { $env:MIOS_UNIT_HERMES_WORKER_SERVICE_UNIT_CONDITIONPATHEXISTS } else { '/usr/lib/mios/agents/.venv/bin/hermes' }
$script:MIOS_UNIT_HERMES_WORKER_SERVICE_UNIT_DESCRIPTION = if ($env:MIOS_UNIT_HERMES_WORKER_SERVICE_UNIT_DESCRIPTION) { $env:MIOS_UNIT_HERMES_WORKER_SERVICE_UNIT_DESCRIPTION } else { '''MiOS'' Hermes-Worker (non-thin agent, native tool loop, :8643)' }
$script:MIOS_UNIT_HERMES_WORKER_SERVICE_UNIT_DOCUMENTATION = if ($env:MIOS_UNIT_HERMES_WORKER_SERVICE_UNIT_DOCUMENTATION) { $env:MIOS_UNIT_HERMES_WORKER_SERVICE_UNIT_DOCUMENTATION } else { 'https://github.com/NousResearch/hermes-agent' }
$script:MIOS_UNIT_HERMES_WORKER_SERVICE_UNIT_WANTS = if ($env:MIOS_UNIT_HERMES_WORKER_SERVICE_UNIT_WANTS) { $env:MIOS_UNIT_HERMES_WORKER_SERVICE_UNIT_WANTS } else { 'network-online.target mios-llm-heavy.service mios-llm-light.service hermes-worker-firstboot.service' }
...
$script:MIOS_UNIT_MIOS_ACCOUNT_SYNC_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNIT_MIOS_ACCOUNT_SYNC_SERVICE_SERVICE_RESTART) { $env:MIOS_UNIT_MIOS_ACCOUNT_SYNC_SERVICE_SERVICE_RESTART } else { 'always' }
...
$script:MIOS_UNIT_MIOS_ACCOUNT_SYNC_SERVICE_UNIT_STARTLIMITBURST = if ($env:MIOS_UNIT_MIOS_ACCOUNT_SYNC_SERVICE_UNIT_STARTLIMITBURST) { $env:MIOS_UNIT_MIOS_ACCOUNT_SYNC_SERVICE_UNIT_STARTLIMITBURST } else { 5 }
$script:MIOS_UNIT_MIOS_ACCOUNT_SYNC_SERVICE_UNIT_STARTLIMITINTERVALSEC = if ($env:MIOS_UNIT_MIOS_ACCOUNT_SYNC_SERVICE_UNIT_STARTLIMITINTERVALSEC) { $env:MIOS_UNIT_MIOS_ACCOUNT_SYNC_SERVICE_UNIT_STARTLIMITINTERVALSEC } else { 300 }
...
$script:MIOS_UNIT_MIOS_ADGUARD_FIRSTBOOT_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNIT_MIOS_ADGUARD_FIRSTBOOT_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNIT_MIOS_ADGUARD_FIRSTBOOT_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/mios-adguard-firstboot' }
$script:MIOS_UNIT_MIOS_ADGUARD_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNIT_MIOS_ADGUARD_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNIT_MIOS_ADGUARD_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
...
$script:MIOS_UNIT_MIOS_AGENTS_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNIT_MIOS_AGENTS_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNIT_MIOS_AGENTS_SERVICE_SERVICE_EXECSTART } else { "/usr/sbin/podman run --replace --name mios-agents --network=host --env PASSWORD=$($script:MIOS_DEFAULT_PASSWORD) --env MIOS_A2O_ENGINE=agy --env MIOS_A2O_WORK=/mnt/mios-root --env MIOS_A2O_ORCH_ENGINE=$($script:MIOS_A2O_ORCH_ENGINE) --env MIOS_A2O_ORCH_MODEL=$($script:MIOS_A2O_ORCH_MODEL) --env MIOS_A2O_ORCH_EFFORT=$($script:MIOS_A2O_ORCH_EFFORT) --env MIOS_A2O_LANE_A_ENGINE=$($script:MIOS_A2O_LANE_A_ENGINE) --env MIOS_A2O_LANE_A_MODEL=$($script:MIOS_A2O_LANE_A_MODEL) --env MIOS_A2O_LANE_A_EFFORT=$($script:MIOS_A2O_LANE_A_EFFORT) --env MIOS_A2O_LANE_B_ENGINE=$($script:MIOS_A2O_LANE_B_ENGINE) --env MIOS_A2O_LANE_B_MODEL=$($script:MIOS_A2O_LANE_B_MODEL) --env MIOS_A2O_LANE_B_EFFORT=$($script:MIOS_A2O_LANE_B_EFFORT) --env MIOS_A2O_CLAUDE_EFFORT_FLAG=$($script:MIOS_A2O_CLAUDE_EFFORT_FLAG) --env MIOS_A2O_AGY_EFFORT_FLAG=${MIOS_A2O_AGY_EFFORT_FLAG} --env MIOS_A2O_GEMINI_EFFORT_FLAG=${MIOS_A2O_GEMINI_EFFORT_FLAG} --volume /:/mnt/mios-root:rw,rslave --volume /var/lib/mios/agents:/home/coder:rw localhost/mios-agents:latest --bind-addr 0.0.0.0:8800 /mnt/mios-root" }
$script:MIOS_UNIT_MIOS_AGENTS_SERVICE_SERVICE_EXECSTARTPRE = if ($env:MIOS_UNIT_MIOS_AGENTS_SERVICE_SERVICE_EXECSTARTPRE) { $env:MIOS_UNIT_MIOS_AGENTS_SERVICE_SERVICE_EXECSTARTPRE } else { '/usr/libexec/mios/mios-agents-firstboot.sh' }
$script:MIOS_UNIT_MIOS_AGENTS_SERVICE_SERVICE_EXECSTOP = if ($env:MIOS_UNIT_MIOS_AGENTS_SERVICE_SERVICE_EXECSTOP) { $env:MIOS_UNIT_MIOS_AGENTS_SERVICE_SERVICE_EXECSTOP } else { '/usr/sbin/podman stop -t 10 mios-agents' }
$script:MIOS_UNIT_MIOS_AGENTS_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNIT_MIOS_AGENTS_SERVICE_SERVICE_RESTART) { $env:MIOS_UNIT_MIOS_AGENTS_SERVICE_SERVICE_RESTART } else { 'on-failure' }
$script:MIOS_UNIT_MIOS_AGENTS_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNIT_MIOS_AGENTS_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNIT_MIOS_AGENTS_SERVICE_SERVICE_RESTARTSEC } else { '10s' }
...
$script:MIOS_UNIT_MIOS_AGENTS_SERVICE_UNIT_WANTS = if ($env:MIOS_UNIT_MIOS_AGENTS_SERVICE_UNIT_WANTS) { $env:MIOS_UNIT_MIOS_AGENTS_SERVICE_UNIT_WANTS } else { 'network-online.target' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_ENVIRONMENT = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_ENVIRONMENT) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_ENVIRONMENT } else { 'MIOS_PORT_AGENT_PIPE=${MIOS_PORT_AGENT_PIPE:-8700},MIOS_AGENT_PIPE_BACKEND_LIGHT=1,MIOS_AI_MODEL=granite4.1:8b,MIOS_NO_TOOL_CHOICE_HINTS=11436,MIOS_KV_PAGING_HINTS=11436,11450,MIOS_LLM_API_HINTS=,MIOS_REFINE_TIMEOUT_S=45,MIOS_POLISH_TIMEOUT_S=45,MIOS_REFINE_MAX_TOKENS=1200,MIOS_AGENT_MEMORY_RECALL=1,MIOS_PASSPORT_AGENT_NAME=agent-pipe' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_ENVIRONMENTFILE = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_ENVIRONMENTFILE) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_ENVIRONMENTFILE } else { '-/etc/mios/agent-pipe.env,-/etc/mios/install.env' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_EXECSTART } else { '/usr/lib/mios/agents/.venv/bin/python3 -u /usr/lib/mios/agent-pipe/server.py' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_GROUP = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_GROUP) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_GROUP } else { 'mios-ai' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_NONEWPRIVILEGES = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_NONEWPRIVILEGES) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_NONEWPRIVILEGES } else { 'yes' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_PRIVATETMP = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_PRIVATETMP) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_PRIVATETMP } else { 'yes' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_PROTECTHOME = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_PROTECTHOME) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_PROTECTHOME } else { 'yes' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_PROTECTSYSTEM = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_PROTECTSYSTEM) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_PROTECTSYSTEM } else { 'strict' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_READONLYPATHS = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_READONLYPATHS) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_READONLYPATHS } else { '/var/lib/mios/agent-passports' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_READWRITEPATHS = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_READWRITEPATHS) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_READWRITEPATHS } else { '/var/lib/mios/agent-pipe /var/lib/mios/skills /var/log /var/lib/mios/agent-env /var/lib/mios/ai' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_RESTART) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_RESTART } else { 'on-failure' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_RESTARTSEC } else { '5s' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_SUPPLEMENTARYGROUPS = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_SUPPLEMENTARYGROUPS) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_SUPPLEMENTARYGROUPS } else { 'mios-hermes' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_TIMEOUTSTARTSEC = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_TIMEOUTSTARTSEC) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_TIMEOUTSTARTSEC } else { '60s' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_TYPE = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_TYPE) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_SERVICE_TYPE } else { 'simple' }
...
# firstboot''s multi-GB download blocked the router for the whole download. firstboot
# `systemctl restart --no-block`s this unit once models land, so the edge is redundant.' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_COMMENT3 = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_COMMENT3) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_COMMENT3 } else { '# R6: the pipe touches pgvector on every route, so upgrade the DB edge from After=
# (ordering only) to a hard Requires= -- a missing DB should hold/fail the unit.' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_COMMENT4 = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_COMMENT4) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_COMMENT4 } else { '# R6: bound the existing Restart=on-failure so a DB-down window (many quick
# restarts) can''t trip the default 5/10s start limit and land permanently failed.' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_COMMENT5 = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_COMMENT5) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_COMMENT5 } else { '# The venv python is baked into the FINAL image, but on the podman-MiOS-DEV substrate it is
# built at first boot by mios-ai-firstboot. Skip (rather than 203/EXEC-fail) until it exists.' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_CONDITIONPATHEXISTS = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_CONDITIONPATHEXISTS) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_CONDITIONPATHEXISTS } else { '/usr/lib/mios/agents/.venv/bin/python3' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_DESCRIPTION = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_DESCRIPTION) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_DESCRIPTION } else { '''MiOS'' Agent Pipe (router + refine + critic FastAPI; fronts hermes for every gateway)' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_DOCUMENTATION = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_DOCUMENTATION) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_DOCUMENTATION } else { 'file:///usr/lib/mios/agent-pipe/server.py' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_REQUIRES = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_REQUIRES) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_REQUIRES } else { 'mios-pgvector.service' }
$script:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_STARTLIMITBURST = if ($env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_STARTLIMITBURST) { $env:MIOS_UNIT_MIOS_AGENT_PIPE_SERVICE_UNIT_STARTLIMITBURST } else { 5 }
...
# (exit 0) on a partial provision, so the unit shows success and the timer simply
# re-fires it on its cadence until the sentinel above is written.' }
$script:MIOS_UNIT_MIOS_AI_FIRSTBOOT_SERVICE_UNIT_CONDITIONPATHEXISTS = if ($env:MIOS_UNIT_MIOS_AI_FIRSTBOOT_SERVICE_UNIT_CONDITIONPATHEXISTS) { $env:MIOS_UNIT_MIOS_AI_FIRSTBOOT_SERVICE_UNIT_CONDITIONPATHEXISTS } else { '!/var/lib/mios/.ai-firstboot-done' }
$script:MIOS_UNIT_MIOS_AI_FIRSTBOOT_SERVICE_UNIT_DESCRIPTION = if ($env:MIOS_UNIT_MIOS_AI_FIRSTBOOT_SERVICE_UNIT_DESCRIPTION) { $env:MIOS_UNIT_MIOS_AI_FIRSTBOOT_SERVICE_UNIT_DESCRIPTION } else { '''MiOS'' AI first-boot provisioning (agent venv + llama.cpp GGUFs)' }
...
$script:MIOS_UNIT_MIOS_BOUND_IMAGES_FIRSTBOOT_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNIT_MIOS_BOUND_IMAGES_FIRSTBOOT_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNIT_MIOS_BOUND_IMAGES_FIRSTBOOT_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/mios-bound-images-firstboot' }
$script:MIOS_UNIT_MIOS_BOUND_IMAGES_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNIT_MIOS_BOUND_IMAGES_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNIT_MIOS_BOUND_IMAGES_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
$script:MIOS_UNIT_MIOS_BOUND_IMAGES_FIRSTBOOT_SERVICE_SERVICE_SUCCESSEXITSTATUS = if ($env:MIOS_UNIT_MIOS_BOUND_IMAGES_FIRSTBOOT_SERVICE_SERVICE_SUCCESSEXITSTATUS) { $env:MIOS_UNIT_MIOS_BOUND_IMAGES_FIRSTBOOT_SERVICE_SERVICE_SUCCESSEXITSTATUS } else { '0 1 2 3' }
...
$script:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/ceph-bootstrap.sh' }
$script:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_EXECSTARTPOST = if ($env:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_EXECSTARTPOST) { $env:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_EXECSTARTPOST } else { '/usr/bin/touch /var/lib/ceph/.bootstrapped,-/bin/chmod 0600 /etc/ceph/ceph.client.admin.keyring' }
$script:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
$script:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_STANDARDERROR = if ($env:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_STANDARDERROR) { $env:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_STANDARDERROR } else { 'journal+console' }
$script:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_STANDARDOUTPUT = if ($env:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_STANDARDOUTPUT) { $env:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_SERVICE_STANDARDOUTPUT } else { 'journal+console' }
...
$script:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_UNIT_DOCUMENTATION = if ($env:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_UNIT_DOCUMENTATION) { $env:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_UNIT_DOCUMENTATION } else { 'https://docs.ceph.com/en/latest/cephadm/' }
$script:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_UNIT_STARTLIMITINTERVALSEC = if ($env:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_UNIT_STARTLIMITINTERVALSEC) { $env:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_UNIT_STARTLIMITINTERVALSEC } else { 0 }
$script:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_UNIT_WANTS = if ($env:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_UNIT_WANTS) { $env:MIOS_UNIT_MIOS_CEPH_BOOTSTRAP_SERVICE_UNIT_WANTS } else { 'network-online.target podman.socket' }
...
$script:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_LOCKPERSONALITY = if ($env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_LOCKPERSONALITY) { $env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_LOCKPERSONALITY } else { 'true' }
$script:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_NONEWPRIVILEGES = if ($env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_NONEWPRIVILEGES) { $env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_NONEWPRIVILEGES } else { 'true' }
$script:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_PRIVATETMP = if ($env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_PRIVATETMP) { $env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_PRIVATETMP } else { 'true' }
$script:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_PROTECTCONTROLGROUPS = if ($env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_PROTECTCONTROLGROUPS) { $env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_PROTECTCONTROLGROUPS } else { 'true' }
$script:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_PROTECTHOME = if ($env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_PROTECTHOME) { $env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_PROTECTHOME } else { 'true' }
$script:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_PROTECTKERNELTUNABLES = if ($env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_PROTECTKERNELTUNABLES) { $env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_PROTECTKERNELTUNABLES } else { 'true' }
$script:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_PROTECTSYSTEM = if ($env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_PROTECTSYSTEM) { $env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_PROTECTSYSTEM } else { 'strict' }
$script:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_READWRITEPATHS = if ($env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_READWRITEPATHS) { $env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_READWRITEPATHS } else { '/var/lib/mios/daemon /var/lib/mios/scratch' }
$script:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_RESTART) { $env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_RESTART } else { 'on-failure' }
$script:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_RESTARTSEC } else { '5s' }
$script:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_RESTRICTADDRESSFAMILIES = if ($env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_RESTRICTADDRESSFAMILIES) { $env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_RESTRICTADDRESSFAMILIES } else { 'AF_INET AF_INET6 AF_UNIX' }
$script:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_RESTRICTREALTIME = if ($env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_RESTRICTREALTIME) { $env:MIOS_UNIT_MIOS_DAEMON_SERVICE_SERVICE_RESTRICTREALTIME } else { 'true' }
...
$script:MIOS_UNIT_MIOS_DAEMON_SERVICE_UNIT_WANTS = if ($env:MIOS_UNIT_MIOS_DAEMON_SERVICE_UNIT_WANTS) { $env:MIOS_UNIT_MIOS_DAEMON_SERVICE_UNIT_WANTS } else { 'mios-llm-light.service' }
$script:MIOS_UNIT_MIOS_DASHBOARD_ISSUE_TIMER_INSTALL_WANTEDBY = if ($env:MIOS_UNIT_MIOS_DASHBOARD_ISSUE_TIMER_INSTALL_WANTEDBY) { $env:MIOS_UNIT_MIOS_DASHBOARD_ISSUE_TIMER_INSTALL_WANTEDBY } else { 'timers.target' }
$script:MIOS_UNIT_MIOS_DASHBOARD_ISSUE_TIMER_TIMER_ACCURACYSEC = if ($env:MIOS_UNIT_MIOS_DASHBOARD_ISSUE_TIMER_TIMER_ACCURACYSEC) { $env:MIOS_UNIT_MIOS_DASHBOARD_ISSUE_TIMER_TIMER_ACCURACYSEC } else { '30s' }
$script:MIOS_UNIT_MIOS_DASHBOARD_ISSUE_TIMER_TIMER_ONBOOTSEC = if ($env:MIOS_UNIT_MIOS_DASHBOARD_ISSUE_TIMER_TIMER_ONBOOTSEC) { $env:MIOS_UNIT_MIOS_DASHBOARD_ISSUE_TIMER_TIMER_ONBOOTSEC } else { '2min' }
...
$script:MIOS_UNIT_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_GROUP = if ($env:MIOS_UNIT_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_GROUP) { $env:MIOS_UNIT_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_GROUP } else { 'mios-ai' }
$script:MIOS_UNIT_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_NONEWPRIVILEGES = if ($env:MIOS_UNIT_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_NONEWPRIVILEGES) { $env:MIOS_UNIT_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_NONEWPRIVILEGES } else { 'true' }
$script:MIOS_UNIT_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_PRIVATETMP = if ($env:MIOS_UNIT_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_PRIVATETMP) { $env:MIOS_UNIT_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_PRIVATETMP } else { 'true' }
$script:MIOS_UNIT_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_PROTECTHOME = if ($env:MIOS_UNIT_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_PROTECTHOME) { $env:MIOS_UNIT_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_PROTECTHOME } else { 'true' }
$script:MIOS_UNIT_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_PROTECTSYSTEM = if ($env:MIOS_UNIT_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_PROTECTSYSTEM) { $env:MIOS_UNIT_MIOS_EMBED_BACKFILL_SERVICE_SERVICE_PROTECTSYSTEM } else { 'strict' }
...
$script:MIOS_UNIT_MIOS_EMBED_BACKFILL_SERVICE_UNIT_DOCUMENTATION = if ($env:MIOS_UNIT_MIOS_EMBED_BACKFILL_SERVICE_UNIT_DOCUMENTATION) { $env:MIOS_UNIT_MIOS_EMBED_BACKFILL_SERVICE_UNIT_DOCUMENTATION } else { 'file:///usr/lib/mios/agent-pipe/mios_pipe/memory/embed_backfill.py' }
$script:MIOS_UNIT_MIOS_EMBED_BACKFILL_SERVICE_UNIT_REQUIRES = if ($env:MIOS_UNIT_MIOS_EMBED_BACKFILL_SERVICE_UNIT_REQUIRES) { $env:MIOS_UNIT_MIOS_EMBED_BACKFILL_SERVICE_UNIT_REQUIRES } else { 'mios-pgvector.service' }
$script:MIOS_UNIT_MIOS_EMBED_BACKFILL_TIMER_INSTALL_WANTEDBY = if ($env:MIOS_UNIT_MIOS_EMBED_BACKFILL_TIMER_INSTALL_WANTEDBY) { $env:MIOS_UNIT_MIOS_EMBED_BACKFILL_TIMER_INSTALL_WANTEDBY } else { 'timers.target' }
$script:MIOS_UNIT_MIOS_EMBED_BACKFILL_TIMER_TIMER_ACCURACYSEC = if ($env:MIOS_UNIT_MIOS_EMBED_BACKFILL_TIMER_TIMER_ACCURACYSEC) { $env:MIOS_UNIT_MIOS_EMBED_BACKFILL_TIMER_TIMER_ACCURACYSEC } else { '1min' }
$script:MIOS_UNIT_MIOS_EMBED_BACKFILL_TIMER_TIMER_ONBOOTSEC = if ($env:MIOS_UNIT_MIOS_EMBED_BACKFILL_TIMER_TIMER_ONBOOTSEC) { $env:MIOS_UNIT_MIOS_EMBED_BACKFILL_TIMER_TIMER_ONBOOTSEC } else { '5min' }
...
$script:MIOS_UNIT_MIOS_FINETUNE_SERVE_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNIT_MIOS_FINETUNE_SERVE_SERVICE_SERVICE_RESTART) { $env:MIOS_UNIT_MIOS_FINETUNE_SERVE_SERVICE_SERVICE_RESTART } else { 'on-failure' }
$script:MIOS_UNIT_MIOS_FINETUNE_SERVE_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNIT_MIOS_FINETUNE_SERVE_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNIT_MIOS_FINETUNE_SERVE_SERVICE_SERVICE_RESTARTSEC } else { 5 }
$script:MIOS_UNIT_MIOS_FINETUNE_SERVE_SERVICE_SERVICE_TIMEOUTSTARTSEC = if ($env:MIOS_UNIT_MIOS_FINETUNE_SERVE_SERVICE_SERVICE_TIMEOUTSTARTSEC) { $env:MIOS_UNIT_MIOS_FINETUNE_SERVE_SERVICE_SERVICE_TIMEOUTSTARTSEC } else { 180 }
...
$script:MIOS_UNIT_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNIT_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNIT_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_EXECSTART } else { '/bin/bash -c ''\
    ...
    echo "MiOS firewalld ports: $(/usr/bin/firewall-cmd --list-ports 2>/dev/null)"; \
    exit 0\''' }
$script:MIOS_UNIT_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_PROTECTCONTROLGROUPS = if ($env:MIOS_UNIT_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_PROTECTCONTROLGROUPS) { $env:MIOS_UNIT_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_PROTECTCONTROLGROUPS } else { 'yes' }
$script:MIOS_UNIT_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_PROTECTKERNELTUNABLES = if ($env:MIOS_UNIT_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_PROTECTKERNELTUNABLES) { $env:MIOS_UNIT_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_PROTECTKERNELTUNABLES } else { 'yes' }
$script:MIOS_UNIT_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNIT_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNIT_MIOS_FIREWALL_PORTS_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
...
$script:MIOS_UNIT_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNIT_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNIT_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNIT_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNIT_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNIT_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/mios-forgejo-runner-firstboot.sh' }
$script:MIOS_UNIT_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNIT_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNIT_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
$script:MIOS_UNIT_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNIT_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_RESTART) { $env:MIOS_UNIT_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_RESTART } else { 'on-failure' }
$script:MIOS_UNIT_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNIT_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNIT_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_RESTARTSEC } else { 30 }
$script:MIOS_UNIT_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_TIMEOUTSTARTSEC = if ($env:MIOS_UNIT_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_TIMEOUTSTARTSEC) { $env:MIOS_UNIT_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_SERVICE_TIMEOUTSTARTSEC } else { '180s' }
...
$script:MIOS_UNIT_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_UNIT_STARTLIMITBURST = if ($env:MIOS_UNIT_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_UNIT_STARTLIMITBURST) { $env:MIOS_UNIT_MIOS_FORGEJO_RUNNER_FIRSTBOOT_SERVICE_UNIT_STARTLIMITBURST } else { 6 }
...
$script:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_NONEWPRIVILEGES = if ($env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_NONEWPRIVILEGES) { $env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_NONEWPRIVILEGES } else { 'yes' }
$script:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_PRIVATETMP = if ($env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_PRIVATETMP) { $env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_PRIVATETMP } else { 'yes' }
$script:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_PROTECTHOME = if ($env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_PROTECTHOME) { $env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_PROTECTHOME } else { 'yes' }
$script:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_PROTECTSYSTEM = if ($env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_PROTECTSYSTEM) { $env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_PROTECTSYSTEM } else { 'strict' }
$script:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_READWRITEPATHS = if ($env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_READWRITEPATHS) { $env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_READWRITEPATHS } else { '/etc/mios/forge /var/lib/mios/forge /var/log/mios/forge /var/lib/containers /run' }
$script:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
$script:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_RESTART) { $env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_RESTART } else { 'on-failure' }
$script:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_RESTARTSEC } else { 30 }
$script:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_RESTRICTREALTIME = if ($env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_RESTRICTREALTIME) { $env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_SERVICE_RESTRICTREALTIME } else { 'yes' }
...
# plain `!container` would skip WSL because systemd reports virtualization
# as `wsl` -- a container variant. Use OR: "not a container OR is WSL".' }
$script:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_CONDITIONPATHEXISTS = if ($env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_CONDITIONPATHEXISTS) { $env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_CONDITIONPATHEXISTS } else { '!/var/lib/mios/forge/.firstboot-done' }
$script:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_CONDITIONVIRTUALIZATION = if ($env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_CONDITIONVIRTUALIZATION) { $env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_CONDITIONVIRTUALIZATION } else { '|!container,|wsl' }
$script:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_DESCRIPTION = if ($env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_DESCRIPTION) { $env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_DESCRIPTION } else { '''MiOS'' Forge first-boot admin-bootstrap (Forgejo)' }
$script:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_DOCUMENTATION = if ($env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_DOCUMENTATION) { $env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_DOCUMENTATION } else { 'https://github.com/mios-dev/MiOS/blob/main/etc/containers/systemd/mios-forge.container' }
$script:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_STARTLIMITBURST = if ($env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_STARTLIMITBURST) { $env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_STARTLIMITBURST } else { 6 }
$script:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_STARTLIMITINTERVALSEC = if ($env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_STARTLIMITINTERVALSEC) { $env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_STARTLIMITINTERVALSEC } else { 1200 }
$script:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_WANTS = if ($env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_WANTS) { $env:MIOS_UNIT_MIOS_FORGE_FIRSTBOOT_SERVICE_UNIT_WANTS } else { 'mios-forge.service network-online.target' }
$script:MIOS_UNIT_MIOS_GPU_AMD_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNIT_MIOS_GPU_AMD_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNIT_MIOS_GPU_AMD_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNIT_MIOS_GPU_AMD_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNIT_MIOS_GPU_AMD_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNIT_MIOS_GPU_AMD_SERVICE_SERVICE_EXECSTART } else { '/usr/bin/bash -c '' \
  # If amd-ctk ever lands in Fedora RPM repos, drop a CDI spec automatically. \
  # Until then containers must pass --device /dev/kfd --device /dev/dri \
  # --group-add keep-groups explicitly. \
  if command -v amd-ctk >/dev/null 2>&1; then \
  ...
  chmod 0660   /dev/kfd 2>/dev/null || true; \
  exit 0\''' }
$script:MIOS_UNIT_MIOS_GPU_AMD_SERVICE_SERVICE_EXECSTARTPRE = if ($env:MIOS_UNIT_MIOS_GPU_AMD_SERVICE_SERVICE_EXECSTARTPRE) { $env:MIOS_UNIT_MIOS_GPU_AMD_SERVICE_SERVICE_EXECSTARTPRE } else { '/usr/sbin/modprobe -a amdgpu' }
$script:MIOS_UNIT_MIOS_GPU_AMD_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNIT_MIOS_GPU_AMD_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNIT_MIOS_GPU_AMD_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
$script:MIOS_UNIT_MIOS_GPU_AMD_SERVICE_SERVICE_STANDARDERROR = if ($env:MIOS_UNIT_MIOS_GPU_AMD_SERVICE_SERVICE_STANDARDERROR) { $env:MIOS_UNIT_MIOS_GPU_AMD_SERVICE_SERVICE_STANDARDERROR } else { 'journal' }
...
$script:MIOS_UNIT_MIOS_GPU_AMD_SERVICE_UNIT_DOCUMENTATION = if ($env:MIOS_UNIT_MIOS_GPU_AMD_SERVICE_UNIT_DOCUMENTATION) { $env:MIOS_UNIT_MIOS_GPU_AMD_SERVICE_UNIT_DOCUMENTATION } else { 'https://github.com/MiOS-DEV/MiOS/blob/main/docs/gpu-passthrough.md' }
$script:MIOS_UNIT_MIOS_GPU_DETECT_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNIT_MIOS_GPU_DETECT_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNIT_MIOS_GPU_DETECT_SERVICE_INSTALL_WANTEDBY } else { 'sysinit.target' }
$script:MIOS_UNIT_MIOS_GPU_DETECT_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNIT_MIOS_GPU_DETECT_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNIT_MIOS_GPU_DETECT_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/gpu-detect' }
$script:MIOS_UNIT_MIOS_GPU_DETECT_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNIT_MIOS_GPU_DETECT_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNIT_MIOS_GPU_DETECT_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
...
$script:MIOS_UNIT_MIOS_GPU_DETECT_SERVICE_UNIT_DESCRIPTION = if ($env:MIOS_UNIT_MIOS_GPU_DETECT_SERVICE_UNIT_DESCRIPTION) { $env:MIOS_UNIT_MIOS_GPU_DETECT_SERVICE_UNIT_DESCRIPTION } else { '''MiOS'' GPU Environment Detection' }
$script:MIOS_UNIT_MIOS_GPU_INTEL_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNIT_MIOS_GPU_INTEL_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNIT_MIOS_GPU_INTEL_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNIT_MIOS_GPU_INTEL_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNIT_MIOS_GPU_INTEL_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNIT_MIOS_GPU_INTEL_SERVICE_SERVICE_EXECSTART } else { '/usr/bin/bash -c '' \
  # Defensive perms on primary render node -- do not touch card* nodes (video group \
  # handled by 99-mios-gpu.rules at udev-add time). \
  if [ -e /dev/dri/renderD128 ]; then \
    chgrp render /dev/dri/renderD128 2>/dev/null || true; \
  ...
  exit 0\''' }
$script:MIOS_UNIT_MIOS_GPU_INTEL_SERVICE_SERVICE_EXECSTARTPRE = if ($env:MIOS_UNIT_MIOS_GPU_INTEL_SERVICE_SERVICE_EXECSTARTPRE) { $env:MIOS_UNIT_MIOS_GPU_INTEL_SERVICE_SERVICE_EXECSTARTPRE } else { '-/usr/sbin/modprobe -a i915 xe amdgpu' }
$script:MIOS_UNIT_MIOS_GPU_INTEL_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNIT_MIOS_GPU_INTEL_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNIT_MIOS_GPU_INTEL_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
$script:MIOS_UNIT_MIOS_GPU_INTEL_SERVICE_SERVICE_STANDARDERROR = if ($env:MIOS_UNIT_MIOS_GPU_INTEL_SERVICE_SERVICE_STANDARDERROR) { $env:MIOS_UNIT_MIOS_GPU_INTEL_SERVICE_SERVICE_STANDARDERROR } else { 'journal' }
...
$script:MIOS_UNIT_MIOS_GPU_NVIDIA_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNIT_MIOS_GPU_NVIDIA_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNIT_MIOS_GPU_NVIDIA_SERVICE_SERVICE_EXECSTART } else { '/usr/bin/bash -c '' \
  if systemctl list-unit-files nvidia-cdi-refresh.service 2>/dev/null \
       | grep -q nvidia-cdi-refresh; then \
    echo "nvidia-cdi-refresh.service present; deferring CDI generation to upstream"; \
    exit 0; \
  ...
  exit 0\''' }
$script:MIOS_UNIT_MIOS_GPU_NVIDIA_SERVICE_SERVICE_EXECSTARTPRE = if ($env:MIOS_UNIT_MIOS_GPU_NVIDIA_SERVICE_SERVICE_EXECSTARTPRE) { $env:MIOS_UNIT_MIOS_GPU_NVIDIA_SERVICE_SERVICE_EXECSTARTPRE } else { '/usr/sbin/modprobe -a nvidia nvidia_uvm nvidia_modeset nvidia_drm' }
$script:MIOS_UNIT_MIOS_GPU_NVIDIA_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNIT_MIOS_GPU_NVIDIA_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNIT_MIOS_GPU_NVIDIA_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
$script:MIOS_UNIT_MIOS_GPU_NVIDIA_SERVICE_SERVICE_STANDARDERROR = if ($env:MIOS_UNIT_MIOS_GPU_NVIDIA_SERVICE_SERVICE_STANDARDERROR) { $env:MIOS_UNIT_MIOS_GPU_NVIDIA_SERVICE_SERVICE_STANDARDERROR } else { 'journal' }
$script:MIOS_UNIT_MIOS_GPU_NVIDIA_SERVICE_SERVICE_STANDARDOUTPUT = if ($env:MIOS_UNIT_MIOS_GPU_NVIDIA_SERVICE_SERVICE_STANDARDOUTPUT) { $env:MIOS_UNIT_MIOS_GPU_NVIDIA_SERVICE_SERVICE_STANDARDOUTPUT } else { 'journal' }
...
$script:MIOS_UNIT_MIOS_GPU_NVIDIA_SERVICE_UNIT_DOCUMENTATION = if ($env:MIOS_UNIT_MIOS_GPU_NVIDIA_SERVICE_UNIT_DOCUMENTATION) { $env:MIOS_UNIT_MIOS_GPU_NVIDIA_SERVICE_UNIT_DOCUMENTATION } else { 'https://github.com/MiOS-DEV/MiOS/blob/main/docs/gpu-passthrough.md' }
$script:MIOS_UNIT_MIOS_GPU_PV_DETECT_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNIT_MIOS_GPU_PV_DETECT_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNIT_MIOS_GPU_PV_DETECT_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNIT_MIOS_GPU_PV_DETECT_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNIT_MIOS_GPU_PV_DETECT_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNIT_MIOS_GPU_PV_DETECT_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/gpu-pv-detect' }
$script:MIOS_UNIT_MIOS_GPU_PV_DETECT_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNIT_MIOS_GPU_PV_DETECT_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNIT_MIOS_GPU_PV_DETECT_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
...
$script:MIOS_UNIT_MIOS_GPU_STATUS_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNIT_MIOS_GPU_STATUS_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNIT_MIOS_GPU_STATUS_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNIT_MIOS_GPU_STATUS_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNIT_MIOS_GPU_STATUS_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNIT_MIOS_GPU_STATUS_SERVICE_SERVICE_EXECSTART } else { '/usr/bin/bash -c '' \
  set +e; \
  HAS_NV=0; HAS_AMD=0; HAS_INTEL=0; \
  [ -e /dev/nvidia0 ]            && HAS_NV=1; \
  [ -e /dev/kfd ]                && HAS_AMD=1; \
  ...
  exit 0\''' }
$script:MIOS_UNIT_MIOS_GPU_STATUS_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNIT_MIOS_GPU_STATUS_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNIT_MIOS_GPU_STATUS_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
$script:MIOS_UNIT_MIOS_GPU_STATUS_SERVICE_SERVICE_RUNTIMEDIRECTORY = if ($env:MIOS_UNIT_MIOS_GPU_STATUS_SERVICE_SERVICE_RUNTIMEDIRECTORY) { $env:MIOS_UNIT_MIOS_GPU_STATUS_SERVICE_SERVICE_RUNTIMEDIRECTORY } else { 'mios' }
$script:MIOS_UNIT_MIOS_GPU_STATUS_SERVICE_SERVICE_RUNTIMEDIRECTORYMODE = if ($env:MIOS_UNIT_MIOS_GPU_STATUS_SERVICE_SERVICE_RUNTIMEDIRECTORYMODE) { $env:MIOS_UNIT_MIOS_GPU_STATUS_SERVICE_SERVICE_RUNTIMEDIRECTORYMODE } else { 0755 }
$script:MIOS_UNIT_MIOS_GPU_STATUS_SERVICE_SERVICE_STANDARDERROR = if ($env:MIOS_UNIT_MIOS_GPU_STATUS_SERVICE_SERVICE_STANDARDERROR) { $env:MIOS_UNIT_MIOS_GPU_STATUS_SERVICE_SERVICE_STANDARDERROR } else { 'journal' }
...
$script:MIOS_UNIT_MIOS_GPU_STATUS_SERVICE_UNIT_DOCUMENTATION = if ($env:MIOS_UNIT_MIOS_GPU_STATUS_SERVICE_UNIT_DOCUMENTATION) { $env:MIOS_UNIT_MIOS_GPU_STATUS_SERVICE_UNIT_DOCUMENTATION } else { 'https://github.com/MiOS-DEV/MiOS/blob/main/docs/gpu-passthrough.md' }
...
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_NONEWPRIVILEGES = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_NONEWPRIVILEGES) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_NONEWPRIVILEGES } else { 'true' }
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_PRIVATETMP = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_PRIVATETMP) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_PRIVATETMP } else { 'false' }
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RESTART) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RESTART } else { 'on-failure' }
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RESTARTSEC } else { '5s' }
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RUNTIMEDIRECTORY = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RUNTIMEDIRECTORY) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RUNTIMEDIRECTORY } else { 'mios-hermes-browser' }
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RUNTIMEDIRECTORYMODE = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RUNTIMEDIRECTORYMODE) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_RUNTIMEDIRECTORYMODE } else { 0700 }
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_STATEDIRECTORY = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_STATEDIRECTORY) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_STATEDIRECTORY } else { 'mios/hermes-browser/profile' }
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_STATEDIRECTORYMODE = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_STATEDIRECTORYMODE) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_STATEDIRECTORYMODE } else { 0700 }
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_TIMEOUTSTOPSEC = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_TIMEOUTSTOPSEC) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_SERVICE_TIMEOUTSTOPSEC } else { '30s' }
...
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_UNIT_DESCRIPTION = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_UNIT_DESCRIPTION) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_UNIT_DESCRIPTION } else { '''MiOS'' Hermes-Browser (ChromeDev w/ CDP for Hermes-Agent)' }
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_UNIT_DOCUMENTATION = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_UNIT_DOCUMENTATION) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_UNIT_DOCUMENTATION } else { 'https://chromedevtools.github.io/devtools-protocol/' }
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_UNIT_WANTS = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_UNIT_WANTS) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_SERVICE_UNIT_WANTS } else { 'mios-agent-pipe.service' }
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_ENVIRONMENT = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_ENVIRONMENT) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_ENVIRONMENT } else { 'HOME=/var/lib/mios/hermes-worker,XDG_RUNTIME_DIR=/run/mios-hermes-browser-worker,HERMES_BROWSER_HEADLESS=1,HERMES_BROWSER_CDP_PORT=${MIOS_PORT_CHROME_CDP_WORKER:-9223},HERMES_BROWSER_PROFILE_DIR=/var/lib/mios/hermes-browser/profile-w2,HERMES_BROWSER_LOG=/var/lib/mios/hermes-browser/launch-w2.log,NO_AT_BRIDGE=1' }
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/mios-hermes-browser start' }
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_GROUP = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_GROUP) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_GROUP } else { 'mios-ai' }
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_NONEWPRIVILEGES = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_NONEWPRIVILEGES) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_NONEWPRIVILEGES } else { 'true' }
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_PRIVATETMP = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_PRIVATETMP) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_PRIVATETMP } else { 'false' }
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RESTART) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RESTART } else { 'on-failure' }
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RESTARTSEC } else { '5s' }
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RUNTIMEDIRECTORY = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RUNTIMEDIRECTORY) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RUNTIMEDIRECTORY } else { 'mios-hermes-browser-worker' }
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RUNTIMEDIRECTORYMODE = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RUNTIMEDIRECTORYMODE) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_RUNTIMEDIRECTORYMODE } else { 0700 }
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_STATEDIRECTORY = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_STATEDIRECTORY) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_STATEDIRECTORY } else { 'mios/hermes-browser/profile-w2' }
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_STATEDIRECTORYMODE = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_STATEDIRECTORYMODE) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_STATEDIRECTORYMODE } else { 0700 }
$script:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_TIMEOUTSTOPSEC = if ($env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_TIMEOUTSTOPSEC) { $env:MIOS_UNIT_MIOS_HERMES_BROWSER_WORKER_SERVICE_SERVICE_TIMEOUTSTOPSEC } else { '30s' }
...
$script:MIOS_UNIT_MIOS_HYBRID_TARGET_UNIT_WANTS = if ($env:MIOS_UNIT_MIOS_HYBRID_TARGET_UNIT_WANTS) { $env:MIOS_UNIT_MIOS_HYBRID_TARGET_UNIT_WANTS } else { 'k3s-agent.service' }
...
# director, hermes-browser) -- consolidation pass 2026-05-15.' }
$script:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_COMMENT4 = if ($env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_COMMENT4) { $env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_COMMENT4 } else { '# Drain timeout: MCP serves a long-lived TCP socket but in-flight
# context requests are sub-second; 30 s is generous.' }
$script:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/mcp-server-runner' }
$script:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_EXECSTARTPRE = if ($env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_EXECSTARTPRE) { $env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_EXECSTARTPRE } else { '/usr/libexec/mios/mcp-init.sh' }
$script:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_GROUP = if ($env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_GROUP) { $env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_GROUP } else { 'mios-ai' }
$script:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_NONEWPRIVILEGES = if ($env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_NONEWPRIVILEGES) { $env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_NONEWPRIVILEGES } else { 'true' }
$script:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_PRIVATETMP = if ($env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_PRIVATETMP) { $env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_PRIVATETMP } else { 'true' }
$script:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_PROTECTHOME = if ($env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_PROTECTHOME) { $env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_PROTECTHOME } else { 'read-only' }
$script:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_PROTECTSYSTEM = if ($env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_PROTECTSYSTEM) { $env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_PROTECTSYSTEM } else { 'strict' }
$script:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_READWRITEPATHS = if ($env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_READWRITEPATHS) { $env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_READWRITEPATHS } else { '/var/lib/mios/mcp /var/log/mios/mcp' }
$script:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_RESTART) { $env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_RESTART } else { 'always' }
$script:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_RESTARTSEC } else { 5 }
$script:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_TIMEOUTSTOPSEC = if ($env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_TIMEOUTSTOPSEC) { $env:MIOS_UNIT_MIOS_MCP_SERVICE_SERVICE_TIMEOUTSTOPSEC } else { '30s' }
...
$script:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_ENVIRONMENT = if ($env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_ENVIRONMENT) { $env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_ENVIRONMENT } else { 'HOME=/var/lib/mios/opencode-gateway/work,MIOS_PORT_OPENCODE_GATEWAY=${MIOS_PORT_OPENCODE_GATEWAY:-8780},MIOS_OPENCODE_HOST=127.0.0.1,MIOS_OPENCODE_BIN=/usr/lib/mios/agents/opencode/bin/opencode,MIOS_OPENCODE_MODEL=mios-opencode:latest,MIOS_OPENCODE_PROVIDER=local,MIOS_OPENCODE_CONFIG=/etc/mios/opencode/opencode.json,MIOS_OPENCODE_TIMEOUT_S=90' }
$script:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_EXECSTART } else { '/usr/lib/mios/agents/.venv/bin/python3 -u /usr/lib/mios/agents/opencode-gateway/server.py' }
$script:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_GROUP = if ($env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_GROUP) { $env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_GROUP } else { 'mios-ai' }
$script:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_LOCKPERSONALITY = if ($env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_LOCKPERSONALITY) { $env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_LOCKPERSONALITY } else { 'true' }
$script:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_NONEWPRIVILEGES = if ($env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_NONEWPRIVILEGES) { $env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_NONEWPRIVILEGES } else { 'true' }
$script:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PRIVATETMP = if ($env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PRIVATETMP) { $env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PRIVATETMP } else { 'true' }
$script:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTCONTROLGROUPS = if ($env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTCONTROLGROUPS) { $env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTCONTROLGROUPS } else { 'true' }
$script:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTHOME = if ($env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTHOME) { $env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTHOME } else { 'true' }
$script:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTKERNELMODULES = if ($env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTKERNELMODULES) { $env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTKERNELMODULES } else { 'true' }
$script:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTKERNELTUNABLES = if ($env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTKERNELTUNABLES) { $env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTKERNELTUNABLES } else { 'true' }
$script:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTSYSTEM = if ($env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTSYSTEM) { $env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_PROTECTSYSTEM } else { 'strict' }
$script:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_READWRITEPATHS = if ($env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_READWRITEPATHS) { $env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_READWRITEPATHS } else { '/etc/mios /var/lib/mios' }
$script:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_RESTART) { $env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_RESTART } else { 'on-failure' }
$script:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_RESTARTSEC } else { '10s' }
$script:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_RESTRICTSUIDSGID = if ($env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_RESTRICTSUIDSGID) { $env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_RESTRICTSUIDSGID } else { 'true' }
$script:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_TIMEOUTSTARTSEC = if ($env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_TIMEOUTSTARTSEC) { $env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_TIMEOUTSTARTSEC } else { '30s' }
$script:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_TYPE = if ($env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_TYPE) { $env:MIOS_UNIT_MIOS_OPENCODE_GATEWAY_SERVICE_SERVICE_TYPE } else { 'simple' }
...
$script:MIOS_UNIT_MIOS_PGVECTOR_BACKUP_SERVICE_SERVICE_PRIVATETMP = if ($env:MIOS_UNIT_MIOS_PGVECTOR_BACKUP_SERVICE_SERVICE_PRIVATETMP) { $env:MIOS_UNIT_MIOS_PGVECTOR_BACKUP_SERVICE_SERVICE_PRIVATETMP } else { 'true' }
...
$script:MIOS_UNIT_MIOS_PGVECTOR_BACKUP_SERVICE_UNIT_DESCRIPTION = if ($env:MIOS_UNIT_MIOS_PGVECTOR_BACKUP_SERVICE_UNIT_DESCRIPTION) { $env:MIOS_UNIT_MIOS_PGVECTOR_BACKUP_SERVICE_UNIT_DESCRIPTION } else { '''MiOS'' daily pg_dump backup of the unified agent-plane datastore (pgvector)' }
$script:MIOS_UNIT_MIOS_PGVECTOR_BACKUP_SERVICE_UNIT_DOCUMENTATION = if ($env:MIOS_UNIT_MIOS_PGVECTOR_BACKUP_SERVICE_UNIT_DOCUMENTATION) { $env:MIOS_UNIT_MIOS_PGVECTOR_BACKUP_SERVICE_UNIT_DOCUMENTATION } else { 'file:///usr/lib/systemd/system/mios-pgvector-backup.service' }
...
$script:MIOS_UNIT_MIOS_POLICY_ARBITER_SERVICE_SERVICE_GROUP = if ($env:MIOS_UNIT_MIOS_POLICY_ARBITER_SERVICE_SERVICE_GROUP) { $env:MIOS_UNIT_MIOS_POLICY_ARBITER_SERVICE_SERVICE_GROUP } else { 'mios-ai' }
$script:MIOS_UNIT_MIOS_POLICY_ARBITER_SERVICE_SERVICE_NONEWPRIVILEGES = if ($env:MIOS_UNIT_MIOS_POLICY_ARBITER_SERVICE_SERVICE_NONEWPRIVILEGES) { $env:MIOS_UNIT_MIOS_POLICY_ARBITER_SERVICE_SERVICE_NONEWPRIVILEGES } else { 'true' }
$script:MIOS_UNIT_MIOS_POLICY_ARBITER_SERVICE_SERVICE_PRIVATETMP = if ($env:MIOS_UNIT_MIOS_POLICY_ARBITER_SERVICE_SERVICE_PRIVATETMP) { $env:MIOS_UNIT_MIOS_POLICY_ARBITER_SERVICE_SERVICE_PRIVATETMP } else { 'true' }
$script:MIOS_UNIT_MIOS_POLICY_ARBITER_SERVICE_SERVICE_PROTECTHOME = if ($env:MIOS_UNIT_MIOS_POLICY_ARBITER_SERVICE_SERVICE_PROTECTHOME) { $env:MIOS_UNIT_MIOS_POLICY_ARBITER_SERVICE_SERVICE_PROTECTHOME } else { 'true' }
$script:MIOS_UNIT_MIOS_POLICY_ARBITER_SERVICE_SERVICE_PROTECTSYSTEM = if ($env:MIOS_UNIT_MIOS_POLICY_ARBITER_SERVICE_SERVICE_PROTECTSYSTEM) { $env:MIOS_UNIT_MIOS_POLICY_ARBITER_SERVICE_SERVICE_PROTECTSYSTEM } else { 'strict' }
$script:MIOS_UNIT_MIOS_POLICY_ARBITER_SERVICE_SERVICE_RESTART = if ($env:MIOS_UNIT_MIOS_POLICY_ARBITER_SERVICE_SERVICE_RESTART) { $env:MIOS_UNIT_MIOS_POLICY_ARBITER_SERVICE_SERVICE_RESTART } else { 'on-failure' }
$script:MIOS_UNIT_MIOS_POLICY_ARBITER_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNIT_MIOS_POLICY_ARBITER_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNIT_MIOS_POLICY_ARBITER_SERVICE_SERVICE_RESTARTSEC } else { '15s' }
...
$script:MIOS_UNIT_MIOS_SHELL_SESSION_GC_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNIT_MIOS_SHELL_SESSION_GC_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNIT_MIOS_SHELL_SESSION_GC_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNIT_MIOS_SHELL_SESSION_GC_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNIT_MIOS_SHELL_SESSION_GC_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNIT_MIOS_SHELL_SESSION_GC_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/mios-shell-session gc' }
$script:MIOS_UNIT_MIOS_SHELL_SESSION_GC_SERVICE_SERVICE_GROUP = if ($env:MIOS_UNIT_MIOS_SHELL_SESSION_GC_SERVICE_SERVICE_GROUP) { $env:MIOS_UNIT_MIOS_SHELL_SESSION_GC_SERVICE_SERVICE_GROUP } else { 'mios-ai' }
...
$script:MIOS_UNIT_MIOS_SKILLS_MINER_TIMER_TIMER_ONBOOTSEC = if ($env:MIOS_UNIT_MIOS_SKILLS_MINER_TIMER_TIMER_ONBOOTSEC) { $env:MIOS_UNIT_MIOS_SKILLS_MINER_TIMER_TIMER_ONBOOTSEC } else { '10min' }
...
$script:MIOS_UNIT_MIOS_USERDB_RENDER_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNIT_MIOS_USERDB_RENDER_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNIT_MIOS_USERDB_RENDER_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/mios-userdb-render' }
$script:MIOS_UNIT_MIOS_USERDB_RENDER_SERVICE_SERVICE_EXECSTARTPRE = if ($env:MIOS_UNIT_MIOS_USERDB_RENDER_SERVICE_SERVICE_EXECSTARTPRE) { $env:MIOS_UNIT_MIOS_USERDB_RENDER_SERVICE_SERVICE_EXECSTARTPRE } else { '-/usr/bin/podman exec mios-pgvector pg_isready -q -h 127.0.0.1 -p ${MIOS_PORT_PGVECTOR:-8600} -U mios -d mios' }
...
$script:MIOS_UNIT_MIOS_WEBTOOLS_FIRSTBOOT_SERVICE_SERVICE_RESTARTSEC = if ($env:MIOS_UNIT_MIOS_WEBTOOLS_FIRSTBOOT_SERVICE_SERVICE_RESTARTSEC) { $env:MIOS_UNIT_MIOS_WEBTOOLS_FIRSTBOOT_SERVICE_SERVICE_RESTARTSEC } else { 30 }
...
$script:MIOS_UNIT_MIOS_WEBTOOLS_FIRSTBOOT_SERVICE_UNIT_WANTS = if ($env:MIOS_UNIT_MIOS_WEBTOOLS_FIRSTBOOT_SERVICE_UNIT_WANTS) { $env:MIOS_UNIT_MIOS_WEBTOOLS_FIRSTBOOT_SERVICE_UNIT_WANTS } else { 'network-online.target' }
$script:MIOS_UNIT_MIOS_WSL_FIRSTBOOT_SERVICE_INSTALL_WANTEDBY = if ($env:MIOS_UNIT_MIOS_WSL_FIRSTBOOT_SERVICE_INSTALL_WANTEDBY) { $env:MIOS_UNIT_MIOS_WSL_FIRSTBOOT_SERVICE_INSTALL_WANTEDBY } else { 'multi-user.target' }
$script:MIOS_UNIT_MIOS_WSL_FIRSTBOOT_SERVICE_SERVICE_EXECSTART = if ($env:MIOS_UNIT_MIOS_WSL_FIRSTBOOT_SERVICE_SERVICE_EXECSTART) { $env:MIOS_UNIT_MIOS_WSL_FIRSTBOOT_SERVICE_SERVICE_EXECSTART } else { '/usr/libexec/mios/wsl-firstboot' }
$script:MIOS_UNIT_MIOS_WSL_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT = if ($env:MIOS_UNIT_MIOS_WSL_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT) { $env:MIOS_UNIT_MIOS_WSL_FIRSTBOOT_SERVICE_SERVICE_REMAINAFTEREXIT } else { 'yes' }
...
$script:MIOS_UNIT_VAR_HOME_MOUNT_MOUNT_OPTIONS = if ($env:MIOS_UNIT_VAR_HOME_MOUNT_MOUNT_OPTIONS) { $env:MIOS_UNIT_VAR_HOME_MOUNT_MOUNT_OPTIONS } else { 'secretfile=/etc/ceph/mios.secret,noatime,_netdev,nofail,x-systemd.device-timeout=30,x-systemd.mount-timeout=30' }
$script:MIOS_UNIT_VAR_HOME_MOUNT_MOUNT_TYPE = if ($env:MIOS_UNIT_VAR_HOME_MOUNT_MOUNT_TYPE) { $env:MIOS_UNIT_VAR_HOME_MOUNT_MOUNT_TYPE } else { 'ceph' }
...
$script:MIOS_UNIT_VAR_HOME_MOUNT_UNIT_WANTS = if ($env:MIOS_UNIT_VAR_HOME_MOUNT_UNIT_WANTS) { $env:MIOS_UNIT_VAR_HOME_MOUNT_UNIT_WANTS } else { 'network-online.target' }
$script:MIOS_UNIT_VAR_LIB_CONTAINERS_MOUNT_INSTALL_WANTEDBY = if ($env:MIOS_UNIT_VAR_LIB_CONTAINERS_MOUNT_INSTALL_WANTEDBY) { $env:MIOS_UNIT_VAR_LIB_CONTAINERS_MOUNT_INSTALL_WANTEDBY } else { 'remote-fs.target' }
$script:MIOS_UNIT_VAR_LIB_CONTAINERS_MOUNT_MOUNT_OPTIONS = if ($env:MIOS_UNIT_VAR_LIB_CONTAINERS_MOUNT_MOUNT_OPTIONS) { $env:MIOS_UNIT_VAR_LIB_CONTAINERS_MOUNT_MOUNT_OPTIONS } else { 'secretfile=/etc/ceph/mios.secret,noatime,_netdev,nofail,x-systemd.device-timeout=30,x-systemd.mount-timeout=30' }
$script:MIOS_UNIT_VAR_LIB_CONTAINERS_MOUNT_MOUNT_TYPE = if ($env:MIOS_UNIT_VAR_LIB_CONTAINERS_MOUNT_MOUNT_TYPE) { $env:MIOS_UNIT_VAR_LIB_CONTAINERS_MOUNT_MOUNT_TYPE } else { 'ceph' }
...
$script:MIOS_UNIT_VAR_LIB_CONTAINERS_MOUNT_UNIT_WANTS = if ($env:MIOS_UNIT_VAR_LIB_CONTAINERS_MOUNT_UNIT_WANTS) { $env:MIOS_UNIT_VAR_LIB_CONTAINERS_MOUNT_UNIT_WANTS } else { 'network-online.target' }
$script:MIOS_UNIT_VAR_LIB_MACHINES_MOUNT_MOUNT_OPTIONS = if ($env:MIOS_UNIT_VAR_LIB_MACHINES_MOUNT_MOUNT_OPTIONS) { $env:MIOS_UNIT_VAR_LIB_MACHINES_MOUNT_MOUNT_OPTIONS } else { 'loop' }
...
$script:MIOS_URLS_NON_ADDRESSABLE = if ($env:MIOS_URLS_NON_ADDRESSABLE) { $env:MIOS_URLS_NON_ADDRESSABLE } else { 'adguard_dns,adguard_ui,agent_pipe,arbiter,ceph_dashboard,crawl4ai,hermes,llm_light,pgvector,chrome_cdp,chrome_cdp_worker,cockpit_link,cpu_node,daemon_agent,firecrawl,forge_ssh,guacamole_web,guacd,hermes_dashboard,k3s_api,mcp,model_router,opencode_gateway,oscontrol,otelcol_otlp,prefilter,pxe_hub_api,rdp,redis,sglang,ssh,ttyd_bash,ttyd_powershell,vllm' }
