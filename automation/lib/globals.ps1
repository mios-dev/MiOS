...
$script:MIOS_BLADE_REQUIRES_MIOS_LLM_HEAVY_ALT = if ($env:MIOS_BLADE_REQUIRES_MIOS_LLM_HEAVY_ALT) { $env:MIOS_BLADE_REQUIRES_MIOS_LLM_HEAVY_ALT } else { 'gpu-serving' }
$script:MIOS_BLADE_REQUIRES_MIOS_LLM_WORKER_ = if ($env:MIOS_BLADE_REQUIRES_MIOS_LLM_WORKER_) { $env:MIOS_BLADE_REQUIRES_MIOS_LLM_WORKER_ } else { 'gpu-serving' }
$script:MIOS_BLADE_TYPE = if ($env:MIOS_BLADE_TYPE) { $env:MIOS_BLADE_TYPE } else { 'hybrid' }
$script:MIOS_BLADE_UNGATED = if ($env:MIOS_BLADE_UNGATED) { $env:MIOS_BLADE_UNGATED } else { 'mios-adguard,mios-ceph,mios-cockpit-link,mios-code-server,mios-cpu-node,mios-forge,mios-forgejo-runner,mios-guacamole,mios-guacd,mios-k3s,mios-llm-light,mios-open-webui,mios-otelcol,mios-pgvector,mios-pxe-hub,mios-searxng,mios-webtools-crawl4ai,mios-webtools-firecrawl-api,mios-webtools-firecrawl-worker,mios-webtools-redis' }
