...
: "${MIOS_BLADE_PLANES_HA_ROLE:=Pacemaker/Corosync: places and live-migrates VMs (ADR-0017 D1)}"
...
: "${MIOS_BLADE_PLANES_ORCHESTRATOR_ROLE:=k3s: places containers across the fleet (ADR-0017 D1)}"
...
: "${MIOS_BLADE_PLANES_RADIO_ROLE:=serves WiFi clients as an access point}"
: "${MIOS_BLADE_PLANES_ROUTER_MARKERS:=firewalld,dnsmasq}"
: "${MIOS_BLADE_PLANES_ROUTER_OWNER:=mini}"
: "${MIOS_BLADE_PLANES_ROUTER_ROLE:=is the uplink: forwards, NATs and filters for the clients it serves}"
...
: "${MIOS_BLADE_PLANES_STORAGE_ROLE:=CephFS: the shared filesystem a shed workload keeps its state on}"
