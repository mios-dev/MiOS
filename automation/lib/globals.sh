...
: "${MIOS_CI_EXEMPT_TESTS_BAKE_SMOKE_SH:=takes an image reference as its argument; the build and smoke jobs invoke it against the image they just produced}"
: "${MIOS_CI_EXEMPT_TESTS_DRIFT_GATE_READONLY_SH:=a developer convenience wrapper over the gate tier; running it would duplicate that tier}"
: "${MIOS_CI_EXEMPT_TESTS_DRIFT_PARITY_SH:=compares the bash gate against miosd drift-check --parity and needs the miosd binary built}"
: "${MIOS_CI_EXEMPT_TESTS_TEST_DIRECTORY_DISPATCH_SH:=dispatch returns no hits without a populated directory index, and the suite indexes hits[0] unguarded -- AGY-1715}"
: "${MIOS_CI_EXEMPT_TESTS_TEST_EXPAND_FROM_PY:=the open-url-fallback-chain skill resolves to not_found, so the suite sees 0 steps where it expects 3 -- AGY-1716}"
: "${MIOS_CI_EXEMPT_TESTS_TEST_REFLECTION_PY:=needs a reachable refine backend; every connection attempt fails and reflection returns None -- AGY-1717}"
: "${MIOS_CI_EXEMPT_TESTS_TEST_THINK_STRIPPER_PY:=the stripper leaves the separator behind, so removing a think block yields a doubled space or blank line -- AGY-1718}"
: "${MIOS_CI_GLOBS_AGENT_PIPE_DIR:=usr/lib/mios/agent-pipe}"
: "${MIOS_CI_GLOBS_AGENT_PIPE_GLOB:=test_mios_*.py}"
: "${MIOS_CI_GLOBS_AGENT_PIPE_SKIP:=test_mios_config_audit.py,test_mios_redact.py,test_mios_vector.py,test_mios_gateway_agent.py,test_mios_gateway_queue.py,test_mios_admission.py,test_mios_mcp_pool.py,test_mios_bench_harness.py}"
[ -n "${MIOS_CI_GLOBS_AGENT_PIPE_SKIP_REASON+x}" ] || MIOS_CI_GLOBS_AGENT_PIPE_SKIP_REASON='needs a live database or a heavy dependency; covered by the build job'"'"'s virtualenv gate'
: "${MIOS_CI_GLOBS_AGENT_PIPE_TIER:=unit}"
: "${MIOS_CI_GLOBS_LIBEXEC_DIR:=usr/libexec/mios}"
: "${MIOS_CI_GLOBS_LIBEXEC_GLOB:=test_mios_*.py}"
: "${MIOS_CI_GLOBS_LIBEXEC_TIER:=unit}"
: "${MIOS_CI_GLOBS_TOOLS_DIR:=tools}"
: "${MIOS_CI_GLOBS_TOOLS_GLOB:=test_*.py}"
: "${MIOS_CI_GLOBS_TOOLS_TIER:=unit}"
: "${MIOS_CI_MAX_EXEMPT_SUITES:=7}"
: "${MIOS_CI_PYTHON_PACKAGES:=pyflakes,fastapi,httpx,uvicorn,websockets}"
: "${MIOS_CI_RUNNERS:=tests/run-suites.sh}"
: "${MIOS_CI_TIERS_GATE:=automation/98-drift-checks.sh,tests/drift-gate-negatives.sh}"
: "${MIOS_CI_TIERS_LINT:=automation/lint-shell.sh,automation/lint-python.sh,automation/lint-json.sh,automation/lint-powershell.sh,automation/lint-ps-analyzer.sh,automation/97-ssot-lint.sh}"
: "${MIOS_CI_TIERS_UNIT:=tests/powershell/run-pester.sh,tests/test-sandbox-seccomp.sh,tests/test-powershell-flatten.sh,tests/test-pgvector-major-upgrade.sh,tests/test-mios-manual-harvest.sh,tests/test-offload-overlay.py,tests/test-blade-reachability.sh,tests/test-seat-auth-posture.sh,tests/test-seat-activates-nothing.py,tests/test-greenboot-blade-guard.sh,tests/test-greenboot-blade-reachability.sh,tests/test-lint-python-coverage.sh,tests/test-lint-shell-coverage.sh,tests/test-owui-pipe-endpoints.py,tests/doc-production-evidence.sh,tests/test-a2o-fallback.sh,tests/test-bake-group.sh,tests/test-firstboot-prestage.sh,tests/test-role-apply-precedence.sh,tests/test-theme-merge.py,tests/test-dispatcher-defense.py,tests/test-ek-smart-extract.py,tests/test-ek-substitution.py,tests/test-launch-args.py,tests/test-refine-guard.py,tests/test-refine-guards.py}"
...
: "${MIOS_LEGIBILITY_MAX_SHELL_LINES:=45120}"
...
: "${MIOS_TESTS_MAX_LEAKED_FIXTURES:=0}"
: "${MIOS_TIMEZONE:=UTC}"
