#!/bin/bash
# MiOS v0.1.1 — 11-hardware: GPU drivers (Mesa + AMD ROCm + Intel + NVIDIA)
#
# NVIDIA strategy (v0.1.1):
...
#
# CHANGELOG:
#   v0.1.1: Dropped COPY-layer fallback. ucore-hci IS already built from
...
#   v0.1.1: (attempted COPY-layer, reverted)
...
# exact failure mode that broke v2.2.x when ucore kernel (v0.1.1) didn't
# match F44's kernel-devel (v0.1.1). If kernel-devel is unavailable, we log
