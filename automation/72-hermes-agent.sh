...
# automation/72-hermes-agent.sh -- UNIFIED agent-plane install driver.
#
...
# pattern 73-model-prep.sh uses for MIOS_LLAMACPP_BAKE_MODELS.
#
...
mios_log "venv interpreter: ${VENV_PY} ($(${VENV_PY} --version 2>&1))"
...
if [ -n "$LOCAL_SOURCE" ]; then
    ...
    if [ ! -d "${VENV_ROOT}/.git" ]; then
        mios_log "cloning ${HERMES_REPO} (${HERMES_REF}) to ${VENV_ROOT}"
        rm -rf "${VENV_ROOT}"
