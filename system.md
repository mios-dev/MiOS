...
> llama.cpp), CLI agents (Claude Code, Gemini Code, Codex, Cursor, Aider, Continue),
> and inter-agent shared scratchpads.
>
> **Deployment paths (all symlinked to the same content):**
> - `/usr/share/mios/ai/system.md` — image-baked authoritative copy
> - `/etc/mios/ai/system-prompt.md` — host-local override (admin-editable)
> - `/CLAUDE.md`, `/GEMINI.md`, `/AGENTS.md`, `/.cursorrules`, `/.clinerules` — repo-root
>   tooling files at MiOS-DEV's root (which IS the OS root — see §3)
>
