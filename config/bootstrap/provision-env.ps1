...
function Export-EnvFile {
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingConvertToSecureStringWithPlainText', '', Justification = 'Env var is already cleartext; this feeds DPAPI encryption for at-rest storage.')]
    param([string]$Path)
...
try {
    ...
    $wslLines = @(
        "# 'MiOS' v0.3.0 -- WSL2 Configuration",
        "[wsl2]",
        "memory=${wslRAM}GB",
        "processors=${wslCPUs}",
        "swap=8GB",
