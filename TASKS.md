...
| T-314 | P2 | in-progress | Lifecycle/Health | BLADE-03 -- Give greenboot's role-awareness an SSOT it actually reads |
| T-315 | P2 | planned | Topology/SSOT | BLADE-04 -- Finish WS-BLADE: karg producer, role-apply demotion, [profile] fold |
| T-316 | P1 | done | Naming/Addressing | ADDR-01 -- 17 executable retired-port fallbacks; Hermes binds an unassigned port |
| T-317 | P1 | planned | Build/SSOT | UNITGEN-01 -- [units] is an SSOT that projects to nothing; the golden master guards a copy against a copy |
| T-318 | P1 | in-progress | Naming/Addressing | ADDR-02 -- Seven sidecar ports were allocated but never bound; the collision check guards numbers nothing uses |
| T-319 | P1 | in-progress | Topology/SSOT | BLADE-05 -- The activation axis gates 3 of 23 services, so a seat still starts the whole service plane |
...
**Status:** in-progress -- gate reads the SSOT, and the blade-reachability question is ANSWERED: operator ruling is configurable-defaulting-to-no, so `[greenboot].blade_reachability_critical = false` now states it in the SSOT and Law 12 holds by default. The probe script still hardcodes its four unit/port pairs instead of reading `[greenboot].critical_services` -- that wiring is what remains. (Its `hermes.service` line, which could never run, now names `hermes-worker.service`: T-316.) | **Domain:** Lifecycle/Health | **Who:** architect
...
**Status:** planned -- one piece landed early: operator ruling is that a SEAT IS AN ARCHETYPE, not an image, and it is `endpoint` rather than a fourth name for the same thing. `[blade.archetypes]` now declares the full table (`hybrid`/`compute`/`controller`/`endpoint`/`headless`/`desktop`); an archetype with no capabilities IS a seat, since a unit activates only when its `ConditionPathExists` marker exists. Behaviour-preserving -- `role-apply` already resolved an undeclared archetype to `[]` -- so it states what the tree did. CORRECTION to an earlier note here: the six role targets are NOT identical boilerplate. Four of them (`compute`, `endpoint`, `controller`, `headless`) are thin -- `Requires=multi-user.target` plus `Conflicts=` -- and that sample is what produced the wrong generalisation. The other two carry real membership: `mios-hybrid.target` (the DEFAULT `[blade].type`) has `Requires=graphical.target` and `Wants=k3s-agent.service`, and `mios-desktop.target` requires `gdm.service` plus the whole libvirt stack. So `role-apply`'s `systemctl start --no-block "$TARGET"` is NOT a no-op and must NOT simply be deleted: on the default blade type it is what brings up the GUI and the k3s agent. Demoting role-apply therefore requires moving those two targets' membership somewhere declarative FIRST -- it is not the free subtraction it looked like. The KARG PRODUCER has now landed: `tools/generate-blade-karg.py` projects `[blade].type` into `usr/lib/bootc/kargs.d/05-mios-blade.toml` as a bare `kargs = ["mios.blade=<type>"]`, wired into `tools/sync-generated.sh` (step 4c) and gated by `check_blade_karg` (gate 171), which regenerates in memory and diffs. It refuses an empty `[blade].type` and a type naming no archetype -- both would emit a karg that selects nothing -- and the gate fails on a hand-edited file AND on the file being absent, which is the state this started in. 8 assertions in `tools/test_generate-blade-karg.py`, one of which checks that `role-apply` actually parses the token this file emits, plus a two-case negative test proven by sabotage. Still open: the `systemctl` removal (see the correction above -- it is NOT free) and the `[profile]` fold. | **Domain:** Topology/SSOT | **Who:** architect
...
**Status:** in-progress -- `check_blade_coverage` (gate 170) requires every Quadlet container to be either capability-gated in `[blade.requires]` or listed in the shrink-only `[blade].ungated` register. It also rejects a requires or register entry naming a container that does not exist, an empty capability list (which gates nothing, so say so in the register instead), a capability NO archetype grants (nothing could ever activate it, so the unit would be dead on every blade type), a service classified both ways, a duplicated register entry, and an empty `[containers]` table rather than passing vacuously. Standing: **23 containers, 3 capability-gated, 20 registered.** 15 assertions in `tools/test_check-blade-coverage.py` -- including that `endpoint` must expand to `[]` or it is not a seat -- plus a two-case negative test (a container classified neither way; a capability no archetype grants), both proven by sabotage. | **Domain:** Topology/SSOT | **Who:** architect
...
**What+How:** MEASURED. `tools/native/mios-unit-gen` has a real `render_units()` that reads `mios.toml [units.*]`, and a real `--check` that renders and diffs against `usr/lib/systemd/system` -- whose own source comment says a previous version "could never detect the one thing it exists to detect". Neither is wired into `98-drift-checks.sh`, `tools/sync-generated.sh`, the justfile or CI. Run by hand, `--check` reports **66 rendered, 66 drifted**. That number is NOT unit rot: the renderer is incomplete -- it emits sections in the wrong order (`[Install]` before `[Service]` before `[Unit]`), emits the SSOT's `comment` key as a literal `comment=` directive, and passes `${VAR:-default}` through verbatim (systemd does NOT expand variables in `Environment=`, so a rendered unit would bind a literal string). The only test, `test_golden_master_matches_systemd_tree`, calls `verify_golden_master(systemd_dir, golden_dir)` -- it compares the shipped tree against a COPY of the shipped tree and never invokes the generator at all. So Law 8 is unenforced across the whole `usr/lib/systemd/system` surface, and `[units]` is documentation nothing projects. Fix the renderer to reproduce a shipped unit byte-for-byte (section order, `comment` -> real comment lines, bake-time substitution for `Environment=`), make the golden test render FROM the SSOT, then wire `--check` into the drift gate with a shrink-only register while the backlog drains.
**Where:** `tools/native/mios-unit-gen/src/lib.rs`, `tools/native/mios-unit-gen/tests/golden_master.rs`, `automation/98-drift-checks.sh`, `tools/sync-generated.sh`, `usr/share/mios/mios.toml` (`[units]`).
**Done When:** `render_units()` reproduces every shipped unit byte-for-byte; the golden test renders from `mios.toml` rather than diffing the tree against itself; `--check` runs in the drift gate; and a hand-edit to a shipped unit that contradicts `[units]` fails the gate.
**Why:** This is why `hermes-worker.service` could bind `8643` while its own `[units]` block said `${MIOS_PORT_HERMES_WORKER:-8730}` -- for however long, with a generator and a checker both sitting in the tree unused. A golden master that compares a copy to a copy is the tree's recurring defect class in its purest form: a gate reporting success over a set that excludes the thing it checks.
**Dep:** none -- independent. Large because the renderer needs to become faithful before the gate can be turned on.
**Status:** planned | **Domain:** Build/SSOT | **Who:** build agent
