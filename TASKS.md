...
**Status:** planned -- one piece landed early: operator ruling is that a SEAT IS AN ARCHETYPE, not an image, and it is `endpoint` rather than a fourth name for the same thing. `[blade.archetypes]` now declares the full table (`hybrid`/`compute`/`controller`/`endpoint`/`headless`/`desktop`); an archetype with no capabilities IS a seat, since a unit activates only when its `ConditionPathExists` marker exists. Behaviour-preserving -- `role-apply` already resolved an undeclared archetype to `[]` -- so it states what the tree did. Also worth recording for the demotion: the six role targets are IDENTICAL boilerplate differing only in `Description`/`Conflicts` and carry no unit membership, so what `role-apply` achieves imperatively beyond `set-default` is nothing. Still open: the karg producer, the `systemctl` removal, and the `[profile]` fold. | **Domain:** Topology/SSOT | **Who:** architect
