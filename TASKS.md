...
| T-295 | P2 | planned | Docs/DocGen | DOCGEN-02 -- durable ratchet floor (`coverage --write-floor` + low-water mark) so a lowered ceiling cannot be silently raised back; monotone gate currently compares only against HEAD and exits 0 when the previous TOML will not parse. **Done-when:** lowering then raising a ceiling in one commit fails the gate. |
| T-296 | P1 | planned | Docs/DocGen | DOCGEN-03 -- `mios-manual harvest`, the missing link: move a MIGRATE comment block into an authored doc passage, stamp the `mios-src:<sha12>` anchor, fill the ledger's landed_* columns. **Done-when:** a harvested block flips `landed()` false->true and the narrative count falls by one. |
| T-297 | P1 | planned | Docs/DocGen | DOCGEN-04 -- `check_comment_landing` + `mios-manual prune`: delete a source comment only once its knowledge is provably landed. **Done-when:** prune refuses when the predicate is false; round-trip covered by a test. |
| T-298 | P2 | planned | Docs/DocGen | DOCGEN-05 -- widen render scope and add the seven missing deriver families (pipeline, verbs, units, index, related, api, root-exceptions, boilerplate); make an unknown deriver ARG fail loudly; close the negative-test hole so the gate also proves prose OUTSIDE a marker stays green. **Done-when:** each family renders from the SSOT and is drift-gated. |
| T-299 | P2 | planned | Docs/DocGen | DOCGEN-06 -- make `manual.md` reproducible (regenerating it today destroys its H1 and ToC) and give llms.txt/llms-full.txt/doc README the same authored-plus-marker treatment; retire the `just manual` generator that silently rewrites a shipped artifact. **Done-when:** regenerating changes nothing, no `file:///C:/` paths survive, ports match `[ports]`. |
| T-300 | P2 | planned | Docs/DocGen | DOCGEN-07 -- cross-repo participation: run the doc CLIs against `mios-bootstrap.git` by root (69 of its 98 taggable files already carry AI-hints, but it ships no docgen tooling) without duplicating any tool across repos (Law 15). **Done-when:** one command documents either repo. |
...
| T-293 | P2 | planned | Security/Upstream | UPSTREAM-06 -- re-render the stale doc port/lane tables (README/CLAUDE/GEMINI/SECURITY/api.md) from the `[ports]` SSOT; three inconsistent schemes circulate and corrupt downstream research. **Done-when:** one port scheme across every doc, guarded by a projection drift-check. |
