...
**Status:** in-progress -- the gate half is done, in two rounds. check_module_length originally used `find -maxdepth 1` and saw 9 of 112 modules; widening it to walk `mios_pipe/` recursively found eleven files of 820-1786 lines. But the audit's second clause was right too: the gate still stopped at `mios_pipe/`, so the agent-pipe ROOT was never scanned -- and that is where the two biggest modules live, `mios_dispatch.py` (1178, this task's named target) and `server.py` (4979, the extraction's own remainder). Coverage is now 112 -> 284 files; re-export shims are excluded (~28 lines of lazy re-export is not a module). All thirteen over-limit files are in the shrink-only `[refactor].oversize` register, which fails on GROWTH and on an unrecorded SHRINK, so the split lands as entries going down. REMAINING: the actual split of mios_dispatch.py and the rest of server.py's decomposition. **Domain:** AI-Plane/Refactor SPLIT PREREQUISITE MET: the two SECURITY gates at the dispatch chokepoint -- `_rule_of_two_gate` (Rule-of-Two / CaMeL-class) and `_quarantine_gate` (dual-context quarantine) -- had ZERO test coverage, so refactoring around them would have been unverifiable. 20 assertions now cover off/audit/enforce for both, the all-three vs strict-superset distinction between them, action-hash-keyed same-turn approval (an approval for a DIFFERENT action must not downgrade a block), and degrade-open on an internal error -- a gate that failed CLOSED there would take the whole dispatch plane down. Mutation-tested three ways: making the gate never block, ignoring the approval hash, and failing closed each red the suite. The command-building cluster (`_build_dispatch_cmd` / `_sandbox_wrap_cmd` / `normalize_container_exec` / `_dispatch_sandbox_profile`) is the right first extraction -- it is already well covered -- but it threads injected `_template_to_cmd` / `_arg_with_synonyms`, so it wants its own configure() seam. | **Who:** AI-plane agent
