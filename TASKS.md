...
| T-304 | P2 | planned | Docs/DocGen | DOCGEN-11 -- retire the old port schemes from the runtime AI-payload and long-tail doc surfaces (~125 files: `usr/share/mios/ai/system.md` + INDEX.md, `etc/mios/ai/system-prompt.md`, `etc/mios/system-prompts/*`, hermes skills SKILL.md, open-webui prompts, cookbooks, FIRST-BOOT, concepts/guides). Many are READONLY llm-payload class -- edit the payloads deliberately, not by sweep; historical archives (knowledge/, archive/, upstream-gaps) stay verbatim. Also reconcile the contradictory heavy-lane engine comments inside `mios.toml` (`[security.privileged_quadlets]` vs `[ai.sglang]`) and consider distill-time retired-port sanitization. **Done-when:** every living payload surface is in `[docs].port_clean` and the gate stays green. |
