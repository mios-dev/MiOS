...
| T-304 | P2 | in-progress | Docs/DocGen | DOCGEN-11 -- retire the old port schemes from the runtime AI-payload and long-tail doc surfaces. **Batch 1 done (12 files, 85 edits):** `usr/share/mios/ai/INDEX.md` (31), `docs/agents/AI-ARCHITECTURE.md` (21), `docs/ai-pipeline-map.md` (15), `ai/audit-prompt.md`, `etc/mios/ai/system-prompt.md`, `etc/mios/system-prompts/mios-reviewer.md`, two hermes SKILL.md, `cookbooks/ingest-kb.md`, `tools/README.md`, root `system-prompt.md`, `security/README.md` -- each edit drafted then adversarially verified (0 rejections), all 12 now in `[docs].port_clean` so the gate holds them. The sweep also corrected INDEX.md's swapped heavy-lane engines AND its wrong `ConditionPathExists` paths. **Remaining (T-305):** the ~22 files whose verification pass did not complete. **Done-when:** every living payload surface is in `[docs].port_clean` and the gate stays green. |
| T-305 | P2 | planned | Docs/DocGen | DOCGEN-12 -- finish the T-304 sweep for the remaining living payload surfaces: `usr/share/mios/ai/system.md`, `ai/hermes-soul-full.md`, `etc/mios/system-prompts/mios-engineer.md` + `mios-troubleshoot.md`, `hermes/skills/mios-environment` + `opencode-delegation`, `open-webui/system-prompts/mios-agent.md`, `cookbooks/finetune-flow.md` + `local-rag-day0.md`, `docs/day-0/FIRST-BOOT.md`, `docs/agents/PC-CONTROL-LOCAL.md`, `docs/terminal/INVOCATIONS.md`, `installation/UNIFY.md`, `tools/windows/README-WINDOWS.md`, the manual ToC blurbs in `usr/share/doc/mios/manual.md` + `manual/ch04`/`ch10`, and 7 AI-hint headers (adr/0006, adr/0009, concepts/architecture, concepts/llamacpp-engine-conversion, concepts/unified-ai-pipeline-2026-06-16, guides/edge-node-join, guides/install) whose rot projects into the generated indexes. Drafts exist and are cached in the run journal; they need the verification pass before they are trusted. Also: reconcile the contradictory heavy-lane engine comments inside `mios.toml` itself. **Done-when:** those files are in `[docs].port_clean` and the gate stays green. |
