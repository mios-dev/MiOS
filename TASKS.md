...
| T-303 | P2 | planned | Docs/DocGen | DOCGEN-10 -- decide who owns root `llms.txt`. mios.git ships a near-copy of mios-bootstrap's (they differ in 2 wording lines) whose AI-hint and header both announce it as the index **for mios-bootstrap**, so an agent reading mios.git's machine-readable index is told it is looking at the installer repo. Either mios.git gets its OWN index (FHS overlay, Containerfile, automation, quadlets) or the mirror is declared bootstrap-owned like `Get-MiOS.ps1`/`install.*` and listed as such in CLAUDE.md. Cross-repo, so it needs the operator (Law 15). **Done-when:** mios.git's llms.txt describes mios.git, or CLAUDE.md names it a bootstrap-owned mirror. |
| T-302 | P2 | planned | Docs/DocGen | DOCGEN-09 -- finish the manual rebuild: manual.md as authored chapter files under `usr/share/doc/mios/manual/` plus marker interiors (so regeneration is provably lossless rather than merely unused), the same treatment for `llms.txt`/`llms-full.txt`, and a `usr/share/doc/mios/README.md` entry point -- 130 baked doc files currently have none on a booted host. **Done-when:** rendering the manual changes nothing and a missing chapter fails the gate. |
