...
| T-288 | P1 | done | Security/Upstream | UPSTREAM-01 -- pgvector pin bumped to verified latest stable (`0.8.6-pg17`) through the SSOT, all projections regenerated. **Done-when:** `just drift-gate` green with the new pin in `[image.sidecars]`, `[build.bake].core`, the Quadlet, plan.d and every generated twin; PG major unchanged. |
| T-289 | P1 | planned | Security/Upstream | UPSTREAM-02 -- derive the `mios-resolve-latest` ref list from `[image.sidecars]` instead of a hand-mirrored array (four refs had silently drifted: `pg16`, `valkey:8.0`, `ceph:v18`, `open-webui:latest`). **Done-when:** deleting a ref from the SSOT changes the resolver's set with no script edit; a parity drift-check covers the surface. |
| T-290 | P2 | planned | Security/Upstream | UPSTREAM-03 -- Renovate customManager for the exact-pinned `[image.sidecars]` entries so the SSOT's "Bump via Renovate" comment becomes true. **Done-when:** a stale exact pin (pgvector, k3s) produces a Renovate PR; float `:latest`/`:main` refs stay unmanaged. |
...
| T-292 | P2 | planned | Security/Upstream | UPSTREAM-05 -- pg17→pg18 major migration path for the mios-pgvector data dir, so the `-pgNN` tag suffix can advance separately from pgvector version bumps. **Done-when:** a populated pg17 datastore comes up on a `-pg18` image with row counts and HNSW recall intact, with a rollback path; runs after T-175 backups land. |
