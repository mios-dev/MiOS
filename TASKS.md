...
**OPEN, deliberately not decided here:** whether `mios-k3s` (which runs `k3s server`, the cluster control plane) should also require `controller`. It reads correct -- a `desktop`, `headless` or `compute` blade arguably should not run a Kubernetes control plane -- but unlike PXE it is enabled today, so gating it CHANGES what those three archetypes run. That is an operator call about what a headless MiOS box is for, not a drift fix, and it is recorded here rather than guessed.
...
**Status:** planned -- RE-MEASURED during T-315, and it is worse than this entry recorded. `--check` now reports **68 rendered, 68 drifted** (100%): a dump of one rendered unit against its shipped file shows sections in ALPHABETICAL order (`[Install]` before `[Unit]`), keys sorted alphabetically inside each section, the SSOT's `comment` key emitted as a literal `comment=` directive followed by a bare continuation line that is not a `key=value` at all, and TOML booleans rendered `AllowIsolate=true` instead of `yes`. So `render_units()` does not merely drift from the tree -- it emits unit files systemd would reject. That is why T-315's role-target fixes had to be hand-applied to all THREE copies (`mios.toml [units.*]`, `usr/lib/systemd/system/`, `tests/golden/`) and verified equal by hand rather than regenerated.

Two further findings, both recorded not fixed because the fix is this task's:
* **The golden test is RED today.** `cargo run -p mios-unit-gen -- --selftest` fails with `Mismatch in golden snapshot for hermes-worker.service`. The copy has diverged from the copy -- so even the degenerate check this entry describes is no longer passing, and nothing runs it.
* **The golden dir is a hiding place for retired ports.** `tests/golden/hermes-worker.service` still carries `:8643`, `:11441`, `:8642`, `:8640` and `tests/golden/mios-firewall-ports.service` carries `:8888`/`:11235` -- four of which are in `[docs].retired_ports`. `check_doc_port_scheme` does not catch them because it scans only the files named in `[docs].port_clean`, an explicit allow-list that has never included this directory. Same defect class as the rest of this entry: a gate reporting success over a set that excludes the thing it checks. Widening `[docs].port_clean` to cover `tests/golden/**` should land WITH the renderer fix, since the goldens must become renderer output first or the gate goes red on day one. | **Domain:** Build/SSOT | **Who:** build agent
