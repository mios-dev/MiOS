...
- `WS-A` — Multi-agent / orchestrator (the agent system) (proposed)
- `WS-B` — AIOS governance (activate the inert) ✅
- `WS-C` — Pod consolidation + port minimization (proposed)
- `WS-D` — Remote / edge AI nodes (remote compute) (proposed)
- `WS-E` — OWUI integration (proposed)
...
- `WS-H` — Sovereign AIOS Architecture & Advanced Orchestration (Additive Research) (proposed)
...
- `WS-NOHC` — NO-HARDCODE / SSOT completeness (proposed)
- `WS-WIN` — Windows-11-minimal install completeness (proposed)
...
- `WS-ACCT` — DB-driven GLOBAL account control plane (Linux + Windows; live, bidirectional) (proposed)

**Part 13: Advanced Multi-Agent Orchestration Strategies (2026-07-05)**
- `WS-MAO` — Multi-Agent Orchestration strategy adoption (on the existing agent plane) (active)

**Part 14: MiOS-XBOX as the self-contained AIO deployment medium (2026-07-06)**
- `WS-AIO` — self-contained embedding + multi-target deployment (proposed)

**Part 15: Global Naming Minification — one unified names/keys registry (2026-07-10)**
- `WS-NAME` — Global naming minification → one unified names/keys SSOT (proposed)

**Part 16: Install/first-boot pipeline reorder + dependency-completeness (2026-07-10)**
- `WS-DEPLOY` — logical dependency ordering; eliminate "missing dependency" states (proposed)

**Part 17: Absorbed top-level plans (2026-06-14/15)**
- `WS-SHELL` — Persistent PTY / stateful shell substrate (G9) (proposed)
- `WS-KENF` — Kernel enforcement plane: Tetragon eBPF/LSM (G2-rest) (proposed)
- `WS-ISOL` — Per-action isolation tier ladder (G4) (active)
- `WS-GVLM` — Grounding-VLM activation on the perception path (G1) (active)
- `WS-CONS` — Weighted consensus judge + JSD drift monitor (K4) (proposed)
- `WS-GUARD` — Autonomous-runtime guardrails (W0-T2/W0-T3) (proposed)
- `WS-DURA` — Mutable-brain durability + redaction (W0-T4/W2-T4) (proposed)
- `WS-LSFS` — Semantic-FS verbs + task-state protocol (K6) (proposed)

**Part 18: Absorbed concept/workstream docs (2026-06)**
- `WS-FBM` — First-boot large-model provisioning (proposed)
- `WS-OFFL` — Offline build-time vendoring (proposed)
- `WS-IGPU` — iGPU-in-WSL + llama.cpp RPC compute lanes (proposed)
- `WS-RDSK` — GPU-accelerated remote-desktop lane (proposed)
- `WS-WSL` — Dual-personality WSL image + offline atomic upgrades (proposed)
- `WS-STD26` — 2026 agent wire-standards upgrade (proposed)
- `WS-OAI` — OpenAI-conformance residual (proposed)
- `WS-KACT` — Built-gated kernel/module activation deltas (active)
- `WS-UISHELL` — Native QML shell surfaces (active)
- `WS-NAME2` — Naming/SSOT reconciliation residual (extends WS-NAME/T-165) (active)
...
- `WS-OSCTL2` — hwnd-threaded target-window resolution for pc_type (extends CU-01) (proposed)

**Part 19: Deploy the stated heavy dGPU lane (model provisioning) (2026-07-10)**
- `WS-HEAVY` — provision the heavy dGPU model so the STATED lane defaults deploy (proposed)

**Part 20: Everything DB-driven + vectorized (2026-07-10)**
- `WS-VECTOR` — Everything DB-driven + vectorized (unified pgvector control plane) (proposed)

**Part 21: Universal-Core / MiOS-Sys Refactor + Sovereign M: Deployment (2026-07-12)**
...
- [Part 9: XDG + CephFS Unified Storage Fabric](#part-9-xdg-cephfs-unified-storage-fabric)
...
- [Part 10: Converged-Resource Architecture (2026-06-25)](#part-10-converged-resource-architecture-2026-06-25)
...
- [Part 13: Advanced Multi-Agent Orchestration Strategies (2026-07-05)](#part-13-advanced-multi-agent-orchestration-strategies-2026-07-05)
- [Part 14: MiOS-XBOX as the self-contained AIO deployment medium (2026-07-06)](#part-14-mios-xbox-as-the-self-contained-aio-deployment-medium-2026-07-06)
- [Part 15: Global Naming Minification — one unified names/keys registry (2026-07-10)](#part-15-global-naming-minification-one-unified-nameskeys-registry-2026-07-10)
- [Part 16: Install/first-boot pipeline reorder + dependency-completeness (2026-07-10)](#part-16-installfirst-boot-pipeline-reorder-dependency-completeness-2026-07-10)
- [Part 17: Absorbed top-level plans (2026-06-14/15)](#part-17-absorbed-top-level-plans-2026-06-1415)
- [Part 18: Absorbed concept/workstream docs (2026-06)](#part-18-absorbed-conceptworkstream-docs-2026-06)
- [Part 19: Deploy the stated heavy dGPU lane (model provisioning) (2026-07-10)](#part-19-deploy-the-stated-heavy-dgpu-lane-model-provisioning-2026-07-10)
- [Part 20: Everything DB-driven + vectorized (2026-07-10)](#part-20-everything-db-driven-vectorized-2026-07-10)
- [Part 21: Universal-Core / MiOS-Sys Refactor + Sovereign M: Deployment (2026-07-12)](#part-21-universal-core-mios-sys-refactor-sovereign-m-deployment-2026-07-12)
<!-- ROADMAP_TOC_END -->
---
...
# Part 14: MiOS-XBOX as the self-contained AIO deployment medium (2026-07-06)
*Source: operator directive — "MiOS-XBOX is the MiOS AIO deployment for MiOS-XBOX AND MiOS-DEV environments; the made ISO is flashed to USBs with ALL MiOS files/dependencies EMBEDDED; deployed bare-metal, VM, QEMU, RAW, ISO, Podman, WSL2/g, Ignition (Fedora mutable minimal), OCI/Bootc. MiOS is self-building, self-deploying, self-developing, self-hosted, self-hostable within itself."*

MiOS-XBOX.iso is not just a VM test target — it is the **bootable, USB-flashable, self-contained AIO** that stands up BOTH the Windows-side MiOS-XBOX gaming edition AND the MiOS-DEV (Fedora bootc) environment, with **zero runtime download** (bare-metal sites may be offline). Today the brain deploys via `irm|iex` from GitHub (MiOS-Host `BootstrapUrl`) — that must become an EMBEDDED, offline-first deploy with the network path kept only as an update fallback.

## WS-AIO — self-contained embedding + multi-target deployment
- **AIO-01 (T-165) embed the installer repos in the ISO [P2]** — stage `mios-bootstrap` (Get-MiOS.ps1 + installer) and `mios` (system overlay) into the image (e.g. `C:\ProgramData\MiOS\repo\`) at build time; MiOS-Host / MiOS-FirstBoot run the LOCAL `Get-MiOS.ps1` (offline) and only fall back to `irm|iex` for updates / if the embedded copy is absent. No GitHub dependency for a clean deploy.
- **AIO-02 (T-166) embed the WSL2/MiOS-DEV rootfs + heavy deps [P2]** — pre-build + bake the MiOS-DEV distro tarball (and the pinned RPM/flatpak/OCI dependency set) into the ISO so `wsl --import` + the agent stack come up OFFLINE. MiOS builds its own deployment artifacts (self-building) and ships them in the medium.
- **AIO-03 (T-167) embed OCI/bootc images + models [P3]** — bake the bound Quadlet images (llama-swap, pgvector, agent-pipe, …) + the default model(s) into `/usr/lib/containers/storage` (Law 3 BOUND-IMAGES) and an offline model cache, so first boot needs no pull. Size-gated (SSOT `[image].embed_*`).
- **AIO-04 (T-168) USB-flash + bare-metal path [P2]** — the oscdimg dual BIOS/UEFI ISO is written to USB (Rufus/dd/`New-MiOSUsb`); the embedded autounattend handles anything DISM+UUP didn't strip; validate a bare-metal install (real hardware / Secure Boot) not just the Hyper-V VM.
- **AIO-05 (T-169) unify the multi-target deploy matrix [P3]** — one SSOT `[deploy].targets` drives the same MiOS image out to bare-metal / VM / QEMU / RAW / ISO / Podman / WSL2g / Ignition (Fedora mutable minimal) / OCI-bootc from the shared artifact set; MiOS deploys itself (self-deploying) to any of them.
...
# Part 21: Universal-Core / MiOS-Sys Refactor + Sovereign M: Deployment (2026-07-12)
*Source: three decision-grade design studies (core bake-gate + universal-core/blade-type; MiOS-Sys shared-base consolidation; run-off-M: sovereign deployment) reconciled against the live tree, plus the release-topology + SBOM-not-hardcode operator directives.*

**The organizing principle — two orthogonal gates.** Every apparent conflict in the studies dissolves once you separate the **BAKE gate** (`mios.toml [build.bake]` — *is this content present in the image at all?*) from the **ACTIVATION gate** (`[blade]` systemd `Condition*` — *does this baked unit START on THIS machine?*). CORE is always baked regardless of any enable flag (*core overrides SSOT*); blade type gates which baked CORE units run; à-la-carte content (today essentially just model weights) is the only thing whose *presence* is enable-gated. One universal image specializes into roles purely by cmdline token + marker files evaluated by systemd — never per-role image variants. The heavy GPU serving ENGINES (~47 GB of vLLM+SGLang) are CORE (baked everywhere, image-only); model WEIGHTS stay à-la-carte (empty `bake_model` = zero bloat).
