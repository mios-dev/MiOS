...
- **XBOX-02 (T-141) gaming loadout + Xbox tuning [P3]** — Xbox services Manual, Teredo/IPv6, Game Mode, Delivery Optimization, FSE regs; winget gaming apps (Steam/Vesktop/Zen) — adopt the reference `unattend-02/03.ps1`, sanitized (branding → MiOS OEM, not "Kabu").
...
The pgvector `account` table is the RUNTIME SSOT for OS accounts. Setup/install seeds it from the mios.toml SSOT; thereafter the DB is authoritative and both OSes reflect DB edits live. A hard rule falls out of the current dashboard bug: the LOGIN account (`account.name`) is distinct from the operator DISPLAY name (`[user].name`, e.g. "Kabu") — the display name must NEVER land in a login/credential slot, and no consumer may resolve the login user from `MIOS_USER` / `[user].name`.
