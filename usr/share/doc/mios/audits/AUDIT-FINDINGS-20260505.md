...
| 3 | HIGH | Architectural Law (LAW 6) | `mios-forgejo-runner` runs as root, exception undocumented | `etc/containers/systemd/mios-forgejo-runner.container:95-96` | YES (documented in `usr/share/mios/ai/INDEX.md` sec 3 row 6 + Quadlet header) |
...
- **Status:** NOT REMEDIATED. The directive runs unconditionally before any logic in all 52 scripts; only the strict "at the top" reading is not satisfied. Recommended action: soften `usr/share/doc/mios/guides/engineering.md` shell convention §1 to "before any executable statement," which is what the code already satisfies.
...
- Justfile-target citations in markdown were grepped via `just [a-z-]+`; the regex caught English-language false positives ("just a", "just spreadsheets") but every actual Justfile target referenced in `README.md`, `CONTRIBUTING.md`, `usr/share/doc/mios/guides/engineering.md`, `usr/share/mios/ai/INDEX.md`, `usr/share/doc/mios/concepts/architecture.md`, `usr/share/doc/mios/guides/self-build.md` exists in the `Justfile`. PASS.
