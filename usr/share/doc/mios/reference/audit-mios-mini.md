...
3. **The GPU→guest map already exists but nothing consumes it for the bind.** `[mini.gpu]` (`mios.toml:322-326`) declares `assignments = { mios-guest = ["0000:01:00.0"] }` and `arbitration = "static"`, but no projector turns that into a driverctl override or a `vfio-pci.ids` stamp. §4.1 is that missing projector.

**Grounding evidence (all real):**

| Concern | File:line | What it gives us | What the Mini needs on top |
|---|---|---|---|
| IOMMU + early vfio bind | `usr/lib/bootc/kargs.d/01-mios-vfio.toml:3-9` | `rd.driver.pre=vfio-pci`, `intel_iommu=on amd_iommu=on iommu=pt` | driven fully from `[mini.gpu]`, all dGPU BDFs |
...
| GPU→guest map | `mios.toml:322-326` `[mini.gpu]` | `assignments`, `arbitration="static"` | the bind projector consumes it (§4.1) |
...
```dockerfile
# Containerfile.mini  --  MiOS-Mini hypervisor-router host (bootc)
# Projected package set == doc-mios-mini.md §2a floor. NO firewalld / ROCm / desktop / k3s.
...
# vendor-reset akmod is AMD-only + image-baked (doc §2a / R5); gate on [mini.gpu] vendor.
...
#   /etc/driverctl.d/20-mios-mini.conf     (per-BDF vfio-pci overrides)                  <- §4.1 drop-in
#   /etc/nftables.d/mios-router.nft        (master inet table)                           <- §4.3 drop-in
#   /etc/hostapd/hostapd.conf              (country/EHT/WPA3-SAE)                         <- from [mini.wifi]
#   /etc/dnsmasq.d/mios-guest.conf         (DHCP/DNS on guest bridge + split-DNS)        <- from [mini.net]
...
RUN systemctl enable libvirtd nftables dnsmasq chrony \
 && systemctl enable mios-headscale.service mios-mini-vfio-verify.service
...
## 3. The SSOT surface (extend `[mini.*]`)

The bind, the routing, the mesh, and the guest split all project from one `[mini]` block. `[mini.gpu]` exists (`mios.toml:322-326`); the rest is the concrete extension this audit proposes. Keep the operator authoring **this**, never the rendered files (doc §1.1: "zero hand-maintained config").
...
[mini.gpu]                       # EXISTS today (mios.toml:322). assignments = guest -> [BDF,...]
...
[mini.cpu]                       # the router-floor-first budget (doc §2e)
...
[mini.wifi]                      # host owns the radio (doc §2a)
enable   = false
country  = "US"
ssid     = "MiOS"
band     = "6g"                  # Wi-Fi 6E/7; 802.11be AP mode is driver-gated (doc R7)

[mini.mesh]                      # host owns the tailnet (doc §2a)
...
> **Reconciliation note (real gap).** `75-kargs-render.sh` reads `[kargs].vfio_ids` as a raw `vendor:device` string (`mios.toml:11438`). `[mini.gpu].assignments` names **BDFs**, not vendor:device IDs. The §4.1 projector closes this: it resolves each BDF → `vendor:device` (SBOM-not-hardcode: resolved at build/first-boot via `lspci`, not hand-pinned), writes the driverctl overrides by BDF, and **stamps** `[kargs].vfio_ids` so the existing kargs render keeps working unchanged.
...
Two coordinated outputs, both projected from `[mini.gpu].assignments`, so the host loads no GPU driver:
...
- **persistent per-slot bind (survives id collisions):** `/etc/driverctl.d/20-mios-mini.conf` — `driverctl` binds by **BDF**, so two identical cards (same `vendor:device`) still each land on `vfio-pci` and the wrong one is never claimed by `amdgpu`/`nvidia`/`i915`.

Save as `usr/libexec/mios/mios-mini-vfio-bind` (mode 0755). It is a **bake-time projector** (like `75-kargs-render.sh`) plus a `--verify` post-boot mode (like `vfio-check.sh`).

```bash
#!/usr/bin/env bash
# MIOS_APPLY_CLASS=bake-only
# AI-hint: Projects [mini.gpu].assignments (BDFs) into the driver-free-host vfio-pci bind:
#   writes /etc/driverctl.d/20-mios-mini.conf (per-BDF overrides) and stamps [kargs].vfio_ids
...
OUT="${DRIVERCTL_D}/20-mios-mini.conf"

# ---- read the SSOT: flatten every BDF across every guest in [mini.gpu].assignments ----
...
mkdir -p "$DRIVERCTL_D"
{
    echo "# GENERATED from mios.toml [mini.gpu].assignments -- DO NOT EDIT."
    echo "# Regenerate: mios-mini-vfio-bind   |   Verify (post-boot): mios-mini-vfio-bind --verify"
    for bdf in "${BDFS[@]}"; do
...
```nft
# AI-hint: GENERATED master router table for MiOS-Mini. DO NOT EDIT -- regenerate from mios.toml [mini].
# Apply: nft -f /etc/nftables.d/mios-router.nft   |   Remove: nft delete table inet mios-router
# Devices are projected from [mini].wan_iface / [mini].guest_bridge; subnets from [mini.mesh].
define WAN   = enp1s0            # <- [mini].wan_iface
define GUEST = mios-guest        # <- [mini].guest_bridge  (the NIC-less guest's only link)
...
```ini
# AI-hint: MiOS-Mini headscale control plane (host owns the tailnet). Root Quadlet --
#   MUST be listed in mios.toml [security.privileged_quadlets].root or drift-check 13 fails.
# AI-related: usr/share/mios/mini/headscale-policy.hujson, mios.toml [mini.mesh]
[Unit]
Description=MiOS-Mini headscale control plane
...
```hujson
{
  // GENERATED from mios.toml [mini.mesh] -- fail-closed. Empty groups == deny.
...
<!-- GENERATED from mios.toml [mini.*]. Placeholders rendered by mios-mini render (sed). -->
<domain type="kvm">
  <name>mios-guest</name>
  <uuid>{{MIOS_GUEST_UUID}}</uuid>

  <!-- 75-90% of RAM as locked 1GiB hugepages ([mini.mem].hugepages_1g) -->
  ...
  <!-- vCPUs == the isolcpus set from [mini.cpu]; emulator == housekeeping cores -->
  <vcpu placement="static">{{MIOS_GUEST_VCPUS}}</vcpu>
  <cputune>
    <!-- {{MIOS_GUEST_VCPUPIN}} expands to one <vcpupin vcpu=N cpuset=C/> per isolated core -->
    {{MIOS_GUEST_VCPUPIN}}
    <emulatorpin cpuset="{{MIOS_HOUSEKEEPING_CPUS}}"/>   <!-- [mini.cpu].housekeeping -->
  </cputune>
  <numatune>
    <memory mode="strict" nodeset="{{MIOS_GUEST_NUMA_NODE}}"/>  <!-- [mini.mem].numa_node -->
  </numatune>
  ...
  <devices>
    ...
    <!-- ===== ALL assigned dGPUs ([mini.gpu].assignments[mios-guest]) ===== -->
    ...
    <interface type="bridge">
      <source bridge="{{MIOS_GUEST_BRIDGE}}"/>          <!-- [mini].guest_bridge -->
...
The renderer (`mios-mini` verb / oneshot) computes `MIOS_GUEST_VCPUS`, `MIOS_GUEST_VCPUPIN`, `MIOS_GUEST_MEM_GIB`, and the `MIOS_GUEST_HOSTDEV_GPUS` loop from `[mini.cpu]`/`[mini.mem]`/`[mini.gpu]`, then `virsh define`s it (mirroring `virt-apply.sh:82-86`). Guest share is computed from the **router floor first** (doc §2e); the drift-gate rejects a render that leaves the housekeeping slice under its declared minimum.

---

## 5. Sequenced build/deploy steps (maps to doc §4)

1. **Build the host image** — `Containerfile.mini` (§2). `dnf remove firewalld`. Bake `vendor-reset` akmod **only if** `[mini.gpu].assignments` contains an AMD BDF.
2. **Project the bind** — run `mios-mini-vfio-bind` (§4.1) at bake: writes `/etc/driverctl.d/20-mios-mini.conf` + stamps `[kargs].vfio_ids`; `75-kargs-render.sh` then emits the UKI cmdline. Bake a **second, non-vfio recovery UKI entry** (doc §2d).
...
4. **Verify the bind** — `mios-mini-vfio-verify.service` runs `mios-mini-vfio-bind --verify`; every assigned BDF must read `vfio-pci` or boot fails loud (respecting `host_console_reserve`).
5. **Assign storage** — validate `[mini.gpu]`/storage never names the boot controller; multi-drive → `<hostdev>` the data NVMe; single-drive → virtio-backed volume on `/var` (§4.6; doc §2c).
...
- **`check_mini_vfio_projection`** — re-run `mios-mini-vfio-bind` to a tmp dir; fail on any drift between the rendered `20-mios-mini.conf`/stamped `vfio_ids` and the committed files. Also assert every `[mini.gpu].assignments` BDF appears exactly once (doc: "each GPU → exactly one guest", `mios.toml:323`).
...
- **`check_mini_guest_isolation`** — assert the rendered `mios-guest.xml` has **exactly one** `<interface>`, that its bridge == `[mini].guest_bridge`, that `<vcpupin>` ⊆ `[mini.cpu].isolcpus`, and that the housekeeping slice ≥ its declared floor (doc §2e). Also assert the swtpm `<tpm>` block is present.
...
- **The `[mini.*]` surface is proposed, not landed.** `[mini.gpu]` exists (`mios.toml:322`); `[mini]/[mini.cpu]/[mini.mem]/[mini.wifi]/[mini.mesh]` are this audit's extension. Landing them is a `mios.toml` + `mios-sync-toml` change (Law 15: update both repos), not covered here.
