<!-- AI-hint: GENERATED by tools/generate-metal-vs-hosted.py from mios.toml. DO NOT EDIT -- re-run the generator. Part 1 compares the two PRODUCTS ([blade.planes]); Part 2 the two MODES ([blade.archetypes]). Every count is derived, so neither can go stale. ADR-0016 D10-D14. -->
<!-- AI-related: usr/share/mios/mios.toml, usr/share/doc/mios/adr/0016-blade-node-topology.md, tools/generate-metal-vs-hosted.py -->

# MiOS-Metal vs hosted MiOS — the products, then the modes
...
A **MiOS-Metal** is a box. A **hosted MiOS OCI image** is the same image in a different position: a container, a VM, or another machine, local or remote. They are not two builds — one artifact, one tag, one bake. What separates them is not what they *contain* but what they *own*.
