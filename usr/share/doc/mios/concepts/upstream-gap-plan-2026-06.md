...
| B | **vLLM/SGLang heavy lane is NOT VRAM-blocked.** `--gpu-memory-utilization ~0.15–0.3` + **KV-cache CPU offload** (vLLM 0.22.1 native; SGLang 0.5.12 HiSparse) runs a quantized heavy/VLM lane in the ~4 GB the Windows host leaves on the 4090. | **OVERTURNS** the "vLLM heavy-lane VRAM-blocked" note in `gemma4_planner_deploy` / AIOS memories. **[VERIFY]** cheap. |
...
Cheap probes before committing engineering. Nothing is retired until these pass.
...
- **T0.2 [VERIFY] vLLM/SGLang in ~4 GB.** Launch vLLM 0.22.1 (or SGLang 0.5.12 HiSparse) with
  `--gpu-memory-utilization 0.2` + KV-CPU-offload on a small quantized model against the partially-occupied
  4090. **Gate:** serves an OpenAI-compatible completion without OOM while the Windows host holds ~20 GB. conf:H
...
- **T1.1 Retire the `mios-podman-ps` snapshot hack with the rootful-socket pattern.** Add a
  ...
  `/etc/tmpfiles.d/podman-rootful.conf` (`d /run/podman 0750 root podman`), add `mios-ai` to the
  `podman` group, point agent-pipe at `unix:///run/podman/podman.sock`. Kills the 15-min-stale JSON
  snapshot + name-matching; gives live inspect data regardless of host-net.
  **Security caveat:** rootful socket = root-equivalent. Podman has **no scoped/read-only socket mode**
  (upstream gap). Decision: grant **read/inspect via group only** and keep writes off `mios-ai`, OR keep a
  thin root read-proxy for least-privilege. Make this an explicit operator call, route through `mios.toml`. conf:H eff:S
- **T1.2 CDI + `nvidia-cdi-refresh` → rootless GPU quadlets.** Install nvidia-container-toolkit **≥1.18.0**
  ...
  driver/reboot — obsoletes any manual regen). Switch llama.cpp/ollama quadlets from privileged/`NVIDIA_VISIBLE_DEVICES=all`
  to `AddDevice=nvidia.com/gpu=all` with `no-cgroups=true` + `--group-add keep-groups`, running **rootless**.
  Hardens the highest-attack-surface services. conf:H eff:M
- **T1.3 `.wslconfig` + image hygiene.** Set `sparseVhd=true` + tune `autoMemoryReclaim` (coordinate
  ...
  **`console-getty.service` mask** (multi-distro getty fix). All via SSOT, not literals. conf:H eff:S
- **T1.4 `Notify=healthy` + HealthCmd across AI quadlets.** Replace hand-managed `After=` ordering for
  agent-pipe/llama.cpp/ollama/OWUI with real systemd readiness gating + rollback-on-failed-health. conf:H eff:M
...
- **T2.1 [VERIFY-gated on T0.1] Collapse the native-Windows iGPU server into an in-VM lane.** If T0.1
  ...
  + its Tailscale hop + the `mios-reasoner-cpu` overlay node — removes a cross-host dependency. Keep the
  Windows-native server as fallback until the in-VM lane is proven under load. conf:M eff:M
- **T2.2 [gated on T0.2] Activate the vLLM/SGLang heavy + VLM lane.** Run a quantized heavy lane in the
  partial 4090 (low gpu-mem-util + KV-CPU-offload). Prefer **SGLang** (HiSparse CPU KV-offload + VLCache)
  for the memory-constrained VLM. **Unblocks computer-use:** serve **Qwen3-VL** or **UI-TARS** for the
  `mios-computer-use`/`verify_launch` visual probe (validate grounding accuracy — Qwen3-VL pixel-grounding
  is reportedly weak; UI-TARS may ground better). Re-enable the gemma4 planner heavy path this frees. conf:H eff:M
- **T2.3 llama.cpp RPC fabric + coopmat2.** Run an `rpc-server` per lane/node (phone/iGPU/dGPU/cluster);
  ...
  the existing `[agents.*.nodes.*]` binding, no Ray/K8s. Verify **coopmat2** is active on the Vulkan lane
  (~4.4× prefill on a 4090). Heed the upstream caveat: only worth RPC when a model doesn't fit one lane. conf:M eff:M
...
The "one bootc image runs anywhere including WSL" goal collides with a hard upstream wall. Formalize the split.
...
- **T4.1 Dual-personality image.** (a) True **bootc** image for bare-metal/VM; (b) the *same* OCI layers
  ...
  agent-plane services as quadlets so they're byte-identical across both personalities. conf:H eff:L
- **T4.2 bootc bare-metal profile + offline atomic upgrades.** Produce qcow2/raw/iso/vmdk/ami via
  **bootc-image-builder** (note: **no WSL/tar.gz output** — osbuild/bootc-image-builder#172 open; that's the
  ...
  Forge/a registry) or `skopeo copy … oci:/usb` → `bootc switch --transport oci` → `bootc upgrade --apply`.
  Exploit **soft-reboot** for non-kernel updates (seconds of agent-plane downtime); split kernel vs userspace
  deltas. Watch zstd:chunked bugs (#509). conf:H eff:L
- **T4.3 Logically Bound Images for the agent stack.** Bind agent-pipe/ollama-lanes/SearXNG/AdGuard as LBI
  (`/usr/lib/bootc/bound-images.d` + `additionalimagestore=/usr/lib/bootc/storage`) so they version + pre-stage
  atomically with the OS image instead of runtime pulls — directly advances "fully offline." Mind the
  storage.conf GC footgun + b-i-b LBI disk-gen bug (#691). conf:M eff:M
...
- **T5.4 Durability + replay + sleep-time memory (biggest robustness gap).** Event-source the swarm/DAG
  ...
  CPU-pinned `mios-daemon-agent`) that rewrites memory blocks during downtime (Letta sleep-time compute). Adopt
  explicit **Memory-Block** abstraction over raw rows/embeddings. Formalize `_admit` against the ArXiv "Agent
  Control Protocol" (static risk score + stateful trace signals + ledger). conf:M eff:L
...
no-hardcode `mios.toml` SSOT, immutable bootc.
