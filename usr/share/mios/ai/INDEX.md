...
| Role | Path (canonical FHS) | Format | Owner | Edited via |
|---|---|---|---|---|
| **Source of truth** | `~/.config/mios/mios.toml` (per-user, highest precedence) | TOML 1.0.0 | operator | configurator UI / text editor |
|  | `/etc/mios/mios.toml` (per-host) | TOML 1.0.0 | sysadmin | configurator UI / text editor |
...
`mios.toml` is what the operator writes. `install.env` is what the
shell/systemd side reads (`EnvironmentFile=`, `source` in firstboot
scripts). The bridge is `mios-sync-env` -- run after editing
