...
| 8 | search | `knowledge_search` broker (mios-ai) can't read OWUI `webui.db` (perms) → silently returns junk from the wrong store. | ⚠️ FLAGGED — needs the OWUI **0.9.6** api_key written to `/etc/mios/owui-admin.key` (0640 root:mios-ai) + consumer read; the 0.9.6 schema dropped the `user.api_key` column so the minting path needs rework. Mitigated by bug #16 (fallback floor) so off-topic junk no longer surfaces. |
