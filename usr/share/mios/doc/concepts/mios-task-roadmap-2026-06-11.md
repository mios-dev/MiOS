...
1. **WSL always-on (stop the ~30s service cycling)** — *S, high.* FOUNDATIONAL: WSL
   ...
   re-probes MCP, swaps the 8B in/out of VRAM, and **destroys the P0 RadixAttention
   prefix** — and makes every latency/VRAM/eval measurement non-reproducible. Fix =
   a Windows KeepAlive scheduled task running `wsl … --exec /usr/bin/sleep infinity`
...
3. **P6 taint→mask wiring** — *M, high, SECURITY-CRITICAL.* Hard precondition for any
   ...
   tool_call row — the one unverified link.
...
6. **Enable DuckDB + Postgres MCP** — *M, high.* Code is fully ready (Playwright proved
   the pattern). Gates: install `uv`/`uvx` (absent); **CREATE the `mios-ai` pg role**
   (only `mios` exists → postgres-mcp probes ready but fails at call-time); mkdir+chown
   `/var/lib/mios/ai/{tmp,.npm,.cache,.duckdb}` to mios-ai (the EACCES trap). Do AFTER
   #2 (clean embeds) + #3 (taint).
7. **Per-node model consolidation (swarm prerequisite)** — *M, high, RISKY.* Assistant
   ...
   qwen3:1.7b CPU-only confirmation, llama-swap quadlet enable, gemma4 21-alias
...
- **Unverified link:** does the b2 MCP dispatch path persist a tainted tool_call row? (P6 assumes it.)
...
- **Postgres role ordering:** create `mios-ai` role before enabling (it reports ready then fails).
...
Assistant cannot push — operator pushes from `C:\MiOS`.
