...
def _build_openui_html(code: str, title: str = "Response", cdn_base: str = _CDN_BASE) -> str:
    """MiOS-patched: inlines the vendored openui-bundle.min.js + openui-styles.css
    from /usr/share/mios/openui/ so the iframe needs ZERO external requests.
    cdn_base is ignored (preserved as a no-op valve for upstream compat)."""
    safe_title = (
        title.replace("&", "&amp;").replace("<", "&lt;")
        .replace(">", "&gt;").replace('"', "&quot;")
    )
    code_json = json.dumps(code)
    css = _read_local(_BUNDLE_CSS_PATH)
    js  = _read_local(_BUNDLE_JS_PATH)
    if not js:
        return ("<!DOCTYPE html><html><body>"
                "<div style='padding:16px;color:#dc2626;font-family:system-ui;"
...
<meta http-equiv="Content-Security-Policy" content="default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; img-src * data: blob:; font-src data:; object-src 'none'; base-uri 'none'">
<style>{css}</style>
<style>
...
.openui-loading {{ display: flex; align-items: center; justify-content: center;
  ...
  font-size: 14px; }}
.openui-error {{ padding: 16px; color: #dc2626; font-family: system-ui, sans-serif;
...
(function() {{
  try {{
    ...
    root.render(OpenUI.React.createElement(OpenUI.Renderer, {{
      response: code, library: OpenUI.openuiChatLibrary,
      isStreaming: false, onAction: handleAction
    }}));
    setTimeout(reportHeight, 500); setTimeout(reportHeight, 2000);
  }} catch(err) {{
