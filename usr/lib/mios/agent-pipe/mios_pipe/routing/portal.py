# AI-hint: WEB PORTAL helper logic + PWA asset builders + the swarm-roster probe, extracted VERBATIM from server.py (refactor R10 wave). Owns the portal config/auth SSOT (_portal_toml/_pcfg config readers, PORTAL_* scalars, the signed session-cookie token mint/verify _portal_make_token/_portal_token_ok/_portal_authed), the Quadlet service auto-discovery (_portal_unit_hidden masked/gated drop + _discover_portal_services openInBrowser-label scan -> _PORTAL_SERVICES catalog), host telemetry (_host_stats /proc reader, _podman_ps rootful-snapshot container map), the dashboard/login/iostest HTML + PWA manifest/service-worker/icon asset strings (_PORTAL_HTML/_PORTAL_LOGIN_HTML/_IOSTEST_HTML/_PORTAL_MANIFEST/_PORTAL_SW/_PORTAL_ICON*), the SSOT palette injector (_portal_theme_css), the baked-asset reader (_read_portal_asset) and the live agent/node reachability probe (_portal_swarm_probe). A later ROUTE-SURFACE wave also moved the portal route-handler BODIES here VERBATIM (portal_stats_logic, portal_service_detail_logic, portal_swarm_logic, the portal_term_ws_logic ttyd WS bridge, portal_login_page_logic, portal_login_logic, portal_page_logic); a still-later R13 wave moved the 13 @app /portal route WRAPPERS themselves onto this module's portal_router (mounted by server.py via app.include_router), each calling its *_logic / asset builder DIRECTLY -- so the 13 /portal routes now bind via the router while the 4 non-/portal portal routes (/, /login, /sw.js, /iostest) stay thin @app wrappers reaching *_logic through sys.modules; neither the HTTP nor the importable surface changes (surface-parity zero-diff). Moved byte-identically -- the @app portal routes stay in server.py as thin wrappers re-importing these names (surface-parity zero-diff). loads_lenient is imported directly; the server helpers the probe + moved logic call (_probe_auth_headers, _agent_lane, the live _AGENT_REGISTRY roster, _sanitize_tool_text, and the websockets client module) are dependency-INJECTED via configure() (one-way boundary -- this module NEVER imports server).
# AI-related: ./server.py, ./mios_config.py, ./mios_jsonsalvage.py, ./test_mios_portal.py
# AI-functions: _portal_toml, _pcfg, _portal_make_token, _portal_token_ok, _portal_authed, _portal_unit_hidden, _discover_portal_services, _host_stats, _podman_ps, _portal_theme_css, _read_portal_asset, _portal_swarm_probe, portal_stats_logic, portal_service_detail_logic, portal_swarm_logic, portal_term_ws_logic, portal_login_page_logic, portal_login_logic, portal_page_logic, portal_router, portal_stats, portal_service_detail, portal_swarm, portal_icon, portal_icon_192, portal_icon_512, portal_manifest, portal_xterm_js, portal_xterm_css, portal_addon_fit, portal_term_ws, portal_login, portal_logout, portal_sw, portal_login_page, iostest_page, portal_page, configure
