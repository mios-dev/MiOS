...
# host-bootloader edits since UKI/composefs ignore /etc/default/grub).
#
#   mios-vfio-toggle                interactive: pick devices to isolate
#   mios-vfio-toggle --status       same as `mios-vfio-check`
#   mios-vfio-toggle --remove       clear /etc/modprobe.d/vfio.conf
#   mios-vfio-toggle --add <id>     append vendor:device to vfio.conf
#                                   (id format: 10de:2684 -- hex no 0x)
#
# After any change: `sudo bootc upgrade --check` then reboot. The
# composefs-backed root re-renders modprobe.d into the next deployment.
set -euo pipefail
...
case "${1:-}" in
    ...
    --add)
        ...
        [[ "$id" =~ ^[0-9a-fA-F]{4}:[0-9a-fA-F]{4}$ ]] || {
            echo "mios-vfio-toggle --add expects vendor:device hex (e.g. 10de:2684)" >&2
            exit 2
        }
...
EOF
    ...
    "")
        if [[ -x "$TOOL_SH" ]]; then
            ...
            echo "mios-vfio-toggle: $TOOL_SH missing -- use --add <id> / --remove" >&2
            exit 1
        fi
        ;;
    *)
        echo "mios-vfio-toggle: unknown flag '$1' (try --help)" >&2
        exit 2
        ;;
esac
